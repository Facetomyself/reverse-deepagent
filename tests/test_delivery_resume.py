...
class DeliveryResumeWorkflowSchedulerTests(unittest.TestCase):
    ...
    def test_resume_workflow_scheduler_dry_run_recommends_lock_acquire_when_provider_lock_missing(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            readiness = payload["workflow_readiness_plan"]
            self.assertEqual(readiness["status"], "ready_for_review")
            self.assertEqual(readiness["required_approval_actions"], ["resume_acquire_delivery_transaction_lock_provider", "resume_preflight_backend_manifest_recovery"])
            self.assertEqual(readiness["missing_approval_actions"], ["resume_acquire_delivery_transaction_lock_provider", "resume_preflight_backend_manifest_recovery"])
    ...
    def test_resume_workflow_scheduler_apply_blocks_without_all_step_approvals(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(readiness["missing_approval_actions"], ["resume_apply_backend_manifest_recovery"])
            self.assertIn("execute_requires_review_approval_for_all_pending_steps", readiness["failed_checks"])
            self.assertEqual(readiness["next_review_actions"], ["inspect_resume_workflow_scheduler_checks"])
            self.assertFalse(readiness["side_effects_performed"])
            self.assertFalse((root / "backend-artifact-manifest-recovery-preflight.json").exists())
            self.assertFalse((root / "delivery-resume-workflow-journal.json").exists())
    ...
    def test_resume_workflow_scheduler_executes_approved_lock_acquire_and_release_steps(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            readiness = payload["workflow_readiness_plan"]
            self.assertEqual(readiness["status"], "ready_to_execute")
            self.assertEqual(readiness["required_approval_actions"], ["resume_acquire_delivery_transaction_lock_provider", "resume_release_delivery_transaction_lock_provider"])
            self.assertEqual(readiness["missing_approval_actions"], [])
            self.assertEqual(readiness["matched_approval_actions"], ["resume_acquire_delivery_transaction_lock_provider", "resume_release_delivery_transaction_lock_provider"])
            self.assertTrue(readiness["requires_lock_provider_action"])
            self.assertTrue(readiness["requires_fencing_review"])
            self.assertFalse(readiness["side_effects_performed"])
            dependencies = readiness["step_dependency_contexts"]
