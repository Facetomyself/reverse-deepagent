...
class CustomLoaderRecursiveTraversalPlanManagerTests(unittest.TestCase):
    def _loop_execution(self, status: str = "journal_appended") -> dict[str, Any]:
        return {
    ...
    def test_plans_graph_rebuild_after_reviewed_loop_execution(self) -> None:
        spec = CustomLoaderRecursiveTraversalPlanSpec.from_context(
            {
                "custom_loader_recursive_traversal_plan": True,
                "custom_loader_traversal_loop_execution": self._loop_execution(),
                "custom_loader_continuation_journal": {"records": [{"candidate_fingerprint": "a|b|c"}]},
            }
        )
        ...
        self.assertEqual(result.recursive_plan["schema_version"], "reverse-deepagent.custom-loader-recursive-traversal-plan.v1")
        self.assertEqual(result.recursive_plan["next_action"], "rebuild_custom_loader_traversal_graph_before_next_recursive_loop")
        self.assertEqual(result.recursive_plan["journal_record_count"], 1)
        self.assertFalse(result.side_effect_policy["traversal_graph_rebuilt"])
        self.assertFalse(result.side_effect_policy["workflow_replanned"])
        self.assertFalse(result.side_effect_policy["automatic_recursive_traversal"])
    ...
    def test_plans_next_loop_review_after_refreshed_graph_and_workflow(self) -> None:
        ...
        self.assertEqual(result.recursive_plan["next_action"], "review_next_custom_loader_traversal_loop_plan")
        self.assertEqual(result.recursive_plan["follow_up_steps"][0]["action"], "plan_next_bounded_custom_loader_traversal_loop")

    def test_blocks_when_loop_execution_has_not_run(self) -> None:
        result = CustomLoaderRecursiveTraversalPlanManager().plan(
            CustomLoaderRecursiveTraversalPlanSpec.from_context(
                {
                    "custom_loader_recursive_traversal_plan": True,
                    "custom_loader_traversal_loop_execution": self._loop_execution(status="ready_for_review"),
                }
            )
        )
...
class CustomLoaderRecursiveTraversalExecutionManagerTests(unittest.TestCase):
    def _recursive_followup(self) -> dict[str, Any]:
        return {
            ...
            "custom_loader_traversal_loop_plan": {
                "status": "ready_for_review",
                "loop_plan": CustomLoaderTraversalLoopExecutionManagerTests()._loop_plan(),
            },
            "custom_loader_traversal_workflow_plan": {
                "status": "ready_for_review",
                "workflow_plan": CustomLoaderTraversalLoopExecutionManagerTests()._workflow_plan(),
            },
        }

    def test_plan_only_next_loop_execution_requires_manual_review_without_loader(self) -> None:
        page = CustomLoaderExecutionPage()
        spec = CustomLoaderRecursiveTraversalExecutionSpec.from_context(
            {
                "custom_loader_recursive_traversal_execution": True,
                "custom_loader_recursive_traversal_followup": self._recursive_followup(),
            }
        )
        ...
        self.assertEqual(result.status, "ready_for_review")
        self.assertEqual(result.execution["schema_version"], "reverse-deepagent.custom-loader-recursive-traversal-execution.v1")
        self.assertEqual(result.execution["next_action"], "review_custom_loader_recursive_traversal_execution_plan")
        self.assertEqual(page.executions, [])
        self.assertTrue(result.side_effect_policy["plan_only_by_default"])
    ...
    def test_reviewed_next_loop_execution_runs_one_iteration_and_stops(self) -> None:
        ...
        spec = CustomLoaderRecursiveTraversalExecutionSpec.from_context(
            {
                "custom_loader_recursive_traversal_execution": True,
                "custom_loader_recursive_traversal_followup": self._recursive_followup(),
                "custom_loader_traversal_plan": CustomLoaderTraversalLoopExecutionManagerTests()._traversal_plan(),
                "plan_continuation_workflow": True,
                "run_preflight": True,
                "execute_custom_loader": True,
                "run_module_diff": True,
        ...
        self.assertEqual(result.status, "next_loop_journal_appended")
        self.assertEqual(page.executions, ["window.__customLoader.load"])
        self.assertEqual(result.execution["loop_execution_status"], "journal_appended")
        self.assertEqual(result.execution["next_action"], "plan_next_custom_loader_recursive_traversal_checkpoint")
    ...
    def test_blocks_next_loop_execution_flags_without_review_approval(self) -> None:
        result = CustomLoaderRecursiveTraversalExecutionManager().execute(
            CustomLoaderExecutionPage(),
            CustomLoaderRecursiveTraversalExecutionSpec.from_context(
                {
                    "custom_loader_recursive_traversal_execution": True,
                    "custom_loader_recursive_traversal_followup": self._recursive_followup(),
                    "execute_custom_loader": True,
        ...
        self.assertEqual(result.status, "blocked")
        self.assertEqual(result.reason, "review_approval_required")
        self.assertEqual(result.execution["next_action"], "resolve_custom_loader_recursive_traversal_execution_blockers")
        self.assertFalse(result.side_effect_policy["loader_invoked"])
        self.assertFalse(result.side_effect_policy["writes_journal"])
        self.assertFalse(result.side_effect_policy["automatic_recursive_traversal"])
...
class ModuleFederationRecursiveTraversalExecutionManagerTests(unittest.TestCase):
    def _workflow_plan(self) -> dict[str, Any]:
        return ModuleFederationTraversalWorkflowExecutionManagerTests()._workflow_plan()

    def _followup(self) -> dict[str, Any]:
        return {
            "schema_version": "reverse-deepagent.module-federation-recursive-traversal-followup.v1",
            "status": "next_step_review_ready",
            "recursive_plan_id": "module-federation-recursive-traversal-plan",
            "next_action": "review_next_module_federation_traversal_workflow_execution",
        }

    def test_plan_only_next_step_execution_requires_manual_review_without_remote_code(self) -> None:
        page = ModuleFederationFactoryPage()
        result = ModuleFederationRecursiveTraversalExecutionManager().execute(
            page,
            ModuleFederationRecursiveTraversalExecutionSpec.from_context(
                {
                    "module_federation_recursive_traversal_execution": True,
                    "module_federation_recursive_traversal_followup": self._followup(),
                    "module_federation_traversal_workflow_plan": self._workflow_plan(),
                }
            ),
        )
        ...
        self.assertEqual(result.status, "ready_for_review")
        self.assertEqual(result.execution["schema_version"], "reverse-deepagent.module-federation-recursive-traversal-execution.v1")
        self.assertEqual(result.execution["workflow_execution_status"], "ready_for_review")
        self.assertEqual(result.execution["next_action"], "review_module_federation_recursive_traversal_execution_plan")
        self.assertEqual(page.factory_invocations, [])
        self.assertTrue(result.side_effect_policy["plan_only_by_default"])
    ...
    def test_reviewed_next_step_invokes_one_remote_factory_and_stops(self) -> None:
        page = ModuleFederationFactoryPage()
        result = ModuleFederationRecursiveTraversalExecutionManager().execute(
            page,
            ModuleFederationRecursiveTraversalExecutionSpec.from_context(
        ...
        self.assertEqual(result.status, "next_step_execution_progressed")
        self.assertEqual(result.execution["workflow_execution_status"], "factory_invoke_success")
        self.assertEqual(result.execution["next_action"], "continue_reviewed_module_federation_traversal_step_or_plan_next_checkpoint")
        self.assertEqual(page.factory_invocations, ["./sign"])
        self.assertTrue(result.side_effect_policy["workflow_execution_started"])
        self.assertTrue(result.side_effect_policy["remote_factory_invoked"])
        self.assertTrue(result.side_effect_policy["remote_code_executed"])
        self.assertFalse(result.side_effect_policy["traversal_graph_rebuilt"])
        self.assertFalse(result.side_effect_policy["workflow_replanned"])
    ...
    def test_blocks_next_step_execution_flags_without_review_approval(self) -> None:
        page = ModuleFederationFactoryPage()
        result = ModuleFederationRecursiveTraversalExecutionManager().execute(
            page,
            ModuleFederationRecursiveTraversalExecutionSpec.from_context(
...
class AsyncChunkTraversalLoopPlanManagerTests(unittest.TestCase):
    def _workflow_plan(self) -> dict[str, Any]:
        return AsyncChunkTraversalWorkflowExecutionManagerTests()._workflow_plan()

    def test_plans_bounded_loop_without_executing_runtime(self) -> None:
        spec = AsyncChunkTraversalLoopPlanSpec.from_context(
            {
                "async_chunk_traversal_loop_plan": True,
                "async_chunk_traversal_workflow_plan": self._workflow_plan(),
                "max_loop_iterations": 2,
            }
        )
        ...
        self.assertEqual(result.status, "ready_for_review")
        self.assertEqual(result.loop_plan["schema_version"], "reverse-deepagent.async-chunk-traversal-loop-plan.v1")
        self.assertEqual(result.loop_plan["planned_iteration_count"], 1)
        self.assertEqual(result.loop_plan["iterations"][0]["chunk_id"], "731")
        self.assertEqual(result.loop_plan["next_action"], "review_async_chunk_traversal_loop_plan")
        self.assertFalse(result.side_effect_policy["runtime_loader_executed"])
        self.assertFalse(result.side_effect_policy["chunk_request_sent"])
        self.assertFalse(result.side_effect_policy["automatic_loop_execution"])
        self.assertFalse(result.side_effect_policy["automatic_queue_advance"])
    ...
    def test_latest_execution_requires_graph_rebuild_checkpoint(self) -> None:
        spec = AsyncChunkTraversalLoopPlanSpec.from_context(
            {
                "async_chunk_traversal_loop_plan": True,
                "async_chunk_traversal_workflow_plan": self._workflow_plan(),
        ...
        self.assertEqual(result.status, "ready_for_review")
        self.assertEqual(result.loop_plan["latest_workflow_execution_status"], "module_diff_ready")
        self.assertEqual(
            result.loop_plan["next_action"],
            "rerun_module_discovery_and_rebuild_async_chunk_traversal_graph_before_next_loop_iteration",
        )
        self.assertFalse(result.loop_plan["loop_checkpoint_policy"]["automatic_queue_advance"])

    def test_blocks_without_workflow_plan(self) -> None:
        result = AsyncChunkTraversalLoopPlanManager().plan(
            AsyncChunkTraversalLoopPlanSpec.from_context({"async_chunk_traversal_loop_plan": True})
        )
...
class AsyncChunkTraversalLoopExecutionManagerTests(unittest.TestCase):
    def _workflow_plan(self) -> dict[str, Any]:
        return AsyncChunkTraversalWorkflowExecutionManagerTests()._workflow_plan()

    def _loop_plan(self) -> dict[str, Any]:
        return AsyncChunkTraversalLoopPlanManager().plan(
            AsyncChunkTraversalLoopPlanSpec.from_context(
                {
                    "async_chunk_traversal_loop_plan": True,
                    "async_chunk_traversal_workflow_plan": self._workflow_plan(),
                    "max_loop_iterations": 2,
    ...
    def test_plan_only_selects_iteration_without_executing_runtime(self) -> None:
        page = AsyncChunkLoadPage()
        spec = AsyncChunkTraversalLoopExecutionSpec.from_context(
            {
                "async_chunk_traversal_loop_execution": True,
                "async_chunk_traversal_loop_plan": self._loop_plan(),
                "async_chunk_traversal_workflow_plan": self._workflow_plan(),
            }
        )
        ...
        self.assertEqual(result.status, "ready_for_review")
        self.assertEqual(result.execution["schema_version"], "reverse-deepagent.async-chunk-traversal-loop-execution.v1")
        self.assertEqual(result.execution["selected_iteration_index"], 0)
        self.assertEqual(result.execution["selected_step_index"], 0)
        self.assertEqual(result.execution["selected_candidate_index"], 0)
        self.assertEqual(result.execution["next_action"], "review_async_chunk_traversal_loop_execution_plan")
        self.assertFalse(page.executed_chunk_ids)
        self.assertFalse(result.side_effect_policy["runtime_loader_executed"])
        self.assertFalse(result.side_effect_policy["chunk_request_sent"])
        self.assertFalse(result.side_effect_policy["automatic_loop_execution"])
    ...
    def test_executes_one_reviewed_iteration_and_stops_before_next_loop(self) -> None:
        ...
        self.assertEqual(result.status, "module_diff_ready")
        self.assertEqual(page.executed_chunk_ids, ["731"])
        self.assertEqual(result.execution["workflow_execution_status"], "module_diff_ready")
        self.assertEqual(result.execution["next_action"], "review_async_chunk_module_diff_then_rebuild_graph")
        self.assertTrue(result.side_effect_policy["runtime_loader_executed"])
        self.assertTrue(result.side_effect_policy["chunk_request_sent"])
        self.assertTrue(result.side_effect_policy["module_diff_executed"])
        self.assertFalse(result.side_effect_policy["traversal_graph_rebuilt"])
    ...
    def test_blocks_without_loop_plan(self) -> None:
        result = AsyncChunkTraversalLoopExecutionManager().execute(
            AsyncChunkLoadPage(),
            AsyncChunkTraversalLoopExecutionSpec.from_context({"async_chunk_traversal_loop_execution": True}),
        )
