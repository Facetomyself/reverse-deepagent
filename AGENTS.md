...
- 改 rebuild / strategy：检查 `tests/test_rebuild_artifacts.py`、`tests/test_strategy_*`；涉及 `evidence_score`、review gate 或 delivery gate 消费时，还必须检查 `tests/test_review_gate.py`、`tests/test_review_subagent.py`，并证明 score 只读消费、不改变 rebuild `ready` 计算、不采集 runtime context、不执行 replay、不启动浏览器、不调用 MCP、不触碰移动端完整链路。
