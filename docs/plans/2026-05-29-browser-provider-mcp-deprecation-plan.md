...
See `.codex/plans/browser-provider-mcp-deprecation-plan.md` for the detailed execution records. Current status: Phase 0-162 is implemented through the review-gated Module Federation recursive traversal follow-up checkpoint baseline after the review-only Module Federation recursive traversal follow-up plan baseline. The remaining active work is capability-gated Web / BrowserProvider / workspace / delivery hardening; Android / iOS / mini-program full runtime chains remain deferred.
