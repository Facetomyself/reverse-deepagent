...
See `.codex/plans/browser-provider-mcp-deprecation-plan.md` for the detailed execution records. Current status: Phase 0-224 is implemented through the review-only legacy fallback tightening apply plan descriptor after the readiness descriptor. The current workspace migration chain remains readiness score -> dual-write expansion plan -> observed expansion result -> foldered-canonical pilot plan -> preflight / rollback descriptor -> apply-plan descriptor -> approval / transaction descriptor -> manifest dry-run / rollback checkpoint descriptor -> physical apply preflight descriptor -> explicit physical apply executor -> post-apply validation descriptor -> post-apply validation result writer -> legacy fallback tightening readiness descriptor -> legacy fallback tightening apply plan descriptor; legacy fallback tightening executor and final migration finalization remain separate reviewed follow-ups. The remaining active work is capability-gated Web / BrowserProvider / workspace / delivery hardening; Android / iOS / mini-program full runtime chains remain deferred.
