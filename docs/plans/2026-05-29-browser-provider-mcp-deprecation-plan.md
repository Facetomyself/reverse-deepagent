...
See `.codex/plans/browser-provider-mcp-deprecation-plan.md` for the detailed execution records. Current status: Phase 0-173 is implemented through the reviewed closure wrapper runtime mutability probe/result baseline after the runtime mutability preflight baseline. Earlier closure wrapper event harvesting and reviewed install / restore baselines remain in place. The remaining active work is capability-gated Web / BrowserProvider / workspace / delivery hardening; Android / iOS / mini-program full runtime chains remain deferred.
