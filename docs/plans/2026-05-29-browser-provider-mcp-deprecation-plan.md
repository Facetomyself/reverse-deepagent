...
See `.codex/plans/browser-provider-mcp-deprecation-plan.md` for the detailed execution records. Current status: Phase 0-232 is implemented through the read-only foldered-canonical migration post-finalization audit descriptor after the explicit finalization executor. The current workspace migration chain remains readiness score -> dual-write expansion plan -> observed expansion result -> foldered-canonical pilot plan -> preflight / rollback descriptor -> apply-plan descriptor -> approval / transaction descriptor -> manifest dry-run / rollback checkpoint descriptor -> physical apply preflight descriptor -> explicit physical apply executor -> post-apply validation descriptor -> post-apply validation result writer -> legacy fallback tightening readiness descriptor -> legacy fallback tightening apply plan descriptor -> legacy fallback tightening executor preflight descriptor -> explicit legacy fallback tightening executor -> finalization readiness descriptor -> finalization plan descriptor -> finalization preflight descriptor -> explicit finalization executor -> post-finalization audit descriptor; broader rollout apply remains a separate reviewed follow-up. The remaining active work is capability-gated Web / BrowserProvider / workspace / delivery hardening; Android / iOS / mini-program full runtime chains remain deferred.
...
Boundary: this closes only the review planning layer before any actual foldered-canonical migration. It does not create directories, run pipelines, enable dual-write, migrate paths, change canonical paths, mutate manifests, start browsers, send CDP commands, call MCP, or touch Android / iOS / mini-program full runtime chains. Any physical canonical-path migration executor remains a separate explicit-review follow-up with rollback evidence.
...
The DeepAgents agent tool list includes `review_workspace_foldered_canonical_broader_rollout_preflight`, and the workspace contract indexes `workspace/workspace-foldered-canonical-broader-rollout-preflight.json` with future alias `/workspace/review/workspace-foldered-canonical-broader-rollout-preflight.json` and category `audit`. Step 233 broader rollout plans now advertise the preflight tool as implemented while keeping `execute_workspace_foldered_canonical_broader_rollout` unimplemented.

Boundary: this closes only the read-only broader rollout preflight layer. It does not write preflight artifacts, record approvals, write journals, mutate manifests, move files, change canonical paths, tighten legacy fallback, enable dual-write, authorize broader rollout apply, run pipelines, start browsers, send CDP commands, call MCP, or touch Android / iOS / mini-program full runtime chains. The broader rollout executor remains a separate explicit-review-only follow-up.
