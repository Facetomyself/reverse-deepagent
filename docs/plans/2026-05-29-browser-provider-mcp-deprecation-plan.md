...
See `.codex/plans/browser-provider-mcp-deprecation-plan.md` for the detailed execution records. Current status: Phase 0-177 is implemented through the closure wrapper strategy descriptor catalog baseline after the paused-session diagnostics hardening and optional runtime-mutability-result-gated closure wrapper replacement execution baseline. Earlier closure wrapper event harvesting and reviewed install / restore baselines remain in place. The remaining active work is capability-gated Web / BrowserProvider / workspace / delivery hardening; Android / iOS / mini-program full runtime chains remain deferred.
