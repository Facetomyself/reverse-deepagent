...
- `virtual://workspace/closure-functions.json` and `virtual://workspace/closure-function-candidates.json` artifact refs when explicit closure-scope function discovery is requested; `virtual://workspace/closure-wrapper-replacement-plan.json` when review-only closure wrapper replacement planning is requested
...
See `.codex/plans/browser-provider-mcp-deprecation-plan.md` for the detailed execution records. Current status: Phase 0-166 is implemented through the read-only paused-session live-continuation preflight baseline after the review-gated Module Federation recursive continuation checkpoint execution baseline. The remaining active work is capability-gated Web / BrowserProvider / workspace / delivery hardening; Android / iOS / mini-program full runtime chains remain deferred.
