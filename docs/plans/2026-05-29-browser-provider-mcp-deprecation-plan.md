...
See `.codex/plans/browser-provider-mcp-deprecation-plan.md` for the detailed execution records. Current status: Phase 0-237 is implemented through the review-only foldered-canonical broader rollout rollback-vs-commit decision plan descriptor after the read-only broader rollout post-audit descriptor. The current workspace migration chain remains readiness score -> dual-write expansion plan -> observed expansion result -> foldered-canonical pilot plan -> preflight / rollback descriptor -> apply-plan descriptor -> approval / transaction descriptor -> manifest dry-run / rollback checkpoint descriptor -> physical apply preflight descriptor -> explicit physical apply executor -> post-apply validation descriptor -> post-apply validation result writer -> legacy fallback tightening readiness descriptor -> legacy fallback tightening apply plan descriptor -> legacy fallback tightening executor preflight descriptor -> explicit legacy fallback tightening executor -> finalization readiness descriptor -> finalization plan descriptor -> finalization preflight descriptor -> explicit finalization executor -> post-finalization audit descriptor -> broader rollout readiness descriptor -> broader rollout plan descriptor -> broader rollout preflight descriptor -> explicit broader rollout executor -> broader rollout post-audit descriptor -> rollback-vs-commit decision plan descriptor. The remaining active work is decision recording / explicit commit-or-rollback executors plus capability-gated Web / BrowserProvider / workspace / delivery hardening; Android / iOS / mini-program full runtime chains remain deferred.
...
Boundary: this closes the explicit broader rollout metadata executor layer only. Step 236 adds read-only post-rollout audit; rollback-vs-commit decision recording / explicit commit-or-rollback executors / automatic materialization / browser / CDP / MCP automation / mobile full runtime chains remain separate reviewed or deferred work.
