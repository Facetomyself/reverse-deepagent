...
See `.codex/plans/browser-provider-mcp-deprecation-plan.md` for the detailed execution records. Current status: Phase 0-169 is implemented through the reviewed same-process closure wrapper restore execution baseline after the reviewed closure wrapper replacement execution MVP. The remaining active work is capability-gated Web / BrowserProvider / workspace / delivery hardening; Android / iOS / mini-program full runtime chains remain deferred.
