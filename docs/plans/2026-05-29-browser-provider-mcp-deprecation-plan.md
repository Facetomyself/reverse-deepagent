...
See `.codex/plans/browser-provider-mcp-deprecation-plan.md` for the detailed execution records. Current status: Phase 0-229 is implemented through the read-only foldered-canonical migration finalization preflight descriptor after the finalization plan descriptor. The current workspace migration chain remains readiness score -> dual-write expansion plan -> observed expansion result -> foldered-canonical pilot plan -> preflight / rollback descriptor -> apply-plan descriptor -> approval / transaction descriptor -> manifest dry-run / rollback checkpoint descriptor -> physical apply preflight descriptor -> explicit physical apply executor -> post-apply validation descriptor -> post-apply validation result writer -> legacy fallback tightening readiness descriptor -> legacy fallback tightening apply plan descriptor -> legacy fallback tightening executor preflight descriptor -> explicit legacy fallback tightening executor -> finalization readiness descriptor -> finalization plan descriptor -> finalization preflight descriptor; finalization executor remains a separate reviewed follow-up. The remaining active work is capability-gated Web / BrowserProvider / workspace / delivery hardening; Android / iOS / mini-program full runtime chains remain deferred.
...
The DeepAgents agent tool list includes `review_workspace_foldered_canonical_migration_finalization_preflight`, and the workspace contract indexes `workspace/workspace-foldered-canonical-migration-finalization-preflight.json` with future alias `/workspace/review/workspace-foldered-canonical-migration-finalization-preflight.json` and category `audit`. The descriptor intentionally reports `execute_workspace_foldered_canonical_migration_finalization` as the next separate follow-up tool that is not implemented yet.

Boundary: this closes only the read-only preflight review layer before explicit finalization execution. It does not write artifacts, record approvals, write journals, mutate manifests, move artifacts, change canonical paths, run pipelines, enable dual-write, execute foldered-canonical finalization, decide rollback vs commit, start browsers, send CDP commands, call MCP, or touch Android / iOS / mini-program full runtime chains. Finalization executor and finalization result recording remain separate reviewed follow-ups.
