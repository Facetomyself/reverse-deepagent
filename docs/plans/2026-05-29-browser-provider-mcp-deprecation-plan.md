...
See `.codex/plans/browser-provider-mcp-deprecation-plan.md` for the detailed execution records. Current status: Phase 0-209 is implemented through the read-only cross-process paused-session lifecycle descriptor, review-only paused-session multi-step loop plan descriptor, and review-only closure wrapper continuation execution plan descriptor, review-gated one-iteration wrapper continuation execution, read-only wrapper continuation checkpoint, review-only wrapper continuation next-iteration plan, and review-gated wrapper continuation next-iteration execution after the read-only closure wrapper continuation readiness descriptor and the review-gated one-iteration multi-step paused-session continuation execution baseline. Earlier closure wrapper event harvesting and reviewed install / restore baselines remain in place. The remaining active work is capability-gated Web / BrowserProvider / workspace / delivery hardening; Android / iOS / mini-program full runtime chains remain deferred.
