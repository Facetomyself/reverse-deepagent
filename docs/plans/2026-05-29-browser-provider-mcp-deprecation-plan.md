...
See `.codex/plans/browser-provider-mcp-deprecation-plan.md` for the detailed execution records. Current status: Phase 0-160 is implemented through the review-gated Module Federation traversal workflow execution baseline after the review-only Module Federation traversal graph / workflow plan baseline. The remaining active work is capability-gated Web / BrowserProvider / workspace / delivery hardening; Android / iOS / mini-program full runtime chains remain deferred.
