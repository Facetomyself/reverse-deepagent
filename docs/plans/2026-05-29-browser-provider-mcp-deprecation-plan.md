...
See `.codex/plans/browser-provider-mcp-deprecation-plan.md` for the detailed execution records. Current status: Phase 0-245 is implemented through the review-only paused-session automatic-loop executor approval / transaction plan descriptor after the read-only executor preflight descriptor. The current workspace migration chain remains readiness score -> dual-write expansion plan -> observed expansion result -> foldered-canonical pilot plan -> preflight / rollback descriptor -> apply-plan descriptor -> approval / transaction descriptor -> manifest dry-run / rollback checkpoint descriptor -> physical apply preflight descriptor -> explicit physical apply executor -> post-apply validation descriptor -> post-apply validation result writer -> legacy fallback tightening readiness descriptor -> legacy fallback tightening apply plan descriptor -> legacy fallback tightening executor preflight descriptor -> explicit legacy fallback tightening executor -> finalization readiness descriptor -> finalization plan -> finalization preflight -> explicit finalization executor -> post-finalization audit -> broader rollout readiness -> broader rollout plan -> broader rollout preflight -> explicit broader rollout executor -> broader rollout post-audit -> rollback-vs-commit decision plan -> decision record writer -> explicit commit executor / rollback preflight -> explicit rollback executor. The remaining active work is capability-gated Web / BrowserProvider / workspace / delivery hardening; Android / iOS / mini-program full runtime chains remain deferred.
