...
See `.codex/plans/browser-provider-mcp-deprecation-plan.md` for the detailed execution records. Current status: Phase 0-188 is implemented through the review-only multi-step paused-session continuation workflow / journal plan baseline after the pre-action subscribe-and-action orchestration baseline. Earlier closure wrapper event harvesting and reviewed install / restore baselines remain in place. The remaining active work is capability-gated Web / BrowserProvider / workspace / delivery hardening; Android / iOS / mini-program full runtime chains remain deferred.
