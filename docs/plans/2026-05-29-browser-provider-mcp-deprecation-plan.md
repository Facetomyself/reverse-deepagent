...
See `.codex/plans/browser-provider-mcp-deprecation-plan.md` for the detailed execution records. Current status: Phase 0-157 is implemented through the review-gated custom-loader recursive traversal next-loop execution baseline after the review-gated custom-loader recursive traversal follow-up checkpoint baseline. The remaining active work is capability-gated Web / BrowserProvider / workspace / delivery hardening; Android / iOS / mini-program full runtime chains remain deferred.
