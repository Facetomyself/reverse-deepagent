...
See `.codex/plans/browser-provider-mcp-deprecation-plan.md` for the detailed execution records. Current status: Phase 0-189 is implemented through the review-gated one-iteration multi-step paused-session continuation execution baseline after the review-only workflow / journal plan baseline. Earlier closure wrapper event harvesting and reviewed install / restore baselines remain in place. The remaining active work is capability-gated Web / BrowserProvider / workspace / delivery hardening; Android / iOS / mini-program full runtime chains remain deferred.
