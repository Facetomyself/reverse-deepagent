...
See `.codex/plans/browser-provider-mcp-deprecation-plan.md` for the detailed execution records. Current status: Phase 0-161 is implemented through the review-only Module Federation recursive traversal follow-up plan baseline after the review-gated Module Federation traversal workflow execution baseline. The remaining active work is capability-gated Web / BrowserProvider / workspace / delivery hardening; Android / iOS / mini-program full runtime chains remain deferred.
