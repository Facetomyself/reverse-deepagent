...
See `.codex/plans/browser-provider-mcp-deprecation-plan.md` for the detailed execution records. Current status: Phase 0-148 is implemented through the review-only bounded custom-loader traversal loop plan baseline after the review-gated custom-loader traversal workflow execution baseline. The remaining active work is capability-gated Web / BrowserProvider / workspace / delivery hardening; Android / iOS / mini-program full runtime chains remain deferred.
