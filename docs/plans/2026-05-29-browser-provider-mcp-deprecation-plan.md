...
See `.codex/plans/browser-provider-mcp-deprecation-plan.md` for the detailed execution records. Current status: Phase 0-230 is implemented through the explicit-review-only foldered-canonical migration finalization executor after the finalization preflight descriptor. The current workspace migration chain remains readiness score -> dual-write expansion plan -> observed expansion result -> foldered-canonical pilot plan -> preflight / rollback descriptor -> apply-plan descriptor -> approval / transaction descriptor -> manifest dry-run / rollback checkpoint descriptor -> physical apply preflight descriptor -> explicit physical apply executor -> post-apply validation descriptor -> post-apply validation result writer -> legacy fallback tightening readiness descriptor -> legacy fallback tightening apply plan descriptor -> legacy fallback tightening executor preflight descriptor -> explicit legacy fallback tightening executor -> finalization readiness descriptor -> finalization plan descriptor -> finalization preflight descriptor -> explicit finalization executor; post-finalization audit and broader rollout remain separate reviewed follow-ups. The remaining active work is capability-gated Web / BrowserProvider / workspace / delivery hardening; Android / iOS / mini-program full runtime chains remain deferred.
...
Boundary: this closes only the review planning layer before any finalization preflight / execution. It does not write artifacts, record approvals, write journals, mutate manifests, move artifacts, change canonical paths, run pipelines, enable dual-write, execute foldered-canonical finalization, decide rollback vs commit, start browsers, send CDP commands, call MCP, or touch Android / iOS / mini-program full runtime chains. Finalization preflight and explicit finalization execution are now implemented as separate reviewed steps; post-finalization audit remains a separate reviewed follow-up.
...
Boundary: this closes the explicit finalization metadata executor layer only. Broader dual-write rollout, post-finalization audit, automatic materialization, rollback-vs-commit decisioning, browser / CDP / MCP automation, and Android / iOS / mini-program full runtime chains remain separate reviewed or deferred work.
