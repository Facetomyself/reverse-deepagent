...
- Source-map handling covers generated offsets, exact / bias remap, `sourceRoot`, indexed sections, `names` metadata, URL equivalence, nested indexed-section metadata, review-only generated/original lookup descriptors, and review-gated credentialless Source Map fetch metadata.
...
- Full source-map consumer semantics and bundler-specific symbol scoping beyond the current review-only source-map lookup / symbol-scope descriptors.
