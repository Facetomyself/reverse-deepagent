...
- Closure wrapper replacement hardening beyond the current same-process reviewed `log-only-call-through` install / restore execution baselines, including wrapper event harvesting, richer assignment safety proofs, and cross-process live CDP continuation integration.
