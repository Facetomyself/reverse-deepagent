...
`assess_workspace_consumer_readiness_score` adds a read-only scoring layer for the same migration seam. It consumes the static consumer audit, optional migration readiness report, optional delivery source audit, and optional dual-write pilot result JSON, then emits `reverse-deepagent.workspace-consumer-readiness-score.v1` with resolver adoption, virtual URI adoption, future path readiness, source_path risk, dual-write pilot evidence, and foldered-canonical readiness scores. The descriptor is indexed as `workspace/workspace-consumer-readiness-score.json` with future alias `/workspace/review/workspace-consumer-readiness-score.json`; it does not inspect files, write artifacts, enable dual-write, migrate paths, start browsers, send CDP commands, call MCP, or touch mobile full runtime chains.

`plan_workspace_dual_write_expansion` consumes that score plus optional verified pilot evidence and reviewed artifact keys to emit `reverse-deepagent.workspace-dual-write-expansion-plan.v1`. It blocks broader expansion until a verified scoped dual-write pilot result is present, defaults to low-risk workspace artifacts, and only returns reviewable opt-in scopes plus dual-write path plans. The descriptor is indexed as `workspace/workspace-dual-write-expansion-plan.json` with future alias `/workspace/review/workspace-dual-write-expansion-plan.json`; it still does not run the pipeline, write artifacts, enable dual-write, migrate paths, or change canonical paths.

`review_workspace_dual_write_expansion_workflow` and `record_workspace_dual_write_expansion_result` close the observed-result review loop after a separately approved scoped expansion run. The workflow consumes a ready `workspace-dual-write-expansion-plan` plus optional observed `workspace-dual-write-plan.json`, reports `ready_for_review` when no run has been observed, and reports `verified` / `partial` / `blocked` after inspecting planned legacy / future files and SHA-256 parity. `write_result=true` writes only `workspace/workspace-dual-write-expansion-result.json`, indexed with future alias `/workspace/review/workspace-dual-write-expansion-result.json`; the workflow route `workspace/workspace-dual-write-expansion-workflow.json` is also indexed under `/workspace/review/`. These tools do not run pipelines, enable dual-write, migrate paths, change canonical paths, start browsers, send CDP commands, call MCP, or touch mobile full runtime chains.

`plan_workspace_foldered_canonical_migration_pilot` adds the next review-only gate after verified expansion evidence. It consumes a foldered-canonical-ready consumer readiness score plus a verified `workspace-dual-write-expansion-result` descriptor, optionally narrows the scope by reviewed artifact keys, and emits `reverse-deepagent.workspace-foldered-canonical-migration-pilot-plan.v1` with future canonical path candidates, virtual URIs, risk metadata, and rollback-plan requirements. The route is indexed as `workspace/workspace-foldered-canonical-migration-pilot-plan.json` with future alias `/workspace/review/workspace-foldered-canonical-migration-pilot-plan.json`. It is plan-only: legacy flat paths remain authoritative, physical migration is disabled, and no files, manifests, pipelines, browsers, CDP sessions, MCP calls, or mobile full runtime chains are touched.

`review_workspace_foldered_canonical_migration_preflight` adds a read-only execution preflight after a ready pilot plan. It consumes inline JSON or the `workspace_foldered_canonical_migration_pilot_plan` artifact ref, inspects existing legacy/current canonical files and future canonical candidate files, compares SHA-256 digests, and emits `reverse-deepagent.workspace-foldered-canonical-migration-preflight.v1` with candidate readiness, rollback requirements, and an explicit reviewed-execution gate. The route is indexed as `workspace/workspace-foldered-canonical-migration-preflight.json` with future alias `/workspace/review/workspace-foldered-canonical-migration-preflight.json`. It does not write artifacts, create directories, run pipelines, enable dual-write, migrate paths, change canonical paths, mutate manifests, start browsers, send CDP commands, call MCP, or touch mobile full runtime chains.
...
`packages/reverse-deepagent-browser-provider-browserbase-cdp/` is a real Browserbase hosted CDP provider package baseline. It declares `browserbase-cdp`, keeps registration and matrix listing side-effect-free, and never reads `BROWSERBASE_API_KEY`, creates sessions, opens sockets, or invokes factories during metadata paths. Explicit `connect()` attaches only to a reviewed Browserbase `connectUrl`; explicit `start()` is the only path that can call the Browserbase Session API to obtain a `connectUrl`, and local `stop()` only closes CDP state. Capability metadata and event logs redact API base URLs, connect URLs, project ids, session ids, and access-material presence.
...
`packages/reverse-deepagent-strategy-detector-template/` is the copy-and-replace package template for external strategy detector integrations. It declares the `reverse_deepagent.strategy_detectors` entry point and returns a `StrategyDetectorProviderRegistration` for `template-strategy-detector` without running detector logic. Metadata listing exposes non-secret rule / emits / side-effect policy data only; explicit detection is the only path that invokes the detector callable, and the template stays pure source-pattern based without runtime context collection, browser startup, JavaScript evaluation, replay execution, MCP calls, or mobile runtime usage.
...
- source-level logpoint baseline can install at script URL / line-number breakpoints, optionally remap bundle offsets or Source Map v3 original source locations to generated CDP line / column positions, including exact matches, greatest-lower-bound bias fallback, sourceRoot-aware source matching, indexed source map section offsets, source-map `names` metadata, URL-like source equivalence, and nested indexed-section stack metadata; `source-map-source-content` adds a review-only descriptor that consumes caller-provided Source Map `sourcesContent`, emits `workspace/source-map-source-content.json`, and exports only availability / digest / size / line-count metadata without raw source text or previews; `source-map-readiness` adds a review-only join descriptor that consumes existing lookup / source-content / bundler-symbol-scope / fetch metadata descriptors, emits `workspace/source-map-readiness.json`, and tells reviewers whether debugger location, rebuild source metadata, and source-logpoint planning evidence is ready for review without fetching maps, exporting raw source, exporting previews, starting browsers, installing logpoints, evaluating JavaScript, sending CDP, calling MCP, or touching mobile runtimes; `source-map-consumer-action-plan` consumes that readiness plus lookup / source-content / symbol-scope descriptors, emits `workspace/source-map-consumer-action-plan.json`, and produces plan-only reviewed consumer actions for debugger location use, source-logpoint planning, rebuild source metadata use, and hook symbol-scope review without executing debugger commands, writing rebuilds, installing logpoints or hooks, evaluating JavaScript, sending CDP, calling MCP, or touching mobile runtimes; `source-map-consumer-materialization` consumes a ready action plan plus optional lookup / source-content / symbol-scope descriptors, emits `workspace/source-map-consumer-materialization.json`, and materializes selected action ids / consumers into typed review payloads for debugger location, source-logpoint, rebuild metadata, and hook symbol-scope use without executing debugger commands, installing logpoints or hooks, running rebuilds, exporting raw source or previews, fetching Source Maps, starting browsers, evaluating JavaScript, sending CDP, calling MCP, or touching mobile runtimes; `bundler-symbol-scope` adds a review-only descriptor that consumes caller-provided Source Map / script / module hints, classifies conservative bundler signals, maps requested symbols to generated candidates, emits `workspace/bundler-symbol-scope.json`, and points reviewers toward source-logpoint / hook readiness without fetching maps, installing logpoints, evaluating JavaScript, sending CDP, calling MCP, or touching mobile runtimes; a separate review-gated external Source Map fetch baseline can plan or explicitly fetch same-origin / allowlisted `sourceMappingURL` and indexed-section `url` payload metadata with credentialless Python requests; source logpoints capture a log expression into `workspace/source-logpoints.json` plus `workspace/source-logpoint-timeline.json`, and optionally pause if explicitly requested.
...
- Provider-neutral strategy evidence scoring baseline for review-only `evidence_score` payloads across strategy detection and rebuild plans; review gate consumes score labels / blockers as conservative delivery-review signals while still not changing rebuild readiness, executing replay, starting browsers, or calling MCP.
- StrategyDetector provider registry baseline for `reverse_deepagent.strategy_detectors`, including built-in detector provider metadata and a copy-and-replace external template package; metadata listing is side-effect-free and does not execute detectors, collect runtime context, run replay, start browsers, send CDP, evaluate JavaScript, call MCP, or touch mobile runtimes.
...
- Workspace artifact reader, read-only review-helper artifact-ref resolver, rebuild generation artifact-ref inputs, delivery artifact-list resolver, delivery source compatibility audit, workspace migration readiness report, workspace consumer readiness score, opt-in dual-write expansion plan, expansion observed-result verifier / review workflow, foldered-canonical migration pilot plan, foldered-canonical migration preflight / rollback descriptor, limited dual-write pilot plan, review workflow, pure-Python scoped dual-write pilot smoke CLI, resolver compatibility metrics, and consumer adoption audit baselines exposed through `read_workspace_artifact`, `audit_workspace_artifact_consumers`, `assess_workspace_migration_readiness`, `assess_workspace_consumer_readiness_score`, `plan_workspace_dual_write_expansion`, `review_workspace_dual_write_expansion_workflow`, `record_workspace_dual_write_expansion_result`, `plan_workspace_foldered_canonical_migration_pilot`, `review_workspace_foldered_canonical_migration_preflight`, `plan_workspace_dual_write_pilot`, `review_workspace_dual_write_pilot_workflow`, `reverse-agent-workspace-dual-write-smoke`, `build_rebuild_delivery`, `review_flow_timeline`, `review_hook_artifacts`, `review_debugger_artifacts`, `review_rebuild_artifacts`, `evaluate_delivery_review_gate`, and `execute_local_delivery`; they consume or classify artifact keys, legacy paths, future paths, virtual URIs, artifact-root-relative fallback paths, legacy / future / external `source_path` usage, limited dual-write pilot readiness, low-risk pilot candidates, foldered-canonical blockers, migration preflight digest parity, and explicit filesystem boundaries without moving artifacts or bypassing delivery gates.
...
- Additional real third-party BrowserProvider plugin implementations beyond the Browserless CDP and Browserbase CDP baselines, functional fixture, hosted-CDP template, and hosted-CDP reference packages, such as vendor anti-detect browsers or other hosted browser services.
...
- Full source-map consumer semantics, credentialed browser Source Map fetching, and automatic debugger / rebuild / logpoint follow-through beyond the local remap, review-only source-map lookup / source-content / readiness / consumer-action-plan / consumer-materialization descriptors, review-only bundler symbol scope descriptor, and review-gated credentialless Source Map URL fetch metadata baseline.
- Full JS heap diff / runtime-collected object graph diff beyond the current descriptor-safe scoped object-root mutation audit baseline and review-only object graph diff descriptor.
...
- Review-gated follow-through beyond the current pure-Python `reverse-agent-workspace-dual-write-smoke`, `review_workspace_dual_write_pilot_workflow`, expansion-result verifier, foldered-canonical migration pilot plan, and migration preflight evidence before any physical canonical-path migration; default pilots stay low-risk, result recording remains explicit, and migration remains blocked while partial consumers or source_path evidence require compatibility monitoring.
...
- Cross-process live CDP paused continuation beyond durable inspect-only snapshots, read-only live-continuation preflight, target attach readiness proof, plan-only execution descriptor, reviewed attach-detach probe, read-only live callFrame recovery proof, review-gated one-action execution, review-only next paused-event capture plan, review-gated one-shot capture execution, review-only continuation checkpoint, review-gated pre-action subscribe-and-action orchestration, and review-only multi-step continuation workflow / journal plan, review-gated one-iteration execution, read-only cross-process session lifecycle, review-only multi-step loop planning, review-gated multi-step loop one-iteration execution, read-only wrapper-continuation readiness, review-only wrapper-continuation execution plan, and review-gated one-iteration wrapper continuation execution baselines; automatic multi-step action loops, automatic wrapper continuation loops, and long-lived cross-process session management remain unsupported.
...
- Full source-map consumer semantics and bundler-specific symbol scoping beyond the current review-only source-map lookup / source-content availability / readiness / consumer-action-plan / consumer-materialization / symbol-scope descriptors; automatic debugger / rebuild / source-logpoint follow-through remains unsupported.
- Full JS heap diff / runtime-collected object graph diff beyond descriptor-safe scoped object-root auditing and caller-provided snapshot diff descriptors.
- Physical foldered-canonical migration apply beyond the current read-only pilot plan and preflight / rollback descriptor remains a separate explicit-review executor.
...
| Strategy detector provider registry / plugin template | Baseline implemented | `src/reverse_deepagent/strategies/registry.py`, `packages/reverse-deepagent-strategy-detector-template/`, `tests/test_strategy_detector_registry.py`, `tests/test_strategy_detector_plugin_template.py`; declares `reverse_deepagent.strategy_detectors`, lists built-in and external detector metadata without running detectors, rejects secret-like provider metadata, and only invokes detector callables during explicit detection |
