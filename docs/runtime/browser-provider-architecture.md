...
- Flow timeline continuation baseline can explicitly normalize caller-provided network / hook / debugger / source-logpoint / mutation / replay fragments plus a previous flow timeline into `workspace/flow-timeline.json`; it is a schema and continuation baseline, not automatic all-event browser subscription or full cross-request stitching.
