...
- Paused-session cross-process support now covers read-only live-continuation preflight, read-only target attach readiness proof, plan-only cross-process execution descriptor, and reviewed Target.attachToTarget / Target.detachFromTarget probe evidence; it still does not recover live callFrames or execute resume / step / evaluate cross-process.
...
- Cross-process live CDP paused continuation beyond durable inspect-only snapshots, read-only live-continuation preflight, target attach readiness proof, plan-only execution descriptor, and reviewed attach-detach probe baselines; live callFrame recovery plus resume / step / evaluate execution remain unsupported.
