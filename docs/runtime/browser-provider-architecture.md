...
| Delivery resume workflow scheduler | Baseline implemented | `src/reverse_deepagent/delivery/resume_scheduler.py`, `src/reverse_deepagent/tools/delivery_tools.py`, `tests/test_delivery_resume.py`, `tests/test_delivery_tools.py`; `execute_delivery_resume_workflow` supports `plan_workflow` / `execute_workflow` over explicit `preflight_backend_manifest_recovery`, `apply_backend_manifest_recovery`, `commit_cross_run_transaction`, and `renew_delivery_transaction_lock_provider` steps, requires review approval ledger matches for apply-mode pending steps, writes `delivery-resume-workflow.json` plus append-only `delivery-resume-workflow-journal.json`, and skips completed journaled step actions; the renewal step invokes the configured `DeliveryTransactionLockProvider.renew_lock` and journals provider / fencing-token / lease-expiry evidence; dry-run is read-only, and the baseline is not a timer daemon, distributed workflow scheduler, external delivery publisher, distributed lock acquirer/releaser, automatic renewal loop, or broader physical rollback engine |
