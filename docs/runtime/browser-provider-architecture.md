...
| Runtime backend entry-point discovery | Baseline implemented | `RuntimeBackendRegistry.load_entry_points()` loads `reverse_deepagent.runtime_backends` registrations without invoking backend factories; external `legacy-mcp` entry points take precedence over the built-in compatibility fallback |
| MCP legacy module isolation | Baseline implemented | `packages/reverse-deepagent-legacy-mcp/` owns the optional legacy MCP registration / factory implementation; `reverse_deepagent.runtime.legacy_mcp` is a compatibility shim with install guidance and fallback; `build_default_runtime_registry(include_legacy_mcp=False)` builds a clean registry without MCP backend |
