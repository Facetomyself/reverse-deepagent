...
- Review-gated follow-through on `assess_workspace_migration_readiness` and `plan_workspace_dual_write_pilot` output before any broader dual-write rollout or foldered-canonical migration pilot; default pilots stay low-risk and plan-only, and migration remains blocked while partial consumers or source_path evidence require compatibility monitoring.
