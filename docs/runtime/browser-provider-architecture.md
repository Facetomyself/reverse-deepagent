...
- Closure wrapper replacement hardening beyond the current same-process reviewed `log-only-call-through` execution MVP, including reviewed restore execution, wrapper event harvesting, richer assignment safety proofs, and cross-process live CDP continuation integration.
...
- Closure-scope discovery proves candidate lexical bindings from a paused callframe; `closure-wrapper-replacement-plan` prepares a review-only replacement plan; `closure-wrapper-replacement-execution` can install one same-process reviewed `log-only-call-through` wrapper from a retained paused session, while arbitrary automatic closure-local wrapper replacement remains unsupported.
