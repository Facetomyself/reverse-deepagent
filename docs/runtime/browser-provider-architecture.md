...
- Full source-map consumer semantics, credentialed browser Source Map fetching, and automatic debugger / rebuild / logpoint follow-through beyond the local remap, review-only source-map lookup / source-content / readiness / consumer-action-plan descriptors, review-only bundler symbol scope descriptor, and review-gated credentialless Source Map URL fetch metadata baseline.
...
- Full source-map consumer semantics and bundler-specific symbol scoping beyond the current review-only source-map lookup / source-content availability / readiness / consumer-action-plan / symbol-scope descriptors; automatic debugger / rebuild / source-logpoint follow-through remains unsupported.
