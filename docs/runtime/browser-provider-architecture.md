...
- Full source-map consumer semantics, credentialed browser Source Map fetching, and automatic logpoint remap follow-through beyond the local remap, review-only source-map lookup descriptor, review-only bundler symbol scope descriptor, and review-gated credentialless Source Map URL fetch metadata baseline.
...
- Source-map handling covers generated offsets, exact / bias remap, `sourceRoot`, indexed sections, `names` metadata, URL equivalence, nested indexed-section metadata, review-only generated/original lookup descriptors, review-only `sourcesContent` availability descriptors that export digest / size / line-count metadata only, and review-gated credentialless Source Map fetch metadata.
...
- Full source-map consumer semantics and bundler-specific symbol scoping beyond the current review-only source-map lookup / source-content availability / symbol-scope descriptors.
