...
- webpack-like module discovery baseline can scan project-owned script inventory for `module.exports` export candidates, read-only introspect webpack-like `require.c` / `require.m` runtime module cache and registry data, emit `workspace/module-registry.json` plus `workspace/module-candidates.json`, and feed explicit `hook-module` requests without running in the default recon path.
...
- retained paused sessions can be stored in an in-process registry keyed by `pause_session_id`, then continued by a follow-up `paused-session` request for live inspect / evaluate / step / resume actions.
- durable paused-session snapshot baseline can be enabled with `persist_paused_session` / `paused_session_store_dir`; it persists debugger session, timeline, callframes, breakpoints, and trigger metadata for later cross-process inspect / audit, but it is explicitly inspect-only and rejects resume / step / evaluate with `live_paused_session_required`.
...
Cross-process live CDP paused execution continuation, arbitrary runtime module cache / module federation introspection beyond webpack-like `require.c` / `require.m`, arbitrary closure-internal function hooks, source-map name resolution / complex URL semantics / complex indexed section semantics, JS heap fine-grained mutation audit, object graph diff, and richer cross-request timeline continuation remain capability-gated future work; they should not be implemented by leaking raw CDP details into the coordinator.
