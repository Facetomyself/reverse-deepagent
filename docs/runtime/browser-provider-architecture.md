...
- Closure wrapper replacement hardening beyond the current review-only assignment safety proof / runtime mutability preflight and same-process reviewed `log-only-call-through` install / restore execution baselines, including review-approved runtime mutability probe/result evidence, arbitrary closure-local wrapper support, and cross-process live CDP continuation integration.
...
- `workspace/paused-session-live-continuation-preflight.json` with read-only same-process / durable / provided-artifact live continuation blockers, support booleans, next action, and fixed side-effect policy for explicit preflight requests.
...
- Debugger baseline writes paused, callframe, session, action, timeline, callframe-evaluation, and read-only paused-session live-continuation preflight artifacts; callframe evaluation defaults to read-only `throwOnSideEffect` and only allows side-effect expressions when explicitly enabled.
...
- Module federation covers review-only `get/init` planning, reviewed `init/get` probe, reviewed remote factory invoke, review-only export hook planning, reviewed export hook install, review-only federation traversal graph / workflow planning, review-gated one-step workflow execution, review-only recursive follow-up planning, review-gated recursive follow-up checkpointing, review-gated recursive next-step execution, and review-only recursive continuation journal / multi-step checkpoint planning, and review-gated recursive continuation checkpoint execution.
- Closure-scope discovery proves candidate lexical bindings from a paused callframe; `closure-wrapper-replacement-plan` prepares a review-only replacement plan; `closure-wrapper-assignment-safety` proves the static reviewed assignment gate for one selected candidate; `closure-wrapper-runtime-mutability-preflight` prepares a review-only future runtime mutability probe plan; `closure-wrapper-replacement-execution` can install one same-process reviewed `log-only-call-through` wrapper from a retained paused session only when that proof is supplied, and `closure-wrapper-restore-execution` can restore that wrapper through the reviewed same-process restore plan; `closure-wrapper-events` can read wrapper return / throw events from the runtime store; arbitrary automatic closure-local wrapper replacement remains unsupported.
...
- Cross-process live CDP paused continuation beyond durable inspect-only snapshots and the read-only live-continuation preflight baseline.
- Wrapper replacement for arbitrary closure-internal functions beyond the current same-process reviewed `log-only-call-through` MVP.
- Deeper recursive custom-loader traversal execution beyond one reviewed recursive next-loop checkpoint, bounded reviewed loop execution, recursive follow-up checkpoint planning, and review-gated follow-up checkpointing.
- Deeper recursive async chunk traversal beyond one reviewed recursive next-loop checkpoint and bounded reviewed loop execution.
- Deeper recursive federation traversal execution and remote-module analysis beyond review-only federation traversal graph / workflow planning, review-gated one-step workflow execution, review-only recursive follow-up planning, review-gated recursive follow-up checkpointing, review-gated recursive next-step execution, review-only recursive continuation journal / multi-step checkpoint planning, review-gated recursive continuation checkpoint execution, and reviewed remote export hook install.
