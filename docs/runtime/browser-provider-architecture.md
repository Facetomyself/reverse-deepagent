...
- `workspace/delivery-receipt.json` and `workspace/delivery-transaction-journal.json` as indexed-only routes for the local delivery executor contract baseline; the default executor mode is dry-run, and explicit apply only copies reviewed artifacts into a local delivery folder without external publication or manifest revision commit.
...
| Local delivery executor | Baseline implemented | `src/reverse_deepagent/delivery/executors.py`, `src/reverse_deepagent/tools/delivery_tools.py`, `tests/test_delivery_executors.py`, `tests/test_delivery_tools.py`; dry-run is default, the delivery subagent exposes `execute_local_delivery`, explicit apply only copies reviewed files locally and writes receipt / journal, without external delivery or manifest revision commit |
