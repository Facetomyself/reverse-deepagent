...
- Execution-style arbitrary custom loader traversal and deeper async chunk traversal beyond the review-only custom-loader traversal plan plus review-gated webpack `require.e(chunkId)` baselines, and remote factory invocation / remote module body analysis beyond the current read-only runtime-path, chunk-graph, review-only get/init plan, and reviewed init/get probe baselines.
