...
- Provider-specific production readiness rule catalog scaffold for metadata-only lifecycle drift checks; current rules cover the Playwright Chromium launch/connect baseline, Remote CDP attach-only contract, CloakBrowser production lifecycle metadata, and hosted-CDP reference allocation / attach / release metadata contract without invoking provider factories, probing CDP endpoints, launching browsers, or calling MCP.
