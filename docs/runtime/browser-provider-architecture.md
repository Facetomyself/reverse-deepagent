...
Single-provider doctor checks keep the existing `browser_provider` shape and add `browser_provider.smoke_matrix`; explicit `--launch-browser-smoke` is still the only path that can open a real provider session. `reverse-agent-browser-provider-smoke` now writes the same single-provider evidence shape to `workspace/browser-provider-smoke.json`: default mode is registry metadata-only and does not invoke provider factories, while `--include-availability` / `--launch-browser-smoke` are explicit side-effect knobs. A reviewed smoke JSON can be attached to a Web pipeline through `reverse-agent-demo --browser-provider-smoke-json <path>`; this path only reads an existing UTF-8 JSON object and includes it in `workspace/browser-provider-smoke.json`, `exports/artifact-index.json`, and `workspace/backend-artifact-manifest.json`. It does not generate smoke, call provider factories, check availability, launch browsers, probe CDP endpoints, or call MCP. The workspace contract indexes that path as `/workspace/browser/browser-provider-smoke.json` without migrating existing outputs.
...
- Provider-specific production readiness rule catalog scaffold for metadata-only lifecycle drift checks; current rules cover the Playwright Chromium launch/connect baseline, Remote CDP attach-only contract, CloakBrowser production lifecycle metadata, hosted-CDP reference allocation / attach / release metadata contract, and Browserless CDP hosted-endpoint contract without invoking provider factories, probing CDP endpoints, launching browsers, or calling MCP.
...
- Deeper execution-style custom loader traversal beyond the bounded continuation planning, review-only multi-step workflow planning, review-gated traversal workflow execution, review-only bounded traversal loop planning, review-gated bounded traversal loop execution, review-only continuation workflow planning, review-gated continuation journal, review-approved one-step continuation execution, and review-approved single-step custom-loader execution baselines, plus deeper recursive async chunk traversal beyond the current review-only async chunk traversal graph / workflow plan, review-gated one-step traversal workflow execution, review-only bounded loop planning, review-gated bounded loop execution, review-gated webpack `require.e(chunkId)`, and review-only async chunk module diff baselines; recursive federation traversal and deeper remote-module analysis beyond the current read-only runtime-path, chunk-graph, review-only get/init plan, reviewed init/get probe, reviewed factory invoke, review-only export-hook-plan, and review-approved remote export hook install baselines.
...
This is enough for fixture-level runtime/replay validation and a basic breakpoint paused/callframe/evaluateOnCallFrame/step/session smoke path with the existing artifact contract. The current callframe evaluation baseline defaults to `read_only`, passes `throwOnSideEffect` to CDP, records side-effect risk metadata, emits `workspace/mutation-audit.json` for the observed evaluation risk summary, and blocks obvious high-risk mutation expressions unless `allow_callframe_side_effects` is explicitly enabled. The target-function wrapper baseline is limited to globally reachable function paths; the module export hook baseline covers explicit webpack-like module exports, and module discovery can now derive best-effort `module.exports` candidates from script inventory plus read-only webpack-like `require.c` / `require.m` runtime cache and registry introspection for follow-up `hook-module` requests. It can also emit `hook_kind=function-path` candidates from explicit custom object runtime paths and module federation exposed-module snapshots for follow-up `hook-function`, and records read-only async chunk graph / loader metadata from static import edges plus runtime loader shape without calling loaders. A separate review-only `custom-loader-traversal-plan` baseline can classify custom loader / dynamic import / federation / webpack candidates into a review plan without invoking loaders, requesting chunks, executing dynamic imports, invoking module factories, or running federation `get/init`; when prior execution evidence and next candidates are provided, it also emits bounded continuation metadata and blocks already-executed candidates from preflight reuse. A separate review-only `custom-loader-traversal-graph` baseline consumes that plan plus journal / previous execution evidence, writes `workspace/custom-loader-traversal-graph.json`, and builds a bounded review queue without running preflight, invoking loaders, installing hooks, appending journals, or recursing automatically. A separate review-only `custom-loader-traversal-workflow-plan` baseline consumes that graph, writes `workspace/custom-loader-traversal-workflow-plan.json`, and composes candidate selection, continuation workflow, preflight, one reviewed loader step, module diff, optional hook, journal append, and graph rebuild into a manual-checkpoint plan without executing any stage or recursing automatically. A separate `custom-loader-continuation-workflow` protection request can consume that continuation-aware plan, select one unexecuted bounded candidate, emit `workspace/custom-loader-continuation-workflow.json`, and prepare review-gated preflight input plus an append-only journal plan without running preflight, invoking loaders, installing hooks, or recursing automatically. A separate `custom-loader-continuation-journal` protection request can consume that workflow and optional preflight / execution / module-diff / hook evidence, then only with `write_journal=true` plus `review_approved=true` append a `workspace/custom-loader-continuation-journal.json` record; the journal append does not run preflight, invoke loaders, install hooks, or recurse. A separate `custom-loader-continuation-execution` protection request can consume the same workflow and orchestrate at most one reviewed continuation step across preflight, single loader execution, module diff refresh, optional module hook install, and optional journal append. Each stage is controlled by an explicit flag such as `run_preflight`, `execute_custom_loader`, `run_module_diff`, `install_module_hook`, or `append_journal`; runtime side effects still require `review_approved=true`, the route writes `workspace/custom-loader-continuation-execution.json`, and it never recurses automatically, calls MCP, or touches mobile runtimes. A reviewed `custom-loader-execution` baseline can consume a successful preflight and execute exactly one arbitrary custom-loader candidate after explicit approval, writing `workspace/custom-loader-execution-result.json` with result preview plus registry/cache before/after diff evidence while still avoiding dynamic import, webpack `require.e`, federation `get/init`, recursive traversal, automatic hook installation, MCP, and mobile runtimes. A separate review-only `custom-loader-module-diff` baseline can consume that reviewed execution result plus refreshed module discovery, write `workspace/custom-loader-module-diff.json`, map added registry/cache keys to newly visible modules, and emit review-only `hook-module` candidates without re-executing the loader, invoking module factories, installing hooks, calling MCP, or touching mobile runtimes. A reviewed `custom-loader-module-hook` follow-through can then consume one approved diff candidate, delegate to `ModuleHookManager`, and write the existing `workspace/module-hooks.json` / `workspace/module-hook-timeline.json` surfaces without re-executing the loader or installing hooks automatically from the diff plan. A separate review-only `module-federation-get-init-plan` baseline can classify federation containers, exposed modules, existing function-path candidates, shared-scope mutation risk, and remote factory execution risk without executing `container.init`, `container.get`, or a returned factory by default. Its reviewed probe path can explicitly execute only `container.init(shareScope)` and `container.get(exposedName)` after `execute_module_federation_get_init=true` and `review_approved=true`, recording shared-scope / container diff evidence in `workspace/module-federation-get-init-result.json` while still avoiding returned remote factory invocation. Its reviewed factory path can further invoke the returned factory after `execute_module_federation_factory=true` and `review_approved=true`, recording export summary evidence in `workspace/module-federation-factory-invoke-result.json`. A separate review-only `module-federation-export-hook-plan` baseline can consume that factory invoke result and emit `workspace/module-federation-export-hook-plan.json` with hookable remote export recommendations, unsupported export-type blockers, and `review_module_federation_export_hook_plan` as the next action while still avoiding automatic remote export hook installation or recursive remote traversal. A separate review-gated `async-chunk-traversal-workflow-execution` baseline can consume `workspace/async-chunk-traversal-workflow-plan.json`, select one planned step, and only with explicit stage flags delegate to async chunk load, module diff, and optional module hook managers; it writes `workspace/async-chunk-traversal-workflow-execution.json`, executes at most one reviewed chunk-load step, never advances the queue automatically, never rebuilds the graph automatically, and stops before recursive traversal. A separate review-gated `async-chunk-load` baseline can plan a candidate and explicitly execute only webpack-style `require.e(chunkId)` after `execute_chunk_load=true` and `review_approved=true`, recording registry/cache diff evidence. A separate review-only `async-chunk-module-diff` baseline can consume that reviewed load result with refreshed module discovery and emit added module keys plus hook-module candidates for review while still avoiding arbitrary custom loaders, dynamic imports, module factory invocation, recursive remote traversal, and automatic hook installation. Closure-scope discovery can prove explicit candidate names resolve to functions in a paused callframe and emit `hook_kind=closure-scope` evidence candidates, but it does not install wrappers around lexical bindings; neither path hooks arbitrary closure-internal functions, executes arbitrary custom loaders, performs deep async chunk traversal, or executes async federation `get/init` flows. The source-level logpoint baseline can now remap generated bundle offsets and Source Map v3 original source locations into CDP generated line / column positions with exact matching, GLB bias fallback, `sourceRoot` matching, indexed section offsets, source-map `names` metadata, URL-like source equivalence, and nested indexed-section stack metadata, and writes remap metadata into source-logpoint artifacts. A separate `source-map-fetch` protection path now plans external `sourceMappingURL` fetches by default and, only with `fetch_source_map=true` plus `review_approved=true`, performs credentialless same-origin / allowlisted Python fetches for root Source Maps and explicitly requested indexed-section `url` entries, emitting digest / size / sources / section metadata to `workspace/source-map-fetch-plan.json` and `workspace/source-map-fetch-result.json` without exporting raw payloads, sending browser cookies, calling MCP, or touching mobile runtimes. It still does not implement bundler-specific symbol scoping or provide webpack module-internal hook discovery. The page-level mutation audit baseline is a coarse before/after summary around an explicit trigger expression and remains separate from callframe evaluation side-effect risk audit. The scoped object-root mutation audit baseline snapshots a strict dotted JS object root around an explicit trigger and diffs added / removed / changed / type-changed / descriptor-changed paths without prototype traversal or accessor getter invocation; it is intentionally narrower than a full JS heap or arbitrary object graph diff. The MutationObserver timeline baseline captures finite DOM mutation records around an explicit trigger. The paused-session registry is in-process and tied to the active browser/CDP session for live continuation; follow-up actions now expose a machine-readable `continuation_preflight` that reports whether the source is `registry`, `durable_snapshot`, or missing, whether live continuation is available, and which action was blocked if a durable snapshot is asked to resume / step / evaluate. Explicit `persist_paused_session` / `paused_session_store_dir` can additionally write a durable paused-session snapshot for later cross-process inspect / audit, but that snapshot is inspect-only and cannot resume, step, or evaluate the original CDP paused execution. It is still intentionally narrower than the legacy MCP path: cross-process live CDP paused execution continuation, deeper execution-style custom loader traversal beyond the review-only traversal graph / queue plus review-only multi-step workflow plan, reviewed one-step continuation execution and single-step custom-loader execution / module-diff refresh / module hook follow-through baseline / deeper async chunk traversal beyond the reviewed one-step traversal workflow / review-only bounded loop planning / reviewed load plus module-diff refresh / recursive federation traversal beyond the reviewed module federation factory invoke plus review-only export-hook-plan plus reviewed export-hook-install baselines, automatic wrapper hooks for arbitrary closure-internal functions, full JS heap / object graph diff beyond scoped object-root auditing, full source-map consumer semantics / bundler-specific symbol scoping beyond the current credentialless URL fetch metadata baseline, and automatic full cross-request timeline materialization writer / full conflict resolver state machine / cross-run physical rollback transaction state machine / GitHub Release asset overwrite / advanced adaptive provider retry policy beyond the current JSON asset upload, explicit existing-release reuse, asset duplicate preflight, and explicit retry baselines / third-party external delivery provider implementations beyond the built-in local archive, webhook, object-storage and GitHub Release providers with asset duplicate preflight / full cross-run manifest recovery executor beyond the explicit rollback-checkpoint apply baseline / cross-run transaction idempotency hardening beyond the external delivery ledger baseline / review-approved automatic materializer remain separate capability-gated follow-up work. The current `flow-timeline` baseline records native-web recon fragments, adds conservative per-entry `correlation` hints, derives non-authoritative `correlation_groups` with readiness verification, promotes reviewable groups into manual-only `stitch_candidates`, scores those candidates through `auto_stitch_dry_runs`, emits review-only `auto_stitch_conflict_resolutions`, evaluates `auto_stitch_policy_decisions`, produces plan-only `auto_stitch_materialization_plans` for policy-eligible decisions without writing artifacts, aggregates review-approved materialization into transaction-log-only `auto_stitch_materialization_transactions`, produces dry-run `auto_stitch_rollback_execution_plans`, records explicitly approved logical `auto_stitch_rollback_execution_results` without mutating `stitched-flow.json`, emits blocking `auto_stitch_rollback_review_gate_recomputations` without replacing `review-gate.json`, emits dry-run `auto_stitch_physical_rollback_dry_run_diffs`, applies explicitly approved `auto_stitch_physical_rollback_review_decisions` as `auto_stitch_physical_rollback_results` by removing matching entries from the current `stitched_flows` artifact model, emits blocking `auto_stitch_post_physical_rollback_review_gate_reruns`, records explicitly approved `auto_stitch_standard_review_gate_replacement_results`, emits `auto_stitch_post_standard_review_gate_replacement_delivery_guard_reruns` with `delivery_allowed=true` but `automatic_delivery=false`, emits artifact-model `auto_stitch_post_standard_review_gate_replacement_final_delivery_packages` with `package_ready=true` / `final_delivery_packaged=true` while keeping `external_delivery_performed=false` / `cross_run_transaction_committed=false`, records explicit-review-only `auto_stitch_transaction_commit_results` with `artifact_model_transaction_commit_recorded=true` while keeping `cross_run_transaction_committed=false` / `filesystem_artifact_mutated=false` / `external_delivery_performed=false`, promotes ready candidates into pending-review `stitch_proposals`, surfaces those pending proposals through evidence promotion / review gate blocking, materializes explicitly approved proposals into `stitched-flow.json`, and continues explicitly supplied timeline fragments, but it does not apply a full cross-request flow automatically.
