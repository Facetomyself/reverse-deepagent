...
- Execution-style arbitrary custom loader traversal, deeper async chunk traversal beyond the review-gated webpack `require.e(chunkId)` baseline, and module federation `get/init` analysis beyond the current read-only runtime-path and chunk-graph baselines.
