...
- retained paused sessions can be stored in an in-process registry keyed by `pause_session_id`, then continued by a follow-up `paused-session` request for inspect / evaluate / step / resume actions.

Cross-process paused session persistence, source map / bundle offset remapping, webpack / module-internal function hooks, and finer-grained side-effectful mutation audit remain capability-gated future work; they should not be implemented by leaking raw CDP details into the coordinator.
...
| NativeWebRuntime | Native collectors, hook baseline, target-function wrapper baseline, source-level logpoint baseline, retained paused-session registry baseline, runtime-eval candidate validation, and paused/callframe breakpoint snapshot implemented | `src/reverse_deepagent/adapters/native_web.py`, `src/reverse_deepagent/browser/hooks/` |
