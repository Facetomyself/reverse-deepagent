...
- Opt-in reviewed wrapper replacement execution for closure-internal functions beyond the current review-only `closure-wrapper-replacement-plan` baseline.
...
- `workspace/closure-functions.json` and `workspace/closure-function-candidates.json` when explicit closure-scope function discovery is requested; `workspace/closure-wrapper-replacement-plan.json` when review-only closure wrapper replacement planning is requested.
...
- Closure-scope discovery proves candidate lexical bindings from a paused callframe; `closure-wrapper-replacement-plan` can prepare a review-only replacement plan, but no closure-local function wrapper is installed and no lexical binding is replaced.
