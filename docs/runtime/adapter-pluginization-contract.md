...
- Runtime backend entry-point loading is implemented as the split seam for optional packages; `legacy-mcp` registration is now isolated in `reverse_deepagent.runtime.legacy_mcp`, can be excluded with `build_default_runtime_registry(include_legacy_mcp=False)`, and has a repo-local optional package skeleton at `packages/reverse-deepagent-legacy-mcp/`. The default registry lets an external `legacy-mcp` entry point take precedence over the built-in compatibility registration, but the legacy MCP code is still shipped in the core distribution until the physical implementation move is completed.
