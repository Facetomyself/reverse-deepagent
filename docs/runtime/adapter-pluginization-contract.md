...
- Runtime backend entry-point loading is implemented as the split seam for optional packages; `packages/reverse-deepagent-legacy-mcp/` now owns the optional legacy MCP registration / factory implementation, while `reverse_deepagent.runtime.legacy_mcp` remains a compatibility shim with install guidance and a built-in fallback. `legacy-mcp` can be excluded with `build_default_runtime_registry(include_legacy_mcp=False)`, and the default registry lets an external `legacy-mcp` entry point take precedence over the fallback. The remaining split work is to remove the built-in fallback from the core distribution after the optional package is the supported installation path.
