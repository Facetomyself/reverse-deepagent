...
- Runtime backend entry-point loading is implemented as the split seam for optional packages; `legacy-mcp` registration is now isolated in `reverse_deepagent.runtime.legacy_mcp` and can be excluded with `build_default_runtime_registry(include_legacy_mcp=False)`, but the legacy MCP code is still shipped in the core distribution until the physical package split is completed.
