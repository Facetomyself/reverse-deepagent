...
> 图示占位：agent.invoke() 背后发生了什么？规划任务 → 搜索信息 → 管理上下文 → 委派子任务（如需要）→ 综合报告，你只写了 1 行调用，Agent 自动完成 10+ 次工具调用（原外部图片资源 `../public/imgs/05-flowchart-agent-workflow.png` 未纳入本仓库，避免保留坏链接。）
...
> 图示占位：两种大模型标准接口对比：OpenAI 兼容接口（/chat/completions，行业最广泛）和 Anthropic 兼容接口（/messages），硅基流动同时兼容两种，支持 GLM-5、Kimi-K2.5、Qwen3.5、DeepSeek-V3.2 等模型（原外部图片资源 `../public/imgs/06-comparison-api-interfaces.png` 未纳入本仓库，避免保留坏链接。）
