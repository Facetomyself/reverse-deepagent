...
> 图示占位：虚拟文件系统六大工具：ls、read_file、write_file、edit_file、glob、grep（原外部图片资源 `../public/imgs/07-infographic-six-tools.png` 未纳入本仓库，避免保留坏链接。）
...
> 图示占位：上下文自动管理两道防线：大结果自动卸载（>20K tokens）和对话历史自动总结（>85% 窗口），Agent 始终拥有精炼的工作记忆和可回溯的完整记录（原外部图片资源 `../public/imgs/08-flowchart-context-management.png` 未纳入本仓库，避免保留坏链接。）
...
> 图示占位：五种存储后端对比：StateBackend（临时）→ FilesystemBackend（本地磁盘）→ StoreBackend（跨会话持久化）→ CompositeBackend（混合路由）→ 沙箱后端（安全执行），从临时到持久化的渐进选择（原外部图片资源 `../public/imgs/09-comparison-backends.png` 未纳入本仓库，避免保留坏链接。）
