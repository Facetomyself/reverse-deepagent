...
2. 开发基线分支只能是 `main`。AI 不得以 `master` 作为日常开发起点，也不得把 PR 目标分支设为 `master`。
3. 发起 PR 前，必须先同步最新远端提交，确保当前分支已经对齐最新 `origin/main`。
4. 如果发现当前 PR 的目标分支是 `master`，AI 必须立刻改为 `main`，然后重新检查 PR 信息。
...
13. 必须严格区分“同步 PR 分支”和“合并 PR 到 `main`”：
    - `git merge origin/main`、`git rebase origin/main`、把最新 `main` 推回 PR 源分支，只是让 PR 分支吸收最新基线，不等于合并 PR。
    - 只有通过 GitHub PR 合并动作，例如 `gh pr merge <PR_NUMBER> --merge`，并确认 `origin/main` 已前进到合并提交，才算“合并到 `main`”。
14. 如果开发者明确说“合并进 main”“合到 main”“通过就合”“没问题就合并”“处理完直接合并”这类表达，AI 必须执行阶段 8 的真实 PR 合并流程；不能只更新 PR 分支后声称已经合并。
...
1. 新任务必须从最新 `main` 拉出功能分支。
2. 不允许直接在 `main` 上开发。
3. 不允许从 `master` 拉开发分支。

#### 场景 B：这是一个已有分支上的继续开发

如果开发已经在某个功能分支上进行，则不能为了同步 `main` 直接丢本地改动。
...
1. 必须真实判断当前分支是否已经落后于 `origin/main`。
2. 如果当前分支落后，先继续开发可以，但在发起 PR 前必须补齐最新 `main`。
...
### 阶段 4：发起 PR 前必须再次同步最新 `main`

这是硬规则。

无论这个分支什么时候开始开发，只要准备发起 PR，就必须再次拉取远端最新提交，并确认当前分支已经吸收了最新 `origin/main`。
...
3. 提交信息必须描述真实功能结果，不要写：
   ...
   - `merge branch`
   - `修改一下`
4. 推送前要确认当前分支不是 `main`，也不是 `master`。
...
PR 默认只能指向 `main`，除非维护者明确指定其他 base。
...
1. 这一步才是“把 PR 合并到 `main`”。把 `origin/main` 合并到功能分支、把功能分支推回 PR、让 PR 变成可合并，都不算已经合并到 `main`。
2. 如果不打算删除 PR 源分支，可以去掉 `--delete-branch`，但仍然必须执行 `gh pr merge`。
3. `gh pr merge` 成功后，必须立刻验证 PR 和本地 `main`：
...
1. `state` 必须是 `MERGED`。
2. `baseRefName` 必须是 `main`。
3. `mergeCommit.oid` 必须存在。
4. `git rev-parse HEAD origin/main` 输出的两个提交必须一致。
5. 本地 `main` 的最新提交必须是该 PR 的合并提交，或包含该 PR 合并结果的后续 `main` 提交。
6. 如果本地有无关脏改动导致不能切回或快进 `main`，必须明确告诉开发者“远端 PR 已合并，但本地 `main` 尚未更新”，不能说本地也已经完成。

限制：

1. AI 只能把自己的 PR 合并到 `main`。
2. AI 不得把任何开发分支直接合并到 `master`。
3. 如果发现 PR 目标分支是 `master`，先改成 `main`，再重新核对是否允许继续。
...
- 如果未跑测试，已明确提醒开发者测试
- 如果开发者授权合并，已确认 PR 状态为 `MERGED`，且本地 `main` 与 `origin/main` 指向同一提交
...
9. 是否已经合并；如果已合并，要明确说明是合并到 `main`
10. 如果已合并，必须说明 PR 状态是否为 `MERGED`、合并提交是什么、本地 `main` 和 `origin/main` 是否已经一致

## 一句话执行要求

AI 在本仓库里做开发与提 PR 时，必须按“先确认环境与当前工作区，再对齐最新 `main`，再开发与整理改动，再次同步最新 `main`，最后只向 `main` 发起或更新 PR；只有在开发者明确授权时，才允许把自己的 PR 合并到 `main`”的顺序执行，不能跳步，不能猜，不能偷懒。
