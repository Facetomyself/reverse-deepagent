...
- full source-map consumer semantics，超过当前 local remap、review-only source-map lookup descriptor、review-only source-map source-content availability descriptor、review-only source-map readiness join descriptor、review-only source-map consumer action plan descriptor、review-only bundler symbol scope descriptor 与 credentialless URL fetch metadata baseline；当前 descriptors 只给出保守 lookup / source-content availability metadata / readiness / reviewed action plan / 候选和 hook readiness，不宣称完整符号绑定 / scope 解析，也不导出 raw source content。
