...
`native-web` / `remote-cdp` 现在会在 BrowserProvider 支持 runtime eval 时执行最小候选函数验证，并沿用既有 artifact 名称输出 `workspace/function-candidates.json`、`workspace/function-validations.json` 和 `workspace/function-validation-summary.json`。当传入显式 breakpoint 触发表达式时，它们还会输出 `workspace/debugger-paused.json`、`workspace/callframes.json`、`workspace/debugger-session.json` 和 `workspace/debugger-timeline.json`；显式传入 `callframe_evaluations` / `evaluate_on_callframe` 时补充 `workspace/callframe-evaluations.json` 和 `workspace/mutation-audit.json`；显式传入 `closure-function-discovery` / `closure-scope` protection，并提供 `closure_function_names` / `function_name`、断点位置和 trigger 时，会在 paused callframe 里用只读 `typeof <name>` 证明候选闭包绑定是否为函数，输出 `virtual://workspace/closure-functions.json` 和 `virtual://workspace/closure-function-candidates.json`；显式传入 `debugger_actions` / `pause_actions` 时还会输出 `workspace/debugger-actions.json`，用于最小 paused/callframe/evaluateOnCallFrame/step/session smoke。显式传入 `hook-function` / `function-hook` protection 和 `function_name` / `function_paths` 时，`native-web` 还可以对全局可达函数路径安装 wrapper，例如 `window.buildSign`，并返回 `virtual://workspace/function-hooks.json` 与 `virtual://workspace/function-hook-timeline.json`。显式传入 `hook-module` / `module-hook` protection、`module_id` 和 `export_name` 时，还可以对 `window.__webpack_require__(731).sign` 这类 webpack-like module export 安装 wrapper，并返回 `virtual://workspace/module-hooks.json` 与 `virtual://workspace/module-hook-timeline.json`。显式传入 `module-discovery` / `discover-modules` protection 或 `module_query` 时，`native-web` 会从项目自有 script inventory 中做 best-effort webpack-like module export discovery，并只读 introspect `window.__webpack_require__.c` / `.m` 这类 runtime module cache / registry；显式传入 `module_runtime_paths` / `moduleRuntimePaths` 时，还会对 custom object runtime 和带 `__reverseAgentExposes` 快照的 module federation container 做只读 baseline introspection；同时 module discovery 会从静态 `import()` / `importScripts()` / webpack `require.e(...)` 边和 runtime loader metadata（例如 `require.e` / `require.u` / `require.f` 的形态）构建只读 async chunk graph，不调用 loader、不请求 chunk。显式传入 `custom-loader-traversal-plan` / `custom-loader-traversal` protection 或 `custom_loader_candidates` / `chunk_graph` 时，`native-web` 会输出 `virtual://workspace/custom-loader-traversal-plan.json`，把 arbitrary custom loader、dynamic import、module federation 和 webpack loader 候选归类成 review-only plan，不执行 loader、不请求 chunk、不执行 dynamic import、不调用 module factory 或 federation `get/init`；当提供 prior execution evidence 和 next candidates 时，它还会标记 bounded continuation candidate、already-executed candidate 和 depth blocker。显式传入 `custom-loader-traversal-graph` / `custom-loader-continuation-queue` protection，并提供 traversal plan、continuation journal 或 prior execution evidence 时，`native-web` 会输出 `virtual://workspace/custom-loader-traversal-graph.json`，构建 review-only node / edge graph 和 bounded queue，用于选择下一步 continuation candidate；这一步不运行 preflight、不调用 loader、不写 journal、不递归。显式传入 `custom-loader-traversal-workflow-plan` / `custom-loader-deep-traversal-workflow` protection，并提供同一个 traversal graph 时，`native-web` 会输出 `virtual://workspace/custom-loader-traversal-workflow-plan.json`，把 review queue 组合成多步人工检查计划，串起 candidate 选择、continuation workflow、preflight、单步 reviewed execution、module diff、可选 hook、journal append 和 graph rebuild 的后续显式动作；这一步仍不运行 preflight、不调用 loader、不安装 hook、不写 journal、不递归。显式传入 `custom-loader-traversal-workflow-execution` / `execute-custom-loader-traversal-workflow` protection，并提供同一个 workflow plan 时，`native-web` 会输出 `virtual://workspace/custom-loader-traversal-workflow-execution.json`，默认只选择一个 planned step 并生成 reviewable execution plan；只有显式 `plan_continuation_workflow` 或 `run_preflight` / `execute_custom_loader` / `run_module_diff` / `install_module_hook` / `append_journal` 这类 stage flag 时才委托 continuation workflow / one-step continuation executor，且一次只处理一个 reviewed step，不自动重建 traversal graph、不递归到下一个 queue item。显式传入 `custom-loader-traversal-loop-plan` / `custom-loader-deep-traversal-loop` protection，并提供同一个 workflow plan 时，`native-web` 会输出 `virtual://workspace/custom-loader-traversal-loop-plan.json`，把多个 planned step 组织成 bounded loop checkpoint plan，记录每轮 selection、reviewed one-step execution、journal append、graph rebuild、workflow replan 和 stop-before-next-review 顺序；它仍是 plan-only，不执行 loop、不自动 advance queue、不重建 graph、不写 journal、不递归。显式传入 `custom-loader-traversal-loop-execution` / `execute-custom-loader-traversal-loop` protection，并提供 loop plan 与 workflow plan 时，`native-web` 会输出 `virtual://workspace/custom-loader-traversal-loop-execution.json`，默认只选择一个 loop iteration 并生成 reviewable execution plan；只有显式 `plan_continuation_workflow` 或 `run_preflight` / `execute_custom_loader` / `run_module_diff` / `install_module_hook` / `append_journal` 这类 stage flag 时才委托现有 traversal workflow executor，且一次只处理一个 reviewed loop iteration，不自动重建 traversal graph、不自动 replan workflow、不自动 advance queue、不递归、不调用 MCP、不触碰 Android / iOS / 小程序完整链路。显式传入 `custom-loader-continuation-workflow` / `custom-loader-continuation-plan` protection，并提供 continuation-aware traversal plan 时，`native-web` 会输出 `virtual://workspace/custom-loader-continuation-workflow.json`，只选择一个 unexecuted bounded continuation candidate，准备 review-gated preflight input 和 append-only journal plan；这一步不运行 preflight、不调用 loader、不写 journal、不递归。显式传入 `custom-loader-continuation-execution` / `execute-custom-loader-continuation-step` protection，并提供同一个 workflow 时，`native-web` 会输出 `virtual://workspace/custom-loader-continuation-execution.json`，在显式 flags 下只推进一个 bounded continuation step：`run_preflight` 只执行 side-effect-free preflight，`execute_custom_loader` 只有在 `review_approved=true` 且 preflight ready 后才调用单个 reviewed loader，`run_module_diff` / `install_module_hook` / `append_journal` 分别复用既有 module diff、module hook 和 continuation journal surface；它不递归、不默认运行、不调用 MCP、不触碰 Android / iOS / 小程序完整链路。显式传入 `custom-loader-execution-preflight` / `preflight-custom-loader-execution` protection、提供同一个 traversal plan 候选并设置 `review_approved=true` 后，`native-web` 会输出 `virtual://workspace/custom-loader-execution-preflight.json`，只验证候选类型、strict dotted loader path、expected loader path 和专用 gate 边界；这一步仍不调用 loader、不请求 chunk、不执行 dynamic import、不递归遍历。显式传入 `async-chunk-traversal-workflow-execution` / `execute-async-chunk-traversal-workflow` protection，并提供 async chunk traversal workflow plan 时，`native-web` 会输出 `virtual://workspace/async-chunk-traversal-workflow-execution.json`，默认只选择一个 planned step 并生成 reviewable execution plan；只有显式 `plan_async_chunk_load` / `execute_async_chunk_load` / `run_module_diff` / `install_module_hook` 这类 stage flag 时才委托 async chunk load、module diff 和 module hook manager，且一次只处理一个 reviewed chunk-load step，不自动 advance queue、不自动重建 traversal graph、不递归、不调用 MCP、不触碰 Android / iOS / 小程序完整链路。显式传入 `async-chunk-traversal-loop-plan` / `async-chunk-deep-traversal-loop` protection，并提供同一个 async chunk traversal workflow plan 时，`native-web` 会输出 `virtual://workspace/async-chunk-traversal-loop-plan.json`，把多个 planned step 组织成 bounded loop checkpoint plan，记录每轮 selection、reviewed one-step workflow execution、module diff refresh、可选 hook、graph rebuild、workflow replan 和 stop-before-next-review 顺序；它仍是 plan-only，不执行 loop、不请求 chunk、不自动 advance queue、不重建 graph、不递归。显式传入 `custom-loader-execution` / `execute-custom-loader` protection、提供 preflight payload 并再次设置 `review_approved=true` 后，`native-web` 只执行单个 reviewed arbitrary custom-loader candidate，输出 `virtual://workspace/custom-loader-execution-result.json`，记录 loader result preview、webpack-like registry/cache before/after snapshot 和 added/removed key diff；它不执行 dynamic import、不走 webpack `require.e`、不执行 module federation `get/init`、不递归遍历、不自动安装 hook、不调用 MCP。显式传入 `custom-loader-module-diff` / `custom-loader-hook-candidates` protection，并提供 reviewed `custom_loader_execution_result` 与刷新后的 `module_discovery` / `modules` 后，`native-web` 会输出 `virtual://workspace/custom-loader-module-diff.json`，把 custom-loader execution 产生的 added registry/cache keys 映射到新增 module，并生成 review-only `hook-module` 候选；这一步不重新执行 loader、不执行 module factory、不自动安装 hook、不调用 MCP。显式传入 `custom-loader-module-hook` / `hook-custom-loader-module` protection、提供同一个 `custom_loader_module_diff` 候选并设置 `review_approved=true` 后，`native-web` 才会委托现有 `ModuleHookManager` 安装对应 module export wrapper，继续用 `virtual://workspace/module-hooks.json` 与 `virtual://workspace/module-hook-timeline.json` 记录安装结果和调用事件；这个 follow-through 不重新执行 loader、不重新 discovery、不执行 module factory，也不会自动从 diff plan 安装 hook。显式传入 `module-federation-get-init-plan` / `federation-get-init-plan` protection，或传入 `module_federation_candidates` / `federation_modules` / `module_candidates` 这类 federation candidate 时，`native-web` 会输出 `virtual://workspace/module-federation-get-init-plan.json`，默认只规划 container path、exposed module、已有 function-path candidate、shared scope mutation 和 remote factory execution 风险，不执行 `container.init`、`container.get` 或返回的 remote factory。若在同一 protection 中显式传入 `execute_module_federation_get_init=true` 且 `review_approved=true`，则会进入审批门控探针，只对严格 dotted container path 调用 `container.init(shareScope)` 和 `container.get(exposedName)`，输出 `virtual://workspace/module-federation-get-init-result.json`，记录 shared scope / container key diff、factory type 与 `remoteFactoryInvoked=false`；它仍不会调用返回的 remote factory，也不会执行 remote module body。若进一步显式传入 `execute_module_federation_factory=true` / `invoke_module_federation_factory=true` 且 `review_approved=true`，`native-web` 会在同一严格候选上先完成 reviewed `init/get` probe，再调用 returned remote factory，输出 `virtual://workspace/module-federation-factory-invoke-result.json`，记录 module type、export names、export previews、shared scope / container diff，并明确标记 `remoteFactoryInvoked=true` / `remoteCodeExecuted=true`；hook review 会要求先 `review_module_federation_factory_exports_before_hooking`；显式传入 `module-federation-export-hook-plan` / `remote-export-hook-plan` protection，并提供该 factory invoke result 时，`native-web` 会输出 `virtual://workspace/module-federation-export-hook-plan.json`，按 export type 生成 review-only hook recommendation，其中 function export 会成为 `remote-export-wrapper` 候选，非函数 export 会保留为 manual inspection blocker；这一步仍不会自动安装 export hook，也不会递归遍历 remote。显式传入 `module-federation-export-hook-install` / `hook-module-federation-remote-export` protection、提供同一个 export hook plan 候选并设置 `review_approved=true` 后，`native-web` 会重新执行受控的 `container.init` / `container.get` / returned factory 以取得 remote module export object，只包装被审阅的 function export，并继续通过 `virtual://workspace/function-hooks.json` 与 `virtual://workspace/function-hook-timeline.json` 记录 `remote_export_*` 调用事件；这个 follow-through 不自动安装所有 exports、不递归遍历 remotes、不调用 MCP。输出仍是 `virtual://workspace/module-registry.json` 与 `virtual://workspace/module-candidates.json`；webpack-like candidate 可继续喂给 `hook-module`，`hook_kind=function-path` 的 custom / federation candidate 应转给 `hook-function`。显式传入 `source-logpoint` / `logpoint` protection、`url_pattern` / `line_number` 和 `log_expression` 时，`native-web` 可以通过 CDP 条件断点安装 source-level logpoint，并返回 `virtual://workspace/source-logpoints.json` 与 `virtual://workspace/source-logpoint-timeline.json`；如果同时传入 `bundle_offset` + `bundle_source`，或 `source_map` + `original_source` + `original_line` / `original_column`，会先把 generated bundle offset 或 Source Map v3 mapping 重映射为 CDP generated line / column；Source Map baseline 现在支持 exact match、greatest-lower-bound bias fallback、`sourceRoot` 源路径归一化、indexed source map `sections` 的 offset 合并、source-map `names` metadata、URL-like source equivalence 和 nested indexed-section stack metadata，并把 `remap` 元数据写入 source-logpoint artifact。当前 baseline 覆盖目标函数路径观测、webpack-like module export 观测、webpack-like module export candidate 自动发现、webpack-like runtime module cache / registry 只读 introspection、custom object runtime / module federation 暴露模块的 function-path candidate baseline、只读 async chunk graph / loader metadata baseline、显式候选名的 closure-scope function evidence baseline、脚本 URL / 行号观测、review-gated credentialless Source Map URL / section URL fetch metadata、descriptor-safe scoped object-root mutation audit，以及 reviewed single-step custom-loader execution、bounded custom-loader traversal continuation planning、review-only custom-loader traversal graph / queue baseline、review-only multi-step custom-loader traversal workflow plan、review-gated custom-loader traversal workflow execution baseline、review-gated bounded custom-loader traversal loop execution baseline、review-only custom-loader continuation workflow planning、review-gated custom-loader continuation journal、review-approved one-step custom-loader continuation execution、review-only custom-loader module diff / hook candidate refresh、review-approved custom-loader module hook follow-through、review-only async chunk traversal graph / queue、review-only async chunk traversal workflow plan、review-gated async chunk traversal workflow execution baseline 和 review-gated bounded async chunk traversal loop execution baseline；它还不等于任意闭包内部函数自动 wrapper hook，也不是完整 source-map consumer、完整 webpack runtime analyzer、deeper execution-style custom-loader traversal beyond bounded continuation planning / review-only multi-step workflow planning / review-gated traversal workflow execution / review-gated bounded loop execution / review-approved one-step continuation execution，或会递归遍历 remotes / 自动 hook remote exports 的 deep federation analyzer；bundler-specific symbol scoping、凭据化浏览器 Source Map fetch、deeper recursive async chunk traversal beyond reviewed bounded loop execution、deeper federation traversal beyond reviewed remote export hook install 和更深 webpack runtime 解析仍是后续 capability-gated 工作。显式传入 `async-chunk-traversal-graph` / `async-chunk-graph-queue` protection，并提供 chunk graph、reviewed async chunk load result 或 module diff evidence 时，`native-web` 会输出 `virtual://workspace/async-chunk-traversal-graph.json`，构建 review-only node / edge graph 和 bounded queue，只把未加载的 supported webpack runtime chunk 候选放入 review queue；这一步不执行 `require.e`、不请求 chunk、不执行 module factory、不安装 hook、不递归、不调用 MCP。显式传入 `async-chunk-traversal-workflow-plan` / `async-chunk-deep-traversal-workflow` protection，并提供同一个 traversal graph 时，`native-web` 会输出 `virtual://workspace/async-chunk-traversal-workflow-plan.json`，把 review queue 组合成选择候选、规划 load、执行一个 reviewed chunk load、刷新 module diff、可选安装 reviewed hook、重新 discovery 并停止在下一轮 review 前的人工检查计划；这一步仍不执行任何 stage、不自动 advance queue、不递归。显式传入 `async-chunk-module-diff` / `async-chunk-hook-candidates` protection，并提供 reviewed `async_chunk_load_result` 与刷新后的 `module_discovery` / `modules` 时，`native-web` 会输出 `virtual://workspace/async-chunk-module-diff.json`，把 added registry/cache keys 映射到 module discovery 里的新增 module，并生成 review-only `hook-module` 候选；这一步不加载 chunk、不执行 module factory、不安装 hook、不调用 MCP。显式传入 `async-chunk-module-hook` / `hook-async-chunk-module` protection、提供同一个 `async_chunk_module_diff` 候选并设置 `review_approved=true` 后，`native-web` 才会委托现有 `ModuleHookManager` 安装对应 module export wrapper，继续用 `virtual://workspace/module-hooks.json` 与 `virtual://workspace/module-hook-timeline.json` 记录安装结果和调用事件；这个 follow-through 不重新加载 chunk、不重新 discovery、不执行 arbitrary custom loader，也不会自动从 diff plan 安装 hook。`pause_session_id` 现在还能把 retained paused session 续用到后续 `paused-session` 动作里；同进程 registry 支持 inspect / evaluate / step / resume 这类 live CDP 调试续作，并会在 `continuation_preflight` 里标出 `source=registry`、requested action、pre-action lifecycle、live continuation 可用性和 action 后 lifecycle。显式传入 `persist_paused_session` / `paused_session_store_dir` 时，会额外落盘 durable paused-session snapshot，后续进程可通过 `paused-session` inspect 读取 debugger session、timeline、callframes 和 breakpoints 做审计；durable snapshot 的 `continuation_preflight` 固定标记 `source=durable_snapshot` / `status=inspect_only`，resume / step / evaluate 会结构化变成 `status=action_blocked` 与 `live_paused_session_required`，仍不支持跨进程 resume / step / evaluate 原 CDP paused execution。callframe evaluation 默认走 `read_only` policy，会给 `Debugger.evaluateOnCallFrame` 加 `throwOnSideEffect` 并阻断明显高风险表达式；显式设置 `allow_callframe_side_effects` 才允许副作用表达式。当前 `workspace/mutation-audit.json` 会记录这些 callframe evaluation 的风险分类与审计摘要。显式传入 `page-mutation-audit` / `page-mutation` protection 时，`native-web` 还会围绕触发表达式采集页面 before / after 摘要，输出 `virtual://workspace/page-mutation-audit.json`，覆盖 DOM 尺寸、storage key、cookie name 与选定 global preview 的粗粒度 diff。显式传入 `object-root-mutation-audit` / `object-mutation-audit` / `js-object-mutation-audit` protection，或传入 `root_path` / `object_root_path` / `js_object_root` 这类 context key 时，`native-web` 会围绕显式 trigger 对 strict dotted JS object root 做 descriptor-safe before / after snapshot，输出 `virtual://workspace/object-root-mutation-audit.json`，记录 added / removed / changed / type-changed / descriptor-changed paths，并在 snapshot 阶段避免 prototype traversal 和 accessor getter invocation。显式传入 `mutation-observer-timeline` / `mutation-timeline` protection，或传入 `observer_wait_ms` / `mutation_record_limit` 这类 context key 时，`native-web` 会围绕显式 trigger 安装 `MutationObserver`，输出 `virtual://workspace/mutation-observer-timeline.json`，记录有限的 `childList` / `attributes` / `characterData` DOM mutation records；这些 baseline 仍不是完整 JS heap diff、任意 object graph diff 或闭包内部状态审计。`debugger-timeline.json` 会把 breakpoint set、trigger、pause、callframe evaluation、debugger action 和 auto-resume/retained state 串成单次调试审计轨迹。常规 `native-web` recon 现在也会把 baseline collector 中的 navigation、network、request initiator、hook timeline 和 replay validation 片段归一化为 `virtual://workspace/flow-timeline.json`，并进入 `workspace/backend-artifact-manifest.json`；每个新增 timeline entry 会带保守的 `correlation` hints，来源包括 request id、URL path、method、initiator function、hook path、candidate id 和 validation function；artifact 顶层还会输出 `correlation_groups`，按共享 request id、URL path + method、function name、candidate id 或 hook path 归为候选证据桶，并固定标注 `stitching=false`；每个 group 还会带 `verification.status=weak|reviewable|ready_for_manual_stitch_review`、evidence booleans 和 `missing_for_ready`，供 review gate 或后续子智能体参考；`reviewable` / `ready_for_manual_stitch_review` 的 group 还会被提升为 `stitch_candidates`，输出人工复核路径、证据布尔值、缺失项和 `automatic_stitching=false`；这些 candidates 会额外生成 `auto_stitch_dry_runs`，用于输出 `confidence_score`、`score_reasons`、`conflict_reasons`、`review_required=true` 和 `would_materialize=false` 的自动拼接 dry-run 建议；随后还会生成 `auto_stitch_conflict_resolutions` / `auto_stitch_conflict_resolution_summary`，把冲突候选、review-preferred candidate、alternatives 和 unresolved conflicts 输出为 `auto-stitch-conflict-resolutions.json` 的 review-only baseline；再生成 `auto_stitch_policy_decisions` / `auto_stitch_policy_summary`，用于把阈值、冲突、缺失证据和 materialization request 归一化成 policy gate 决策；policy-eligible decision 还会生成 `auto_stitch_materialization_plans` / `auto_stitch_materialization_summary`，描述写入 `stitched-flow.json` 前必须 review 的 entry path、review requirements、conflict resolution 和 rollback plan，默认仍然 plan-only；显式 `auto_stitch_materialization_review_decisions` 审批通过后，会生成 `auto_stitch_materialization_results` / `auto_stitch_materialization_result_summary` 和 `auto-stitch-materialization-results.json`，并把 approved plan 转成 reviewer-approved `stitched-flow.json` baseline；同时会生成 `auto_stitch_materialization_audit_entries` / `auto_stitch_materialization_audit_summary`、`auto_stitch_materialization_rollback_plans` / `auto_stitch_materialization_rollback_summary`、`auto_stitch_materialization_transactions` / `auto_stitch_materialization_transaction_summary`，对应 `stitched-flow-materialization-audit.json`、`stitched-flow-rollback-plan.json`、`stitched-flow-materialization-transactions.json`，并继续派生 `auto_stitch_rollback_execution_plans` / `auto_stitch_rollback_execution_summary` 和 `stitched-flow-rollback-executions.json`；显式 `auto_stitch_rollback_execution_review_decisions` 审批后只记录 `auto_stitch_rollback_execution_results` logical rollback result，并派生 `auto_stitch_rollback_review_gate_recomputations` / `review-gate-after-rollback.json` blocking baseline，以及 `auto_stitch_physical_rollback_dry_run_diffs` / `stitched-flow-physical-rollback-diff.json` dry-run diff；显式 `auto_stitch_physical_rollback_review_decisions` 审批后还会生成 `auto_stitch_physical_rollback_results` 并移除当前 `stitched_flows` artifact model 中的匹配项；显式 `auto_stitch_standard_review_gate_replacement_review_decisions` 审批后会生成 `auto_stitch_standard_review_gate_replacement_results`，并进一步派生 `auto_stitch_post_standard_review_gate_replacement_delivery_guard_reruns` / `delivery-guard-after-review-gate-replacement.json`，记录 `delivery_guard_passed=true` / `delivery_allowed=true` / `automatic_delivery=false`；delivery guard passed 后还会派生 `auto_stitch_post_standard_review_gate_replacement_final_delivery_packages` / `final-delivery-package-after-review-gate-replacement.json`，记录 `package_ready=true` / `final_delivery_packaged=true` / `delivery_allowed=true`，但仍保持 `external_delivery_performed=false` / `cross_run_transaction_committed=false` / `manifest_revision_committed=false`；显式 `auto_stitch_transaction_commit_review_decisions` 审批后会派生 `auto_stitch_transaction_commit_results` / `final-delivery-transaction-commit.json`，记录 `artifact_model_transaction_commit_recorded=true`，但仍保持 `cross_run_transaction_committed=false` / `filesystem_artifact_mutated=false` / `external_delivery_performed=false`；同时仍保持 `automatic_stitching=false` / `automatic_rollback=false`，physical diff 不写 artifact，post-rollback gate baseline 不替换标准 `review-gate.json`，transaction log 只做 result / audit / rollback plan 聚合；其中 `ready_for_manual_stitch_review` candidate 会进一步生成 pending-review `stitch_proposals`，包含 reviewer approval requirements、blocking conditions 和 `review_decision.status=pending_review`，并会通过 evidence promotion 暴露给 review gate；review gate 会以 `review_stitch_proposals_before_delivery` 阻断自动交付；显式传入 `stitch_review_decisions` 或 `auto_stitch_materialization_review_decisions` 审批通过后，会生成 reviewer-approved `stitched_flows` 和 `stitched-flow.json` baseline，但仍保持 `automatic_stitching=false`，不会无需审批地自动 materialize 全链路；未审批前 `approved=false` / `stitching=false` / `automatic_stitching=false`，不会生成已拼接链路；显式传入 `flow-timeline` / `cross-request-timeline` protection，并提供 `previous_flow_timeline`、`flow_events`、`network_requests`、`hook_timeline`、`debugger_timeline`、`function_hook_timeline`、`module_hook_timeline`、`source_logpoint_timeline`、`mutation_observer_timeline` 或 `replay_validation` 这类已采集片段时，`native-web` 还会归一化并延续输出同一个 `flow-timeline.json` artifact，作为跨请求证据串联的 baseline；这些 hints / groups / readiness / candidates / proposals 不会自动订阅所有浏览器事件，也不等于完整自动 stitching。跨进程 live CDP paused execution continuation、任意闭包内部函数自动 wrapper hook、bounded custom-loader traversal continuation planning / review-only custom-loader continuation workflow planning / review-gated custom-loader continuation journal / reviewed single-step custom-loader execution plus module-diff refresh / reviewed module hook follow-through 之外的 deeper recursive custom-loader traversal / reviewed one-step async chunk traversal workflow + reviewed chunk load + module diff refresh 之外的 deeper async chunk traversal / reviewed module-federation factory invoke + review-only export-hook-plan baseline 之外的 deeper federation traversal beyond reviewed remote export hook install、full source-map consumer semantics / bundler-specific symbol scoping beyond credentialless URL fetch metadata、完整 JS heap / object graph diff beyond scoped object-root audit 和无需审批的自动全链路跨请求 timeline materialization、更完整 conflict resolver、写入型 cross-run rollback executor / physical transaction state machine beyond read-only rollback state baseline、advanced adaptive provider retry policy、third-party external delivery provider、write-capable cross-run rollback executor / physical state machine beyond read-only rollback state baseline 和 real external distributed transaction lock providers beyond the local-file / SQLite local transactional baselines / full durable resume workflow scheduler beyond the single-transition runner baseline 仍然是后续 capability-gated 工作；GitHub 同名 asset overwrite/delete 已支持显式审批后的 DELETE + replacement upload baseline，但更复杂的 rate-limit / partial-failure recovery 仍需后续收口。
