...
- cross-process live CDP paused execution continuation，超过当前只读 live-continuation preflight、target attach readiness proof、cross-process execution plan、reviewed attach/detach probe、read-only live callFrame recovery proof、review-gated resume / step / evaluate one-action executor、review-only next paused-event capture plan 和 review-gated one-shot next paused-event capture execution baseline；多步 live continuation、pre-action subscribe-and-action orchestration、wrapper continuation 集成和跨进程长期 session 管理仍未实现。
