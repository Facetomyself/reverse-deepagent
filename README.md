...
- full source-map consumer semantics，超过当前 local remap、review-only source-map lookup descriptor、review-only source-map source-content availability descriptor、review-only source-map readiness join descriptor、review-only source-map consumer action plan descriptor、review-only source-map consumer materialization descriptor、review-only bundler symbol scope descriptor 与 credentialless URL fetch metadata baseline；当前 descriptors 只给出保守 lookup / source-content availability metadata / readiness / reviewed action plan / reviewed materialized payload / 候选和 hook readiness，不宣称完整符号绑定 / scope 解析，也不导出 raw source content。
- full JS heap diff / runtime-collected object graph diff，超过当前 scoped object-root audit baseline 与 review-only object graph diff descriptor；当前 `object-graph-diff` 只是基于调用方 snapshots 的保守 diff / review hints，不宣称完整 heap traversal。
