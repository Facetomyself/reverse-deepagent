...
- Custom-loader / async-chunk：review-only traversal plan、graph / queue、workflow plan、review-gated one-step workflow execution、bounded loop plan、review-gated bounded loop execution、module diff refresh 和 reviewed module hook follow-through。
...
- deeper recursive custom-loader traversal，超过当前 bounded continuation / workflow / loop / one-step execution baseline。
