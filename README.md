...
- cross-process live CDP paused execution continuation，超过当前只读 live-continuation preflight、target attach readiness proof、cross-process execution plan、reviewed attach/detach probe、read-only live callFrame recovery proof 和 review-gated resume / step / evaluate one-action executor baseline；多步 live continuation、自动 paused-event capture、wrapper continuation 集成和跨进程长期 session 管理仍未实现。
