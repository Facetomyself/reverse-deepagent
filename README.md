...
输出里的 `external_delivery_provider_matrix` 会列出 `review-only` 以及 `noop` / `manual-handoff` alias，并显示 `reverse_deepagent.external_delivery_providers` entry point group、provider transport、`review_only`、`supports_external_delivery` 和 side-effect policy。真实 release / webhook / object storage / GitHub Release provider 仍需以后续插件形式接入。
