...
本地交付执行器已提供最小 contract baseline：`LocalDeliveryExecutor` 默认 dry-run，只规划 reviewed artifact 的本地交付；delivery subagent 暴露 `execute_local_delivery` tool，显式 `apply` 才会复制文件并写 `delivery-receipt.json` / `delivery-transaction-journal.json`；显式 `commit_manifest_revision=true` 时还会写本地 `delivery-manifest-revision.json`；显式 `commit_backend_manifest_mutation=true` 时会写本地 `backend-artifact-manifest-mutation.json` 与 `backend-artifact-manifest.patched.json`。它仍不是 external delivery executor，不上传、不推送，也不 in-place 改写标准 `backend-artifact-manifest.json`；`backend_manifest_patch_written=true` 只表示 patched copy 已写入，不等价于跨运行 manifest commit。
