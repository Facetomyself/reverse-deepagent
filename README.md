...
- `flow_timeline.auto_stitch_dry_runs` 只提供自动拼接建议、`confidence_score`、`score_reasons`、`conflict_reasons`、`review_required=true` 和 `would_materialize=false`；它不会生成 `stitched-flow.json`，也不会绕过 review gate。
