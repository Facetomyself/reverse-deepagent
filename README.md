...
- closure wrapper replacement hardening，超过当前 same-process `log-only-call-through` reviewed install / restore execution baseline，包括 wrapper event harvesting、更强 assignment safety proof 和 cross-process live continuation；当前仍不支持任意闭包内部函数 automatic wrapper hook。
