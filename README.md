...
- closure wrapper replacement hardening，超过当前 review-only assignment safety proof、runtime mutability preflight、review-approved runtime mutability probe/result 与 same-process `log-only-call-through` reviewed install / restore execution baseline，包括 arbitrary closure-local wrapper support 和 cross-process live continuation；当前仍不支持任意闭包内部函数 automatic wrapper hook。
