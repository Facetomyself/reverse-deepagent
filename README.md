...
- 更完整的自动全链路跨请求 conflict resolver、physical rollback state machine、advanced adaptive provider retry、第三方 external delivery provider、broader durable scheduler，以及 broader rollout decision record 之后的 explicit commit-or-rollback executor；这些仍必须保持显式 review / apply intent。
