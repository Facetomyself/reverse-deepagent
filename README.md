...
- cross-process live CDP paused execution continuation，超过当前只读 live-continuation preflight、target attach readiness proof、cross-process execution plan、reviewed attach/detach probe、read-only live callFrame recovery proof、review-gated resume / step / evaluate one-action executor、review-only next paused-event capture plan、review-gated one-shot next paused-event capture execution、review-only continuation checkpoint baseline、review-gated pre-action subscribe-and-action orchestration baseline、review-only multi-step continuation workflow / journal plan baseline、review-gated one-iteration multi-step continuation execution baseline 和 read-only closure-wrapper continuation readiness descriptor；自动多步 live action 循环执行器、自动 wrapper continuation execution 和跨进程长期 session 管理仍未实现。
- closure wrapper replacement hardening，超过当前 review-only assignment safety proof、runtime mutability preflight、review-approved runtime mutability probe/result、strategy descriptor/catalog metadata 与 same-process `log-only-call-through` reviewed install / restore execution baseline，以及 read-only wrapper-continuation readiness descriptor，包括 arbitrary closure-local wrapper support、非 `log-only-call-through` 策略的 reviewed executor 和 cross-process live wrapper execution；当前仍不支持任意闭包内部函数 automatic wrapper hook。
