...
- Source map：generated bundle offset、Source Map exact / bias / `sourceRoot` / indexed section / `names` / URL equivalence / nested indexed-section remap、review-only `source-map-lookup` descriptor、review-only bundler symbol scope descriptor，以及 review-gated credentialless source-map fetch metadata。`source-map-lookup` 只消费调用方提供的 Source Map payload 与 generated/original 位置，输出 `workspace/source-map-lookup.json`，不 fetch Source Map、不启动浏览器、不 evaluate JavaScript、不发送 CDP、不调用 MCP，也不触碰移动端完整链路；`bundler-symbol-scope` 只消费调用方提供的 Source Map / script / module hints，输出 `workspace/bundler-symbol-scope.json`，不 fetch Source Map、不安装 source logpoint、不 evaluate JavaScript、不发送 CDP、不调用 MCP，也不触碰移动端完整链路。
...
- full source-map consumer semantics，超过当前 local remap、review-only source-map lookup descriptor、review-only bundler symbol scope descriptor 与 credentialless URL fetch metadata baseline；当前 descriptors 只给出保守 lookup / 候选和 hook readiness，不宣称完整符号绑定 / scope 解析。
