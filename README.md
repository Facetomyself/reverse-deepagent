...
- `flow_timeline.auto_stitch_dry_runs` 只提供自动拼接建议、`confidence_score`、`score_reasons`、`conflict_reasons`、`review_required=true` 和 `would_materialize=false`；`flow_timeline.auto_stitch_policy_decisions` 进一步给出 conservative policy gate 结果、threshold、blockers 和 review-gate eligibility；`flow_timeline.auto_stitch_materialization_plans` 只生成 plan-only 的目标 artifact、entry path、review requirements 和 rollback plan。三者都固定 `would_materialize=false` / `automatic_stitching=false` / `writes_artifact=false`，不会生成 `stitched-flow.json`，也不会绕过 review gate。
- 显式 `stitch_review_decisions` 审批通过后，`native-web` 可以生成 `workspace/stitched-flow.json` / `virtual://workspace/stitched-flow.json`，但这只是 reviewer-approved materialization baseline，仍保持 `automatic_stitching=false`，不代表自动全链路 stitching。
