...
本地交付执行器已提供最小 contract baseline：`LocalDeliveryExecutor` 默认 dry-run，只规划 reviewed artifact 的本地交付；delivery subagent 暴露 `execute_local_delivery` tool，显式 `apply` 才会复制文件并写 `delivery-receipt.json` / `delivery-transaction-journal.json`；显式 `commit_manifest_revision=true` 时还会写本地 `delivery-manifest-revision.json`。它仍不是 external delivery executor，不上传、不推送，也不改写标准 `backend-artifact-manifest.json`。
