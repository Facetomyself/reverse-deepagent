...
- Same-process paused-session continuation, durable inspect-only paused-session snapshots, and read-only paused-session live-continuation preflight evidence for same-process / durable / provided-artifact debugger state, including live-session / target / callframe / action-capability diagnostics without sending CDP commands.
