...
- Cross-process live CDP paused execution continuation beyond the read-only live-continuation preflight, target attach readiness proof, plan-only cross-process execution descriptor, reviewed attach/detach probe, read-only live callFrame recovery proof, and review-gated one-action resume / step / evaluate executor baselines; multi-step continuation, automatic paused-event capture, wrapper continuation integration, and longer-lived cross-process session management remain capability-gated follow-ups.
