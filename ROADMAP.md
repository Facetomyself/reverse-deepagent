...
- Full source-map consumer semantics beyond the current local remap, review-only source-map lookup descriptor, review-only source-map source-content availability descriptor, review-only source-map readiness join descriptor, review-only source-map consumer action plan descriptor, review-only bundler symbol scope descriptor, and review-gated credentialless URL fetch metadata baseline; current lookup / source-content / readiness / action-plan / symbol-scope outputs are conservative review descriptors, not automatic debugger / rebuild / source-logpoint execution and not complete binding / lexical scope resolution, and source content descriptors never export raw source text or previews.
