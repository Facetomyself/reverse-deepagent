...
- Closure-scope function discovery baseline, review-only closure wrapper replacement planning baseline, and opt-in same-process reviewed closure wrapper replacement execution MVP with restore-plan artifact; arbitrary automatic closure wrapper replacement remains unsupported.
...
- Closure wrapper replacement hardening beyond the current same-process `log-only-call-through` reviewed execution MVP, including reviewed restore execution, wrapper event harvesting, richer assignment safety proofs, and cross-process live CDP continuation integration.
