...
- Review `workspace-foldered-canonical-migration-post-apply-validation` after a ready manifest dry-run, matching apply plan, and observed post-apply backend manifest to confirm canonical-path promotion evidence, digest guard, compatibility validation, and legacy fallback review before tightening any readers; current support remains post-apply validation descriptor only and physical apply remains a separate explicit-review executor.
