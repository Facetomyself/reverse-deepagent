...
- Cross-process live CDP paused execution continuation beyond the read-only live-continuation preflight, target attach readiness proof, plan-only cross-process execution descriptor, reviewed attach/detach probe, and read-only live callFrame recovery proof baselines; resume / step / evaluate one-action executor remains a capability-gated follow-up.
