...
- Review `workspace-foldered-canonical-migration-manifest-dry-run` after a ready `workspace-foldered-canonical-migration-approval-plan` and matching `workspace-foldered-canonical-migration-apply-plan` to confirm backend manifest dry-run changes, apply-plan digest guard, rollback checkpoint requirements, compatibility fallback, and explicit apply executor inputs before any physical canonical-path migration; current support remains manifest dry-run / rollback checkpoint descriptor only and keeps flat paths authoritative.
