...
- Source-level logpoints and Source Map exact / bias / sourceRoot / indexed-section remap baseline, with source-map `names` metadata, URL-like source equivalence, nested indexed-section stack metadata, review-only generated/original source-map lookup descriptor, review-only source-map `sourcesContent` availability descriptor that exports only digest / size / line-count metadata, review-only bundler symbol scope descriptor, and review-gated credentialless external Source Map / indexed-section URL fetch metadata baseline.
...
- Full source-map consumer semantics beyond the current local remap, review-only source-map lookup descriptor, review-only source-map source-content availability descriptor, review-only bundler symbol scope descriptor, and review-gated credentialless URL fetch metadata baseline; current lookup / source-content / symbol-scope outputs are conservative review descriptors, not complete binding / lexical scope resolution, and source content descriptors never export raw source text or previews.
