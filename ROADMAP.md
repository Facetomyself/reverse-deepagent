...
- Review `workspace-foldered-canonical-migration-apply-plan` after a ready `workspace-foldered-canonical-migration-preflight` to confirm manifest mutation guards, rollback requirements, compatibility fallback, and explicit apply executor inputs before any physical canonical-path migration; current support remains apply-plan-only and keeps flat paths authoritative.
