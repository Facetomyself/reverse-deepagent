...
- Cross-process live CDP paused execution continuation beyond the read-only live-continuation preflight, target attach readiness proof, plan-only cross-process execution descriptor, reviewed attach/detach probe, read-only live callFrame recovery proof, review-gated one-action resume / step / evaluate executor, review-only next paused-event capture plan, review-gated one-shot next paused-event capture execution, review-only cross-process continuation checkpoint, review-gated pre-action subscribe-and-action orchestration, and review-only multi-step continuation workflow / journal plan and review-gated one-iteration multi-step continuation execution baselines; automatic multi-step action loops, wrapper continuation integration, and longer-lived cross-process session management remain capability-gated follow-ups.
