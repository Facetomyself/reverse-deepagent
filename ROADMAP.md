...
- Review `workspace-foldered-canonical-migration-approval-plan` after a ready `workspace-foldered-canonical-migration-apply-plan` to confirm approval ledger requirements, transaction / idempotency guards, stale evidence guards, manifest dry-run, rollback checkpoint, post-apply validation, compatibility fallback, and explicit apply executor inputs before any physical canonical-path migration; current support remains approval / transaction descriptor only and keeps flat paths authoritative.
