...
- Same-process paused-session continuation, durable inspect-only paused-session snapshots, read-only paused-session live-continuation preflight evidence, read-only paused-session target attach readiness proof, plan-only cross-process paused-session execution descriptor, and reviewed cross-process Target.attachToTarget / Target.detachFromTarget probe baseline for same-process / durable / provided-artifact debugger state, including live-session / target / callframe / target-correlation / attachability / action-capability diagnostics and follow-up live-callframe recovery review gates. The attach probe is explicit-review-only and does not enable Debugger, recover live callFrames, resume, step, evaluate, call MCP, or touch mobile runtimes.
...
- Cross-process live CDP paused execution continuation beyond the read-only live-continuation preflight, target attach readiness proof, plan-only cross-process execution descriptor, and reviewed attach/detach probe baselines; live callFrame recovery plus resume / step / evaluate executor remain capability-gated follow-ups.
