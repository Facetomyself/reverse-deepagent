...
- Same-process paused-session continuation, durable inspect-only paused-session snapshots, read-only paused-session live-continuation preflight evidence, read-only paused-session target attach readiness proof, and plan-only cross-process paused-session execution descriptor for same-process / durable / provided-artifact debugger state, including live-session / target / callframe / target-correlation / attachability / action-capability diagnostics and future attach-probe / live-callframe recovery review gates without sending CDP commands.
...
- Cross-process live CDP paused execution continuation beyond the read-only live-continuation preflight, target attach readiness proof, and plan-only cross-process execution descriptor baselines; reviewed attach probe, live callFrame recovery, resume / step / evaluate executor remain capability-gated follow-ups.
