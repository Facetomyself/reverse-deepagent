...
- 标准 `workspace/review-gate.json` 替换式重算仍未实现；审批后的 physical rollback result 只要求 `standard_review_gate_rerun_required=true`。
