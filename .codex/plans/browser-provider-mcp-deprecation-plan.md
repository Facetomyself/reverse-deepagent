...
- 后续仍需物理 folder migration / dual-write compatibility、消费者迁移和更完整 transaction state machine；Android / iOS / 小程序完整运行链路继续搁置。
...
- 后续仍需 DeepAgents 虚拟文件夹物理迁移 / dual-write compatibility、transaction state machine、GitHub Release provider，以及更高级 Browser/CDP 调试能力；Android / iOS / 小程序完整运行链路继续搁置。
...
- 后续仍需 GitHub Release provider，以及更完整 transaction state machine / cross-run rollback state machine；本 baseline 不管理云 SDK、bucket、credentials，也不绕过 duplicate guard。
...
- 后续仍需 GitHub Release 等 provider，以及更完整 transaction state machine；webhook 与 presigned object-storage provider 已有内置 baseline。
...
- 后续仍需 GitHub Release provider，以及更完整 transaction state machine。
