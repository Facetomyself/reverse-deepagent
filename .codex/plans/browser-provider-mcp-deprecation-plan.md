...
Boundary: this closes the reviewable safety gate before a future reviewed custom-loader execution step. It does not invoke custom loaders, execute webpack loaders, request chunks, run dynamic imports, invoke module factories, execute module federation `get/init`, mutate browser state, call MCP, recurse async graphs, or touch Android / iOS / mini-program full runtime chains. Review-approved single-step custom loader execution and deeper async traversal remain capability-gated follow-ups.
