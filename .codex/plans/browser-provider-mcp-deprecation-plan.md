...
- Boundary：这是 external Redis lease baseline；不实现 Redlock quorum consensus，不自动 lease renewal，不自动 stale takeover，不替换现有 `delivery-transaction-lock.json` gate，不强制 downstream fencing enforcement。
