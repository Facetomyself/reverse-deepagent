...
| 9. Hook / breakpoint migration | 已完成（深度 step/resume 待后续） | fetch/xhr、cookie、WebSocket、anti-debug hook baseline 已实现；BreakpointManager 通过 CDP capability gate 接入 `apply_minimal_protection`，`breakpoints.json` / `debugger-paused.json` / `callframes.json` / 显式 `callframe-evaluations.json` artifact ref 与 evidence 映射已补齐；native-web runtime-eval 候选验证已作为迁移中的最小 replay baseline |
...
当前下一步：Phase 0-10 已完成，`remote-cdp` smoke 路径已接入，Playwright system Chrome smoke、CloakBrowser fixture smoke、Playwright breakpoint paused/callframe smoke 和显式 evaluateOnCallFrame baseline 均已验证，MCP alias deprecation warning 已接入，后续进入深度 debugger step/resume 评估和最终 code review。Step 5.1 到 Step 10 保留为已执行阶段记录，便于 review 和回溯。
