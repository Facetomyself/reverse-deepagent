...
| 9. Hook / breakpoint migration | 已完成（跨请求 pause 生命周期、source map / bundle offset remapping、webpack 内部函数 hook 待后续） | fetch/xhr、cookie、WebSocket、anti-debug hook baseline 已实现；target-function wrapper baseline 已实现，可对 `window.buildSign` 这类全局可达路径安装 wrapper 并输出 `function-hooks.json` / `function-hook-timeline.json`；source-level logpoint baseline 已实现，可对脚本 URL / 行号安装条件断点并输出 `source-logpoints.json` / `source-logpoint-timeline.json`；BreakpointManager 通过 CDP capability gate 接入 `apply_minimal_protection`，`breakpoints.json` / `debugger-paused.json` / `callframes.json` / 显式 `callframe-evaluations.json` / `debugger-actions.json` / `debugger-session.json` / `debugger-timeline.json` artifact ref 与 evidence 映射已补齐；native-web runtime-eval 候选验证已作为迁移中的最小 replay baseline |
...
当前下一步：Phase 0-10 已完成，`remote-cdp` smoke 路径已接入，Playwright system Chrome smoke、CloakBrowser fixture smoke、Playwright breakpoint paused/callframe smoke、显式 evaluateOnCallFrame baseline、callframe evaluation policy baseline、debugger step-control baseline、paused-session snapshot baseline、single-run debugger timeline baseline，以及 target-function wrapper baseline / source-level logpoint baseline 均已验证，MCP alias deprecation warning 已接入，后续进入跨请求 paused session 生命周期管理、source map / bundle offset remapping、webpack / module-internal function hook 和最终 code review。Step 5.1 到 Step 10 保留为已执行阶段记录，便于 review 和回溯。
