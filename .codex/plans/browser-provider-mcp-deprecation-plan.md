...
- 真实 release / webhook / object storage / GitHub release / third-party delivery provider 仍需后续 provider 插件实现。
...
- 真实 release / webhook / object storage / GitHub Release provider 应以后续插件形式挂到 `reverse_deepagent.external_delivery_providers`。
...
- 真实 release / webhook / object storage / GitHub Release provider 仍需后续插件实现。
...
- 该 baseline 不是完整 cross-run transaction state machine，也不实现真实 release / webhook / object storage / GitHub Release provider。
...
- `DeliveryExecutorConfig` / `execute_local_delivery` tool：新增 `external_delivery_provider_config` 与 `external_delivery_provider_config_json`，当前可用于传入 `archive_root`，后续 webhook / object storage / GitHub Release provider 可复用同一 provider-specific config seam。
...
- 后续仍需 webhook / object storage / GitHub Release 等 provider，以及更完整 transaction state machine。
...
- 后续 webhook / object storage / GitHub Release provider 必须复用该 summary / metadata guard，不得自定义绕过。
...
- 后续仍需 object storage / GitHub Release provider，以及更完整 transaction state machine。
