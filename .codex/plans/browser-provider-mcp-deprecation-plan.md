...
### Step 267：Read-only paused-session automatic-loop multi-iteration executor-input preflight descriptor baseline

Status: implemented as a read-only / review-only / preflight-only executor-input descriptor after the Step 266 multi-iteration next-step review plan. This is not a next-iteration executor, not a checkpoint writer, not automatic live callFrame recovery, not automatic queue / loop advancement, not a long-lived cross-process session manager, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`PausedSessionAutomaticLoopMultiIterationExecutorInputPreflightManager` consumes a ready `paused_session_automatic_loop_multi_iteration_next_step_plan` plus the existing Step 264 executor input evidence: written multi-iteration transaction journal, ready bounded executor gate, ready loop plan, ready multi-step workflow, fresh live callFrame recovery evidence, and retained attached session identifiers. It reuses the Step 264 executor input blockers as validation only, forcibly disables execution / review approval flags, and emits `reverse-deepagent.paused-session-automatic-loop-multi-iteration-executor-input-preflight.v1` with `ready_for_execution_review=true` only when the reviewed executor input package is coherent.

`native-web` exposes this through `paused-session-automatic-loop-multi-iteration-executor-input-preflight` / `review-paused-session-automatic-loop-multi-iteration-executor-input-preflight` / `preflight-paused-session-automatic-loop-multi-iteration-executor-input` / `review-next-paused-session-automatic-loop-multi-iteration-executor-input` / `paused-session-automatic-loop-multi-iteration-executor-input-review` and returns `virtual://workspace/paused-session-automatic-loop-multi-iteration-executor-input-preflight.json`. The workspace contract indexes `workspace/paused-session-automatic-loop-multi-iteration-executor-input-preflight.json` with future alias `/workspace/debugger/paused-session-automatic-loop-multi-iteration-executor-input-preflight.json` and category `triage`; coordinator extraction / artifact category mapping classify the optional evidence source as `triage`; debugger review blocks blocked descriptors and warns when a ready descriptor can proceed to explicit Step 264 executor review.
...
### Step 270：Source Map consumer typed review payload metadata hardening

Status: implemented as a review-only typed payload hardening layer after the Step 212 Source Map consumer materialization descriptor. This is not full source-map consumer semantics, not automatic debugger execution, not source-logpoint installation, not hook installation, not rebuild execution, not Source Map fetching, not browser launch, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`SourceMapConsumerMaterializationManager` now emits a stable `reverse-deepagent.source-map-consumer-typed-review-payload.v1` schema through top-level `typed_payload_schema_version`, `typed_review_payloads`, `typed_review_payload_count`, and `typed_review_payload_consumers`. The typed review payloads cover debugger location review, source-logpoint plan review, rebuild source metadata review, and hook symbol-scope review. Each payload includes consumer-specific `executor_input`, `review_required=true`, `execute_automatically=false`, and explicit safety booleans proving that raw source / preview export, Source Map fetch, browser start, CDP command, debugger execution, runtime evaluation, logpoint / hook installation, rebuild execution, MCP calls, and mobile runtime use did not occur.

`native-web` now carries the typed payload schema, count, and consumers into `source_map_consumer_materialization_*` verification strings and `virtual://workspace/source-map-consumer-materialization.json` artifact metadata. The runtime path remains pre-browser-session and metadata-only for this descriptor; it does not invoke provider factories, start browsers, send CDP commands, evaluate JavaScript, install hooks / logpoints, execute rebuilds, call MCP, or touch Android / iOS / mini-program full runtime chains.
...
### Step 273：Source Map follow-through single-surface selection descriptor baseline

Status: implemented as a read-only / review-only / selection-only descriptor after the Step 272 follow-through review handoff. This is not automatic debugger execution, source-logpoint installation, hook installation, rebuild execution, Source Map fetching, browser launch, MCP integration, or an Android / iOS / mini-program full runtime chain.

`SourceMapFollowthroughSurfaceSelectionManager` consumes a ready `source-map-followthrough-review` descriptor or explicit follow-through review items, requires an unambiguous selected action id / consumer / review surface when multiple candidates exist, validates that the selected debugger / source-logpoint / rebuild / hook item is still ready for explicit downstream review, and emits `reverse-deepagent.source-map-followthrough-surface-selection.v1` with the selected review, selected executor input, downstream prompt / next action, `selection_only=true`, `surface_executor_invoked=false`, blocker details, and a fixed no-side-effect policy.

`native-web` exposes this through `source-map-followthrough-surface-selection` / `source-map-followthrough-surface-review` / `source-map-followthrough-surface-selector` / `select-source-map-followthrough-surface` / `review-source-map-followthrough-surface-selection` / `review-selected-source-map-followthrough-surface` protection names before browser session acquisition and returns `virtual://workspace/source-map-followthrough-surface-selection.json`. The workspace contract indexes `workspace/source-map-followthrough-surface-selection.json` with future alias `/workspace/debugger/source-map-followthrough-surface-selection.json` and category `triage`; coordinator extraction / artifact category mapping classify the optional evidence source as `triage`; hook review warns when ready and blocks blocked descriptors.

Boundary: this closes only the single-surface selection layer after the follow-through review handoff. It executes no debugger command, installs no source-logpoint or hook, runs no rebuild, exports no raw source or preview, fetches no Source Map, starts no browser, evaluates no JavaScript, sends no CDP, calls no MCP, and touches no Android / iOS / mini-program full runtime chain.

Tests cover manager-level ready / ambiguous-selection / unsafe-selected-review blockers, native-web route metadata and no-browser-start verification, workspace route aliasing, coordinator payload/category mapping, hook-subagent warning / blocker wiring, compileall, and source-map / native-web / workspace / coordinator / hook regressions.

### Step 274：Source Map selected executor-input review descriptor baseline

Status: implemented as a read-only / review-only / preflight-only executor-input review descriptor after the Step 273 follow-through surface selection descriptor. This is not automatic debugger execution, source-logpoint installation, hook installation, rebuild execution, Source Map fetching, browser launch, MCP integration, or an Android / iOS / mini-program full runtime chain.

`SourceMapSelectedExecutorInputReviewManager` consumes a ready `source-map-followthrough-surface-selection` descriptor or explicit selected review / executor input, verifies selected action id / consumer / surface expectations, revalidates consumer-specific executor-input shape for debugger / source-logpoint / rebuild / hook, and emits `reverse-deepagent.source-map-selected-executor-input-review.v1` with a stable downstream executor review package, review gate metadata, selected executor input, `executor_input_review_only=true`, `ready_for_executor_review`, blocker details, and a fixed no-side-effect policy.

`native-web` exposes this through `source-map-selected-executor-input-review` / `source-map-followthrough-executor-input-review` / `source-map-selected-followthrough-review` / `review-source-map-selected-executor-input` / `review-selected-source-map-executor-input` / `preflight-selected-source-map-followthrough-executor-input` protection names before browser session acquisition and returns `virtual://workspace/source-map-selected-executor-input-review.json`. The workspace contract indexes `workspace/source-map-selected-executor-input-review.json` with future alias `/workspace/debugger/source-map-selected-executor-input-review.json` and category `triage`; coordinator extraction / artifact category mapping classify the optional evidence source as `triage`; hook review warns when ready and blocks blocked descriptors.

Boundary: this closes only the selected executor-input review package layer after one Source Map follow-through surface has been chosen. It does not invoke debugger commands, install source-logpoints or hooks, run rebuilds, export raw source or previews, fetch Source Maps, start browsers, evaluate JavaScript, send CDP, call MCP, or touch Android / iOS / mini-program full runtime chains. Actual debugger / source-logpoint / rebuild / hook execution remains explicit-review-only follow-up work.

### Step 275：Source Map selected executor approval / apply-plan descriptor baseline

Status: implemented as a read-only / review-only / approval-plan-only / apply-plan-only descriptor after the Step 274 selected executor-input review descriptor. This is not approval recording, debugger execution, source-logpoint installation, hook installation, rebuild execution, Source Map fetching, browser launch, MCP integration, or an Android / iOS / mini-program full runtime chain.

`SourceMapSelectedExecutorApprovalPlanManager` consumes a ready `source-map-selected-executor-input-review` descriptor or explicit executor review package, verifies selected action id / consumer / review gate expectations, revalidates no-side-effect package policy, and emits `reverse-deepagent.source-map-selected-executor-approval-plan.v1` with approval requirements, a future apply plan, `approval_recorded=false`, `ready_to_apply_now=false`, blocker details, and a fixed no-side-effect policy.

`native-web` exposes this through `source-map-selected-executor-approval-plan` / `source-map-selected-executor-apply-plan` / `source-map-followthrough-approval-plan` / `review-source-map-selected-executor-approval-plan` / `plan-source-map-selected-executor-apply` / `review-selected-source-map-executor-approval` protection names before browser session acquisition and returns `virtual://workspace/source-map-selected-executor-approval-plan.json`. The workspace contract indexes `workspace/source-map-selected-executor-approval-plan.json` with future alias `/workspace/debugger/source-map-selected-executor-approval-plan.json` and category `triage`; coordinator extraction / artifact category mapping classify the optional evidence source as `triage`; hook review warns when ready and blocks blocked descriptors.

Boundary: this closes only the approval / apply-plan review layer after one Source Map executor input has been reviewed. It does not record approval, write approval artifacts, invoke debugger commands, install source-logpoints or hooks, run rebuilds, export raw source or previews, fetch Source Maps, start browsers, evaluate JavaScript, send CDP, call MCP, or touch Android / iOS / mini-program full runtime chains. Actual approval recording and executor application remain explicit follow-up work.

Tests cover manager-level ready / blocked / expectation-mismatch approval planning, native-web route metadata and no-browser-start verification, workspace route aliasing, coordinator payload/category mapping, hook-subagent warning / blocker wiring, compileall, and source-map / native-web / workspace / coordinator / hook regressions.

### Step 276：Source Map selected executor approval-record writer baseline

Status: implemented as an explicit-review-only approval-record writer after the Step 275 Source Map selected executor approval / apply-plan descriptor. This is not selected executor application, debugger execution, source-logpoint installation, hook installation, rebuild execution, Source Map fetching, browser launch, MCP integration, or an Android / iOS / mini-program full runtime chain.

`record_source_map_selected_executor_approval` consumes a ready `source-map-selected-executor-approval-plan` descriptor from JSON or artifact-ref, verifies reviewer / decision / mode / write-result / approval flag / selected action id / consumer / review gate / optional digest guards, and writes `reverse-deepagent.source-map-selected-executor-approval-record.v1` to `workspace/source-map-selected-executor-approval-record.json` only when `mode=apply`, `write_result=true`, `approve_approval_record=true`, reviewer is non-empty, and all plan gates pass. Dry-run remains read-only and blocked attempts write nothing.

The hook subagent exposes `record_source_map_selected_executor_approval` alongside Source Map hook-review surfaces. The workspace contract indexes `workspace/source-map-selected-executor-approval-record.json` with future alias `/workspace/debugger/source-map-selected-executor-approval-record.json` and category `audit`; coordinator extraction / artifact category mapping classify the optional evidence source as `audit`; hook review warns when a written approved record requires apply-preflight follow-up and blocks blocked records.

Boundary: this closes only the explicit approval-record layer after Source Map selected executor approval planning. It records approval metadata only, writes no apply preflight, invokes no debugger command, installs no source-logpoint or hook, runs no rebuild, exports no raw source or preview, fetches no Source Map, starts no browser, evaluates no JavaScript, sends no CDP, calls no MCP, and touches no Android / iOS / mini-program full runtime chains. Actual apply preflight and selected executor application remain explicit follow-up work.

Tests cover dry-run read-only approval planning, apply approval gates, apply record writing without executor / CDP / hook / rebuild side effects, hook subagent tool exposure, hook-review warning / blocker wiring, workspace route aliasing, coordinator payload/category mapping, compileall, and related hook / workspace / coordinator regressions.

### Step 277：Source Map selected executor apply-preflight descriptor baseline

Status: implemented as a read-only / review-only / apply-preflight-only descriptor after the Step 276 Source Map selected executor approval-record writer. This is not selected executor application, debugger execution, source-logpoint installation, hook installation, rebuild execution, Source Map fetching, browser launch, MCP integration, or an Android / iOS / mini-program full runtime chain.

`SourceMapSelectedExecutorApplyPreflightManager` consumes a ready `source-map-selected-executor-approval-plan` descriptor and a written approved `source-map-selected-executor-approval-record` descriptor, verifies selected action id / consumer / review gate / approval record id / approval-plan digest / executor input / no-side-effect gates, and emits `reverse-deepagent.source-map-selected-executor-apply-preflight.v1` with `approval_record_verified`, `executor_input_ready`, `ready_for_selected_executor_review`, `ready_to_apply_now=false`, and `future_executor_contract.implemented=false`.

`native-web` exposes this through `source-map-selected-executor-apply-preflight` / `source-map-selected-executor-application-preflight` / `source-map-followthrough-apply-preflight` / `review-source-map-selected-executor-apply-preflight` / `preflight-source-map-selected-executor-apply` / `review-selected-source-map-executor-apply-preflight` protection names before browser session acquisition and returns `virtual://workspace/source-map-selected-executor-apply-preflight.json`. The workspace contract indexes `workspace/source-map-selected-executor-apply-preflight.json` with future alias `/workspace/debugger/source-map-selected-executor-apply-preflight.json` and category `audit`; coordinator extraction / artifact category mapping classify the optional evidence source as `audit`; hook review warns when ready for selected executor application review and blocks blocked descriptors.

Boundary: this closes only the apply-preflight review layer after Source Map selected executor approval recording. It writes no artifact, invokes no debugger command, installs no source-logpoint or hook, runs no rebuild, exports no raw source or preview, fetches no Source Map, starts no browser, evaluates no JavaScript, sends no CDP, calls no MCP, and touches no Android / iOS / mini-program full runtime chains. Subsequent Steps 278-282 close the explicit-review-only selected source-logpoint, rebuild metadata, debugger, hook, and reviewed rebuild bundle generation MVP surfaces; automatic Source Map follow-through, automatic raw-source-aware rebuild generation, and mobile full runtime chains remain separate follow-up work.

Tests cover manager-level ready / missing approval-record / digest-mismatch preflight review, native-web route metadata and no-browser-start verification, workspace route aliasing, coordinator payload/category mapping, hook-review warning / blocker wiring, compileall, and related source-map / native-web / workspace / coordinator / hook regressions.

### Step 278：Source Map selected source-logpoint application MVP

Status: implemented as an explicit-review-only selected executor application after the Step 277 Source Map selected executor apply-preflight descriptor. This is not automatic debugger execution, rebuild execution, hook installation, automatic source-logpoint installation without review, Source Map fetching, MCP integration, or an Android / iOS / mini-program full runtime chain.

`native-web` exposes this through `source-map-source-logpoint-application` / `source-map-source-logpoint-install` / `source-map-selected-source-logpoint-application` / `source-map-selected-source-logpoint-executor-application` / `apply-source-map-source-logpoint` / `install-reviewed-source-map-source-logpoint`. The route first validates a ready `source-map-selected-executor-apply-preflight` descriptor for `selected_consumer=source-logpoint`, explicit `mode=apply`, `review_approved=true`, `approve_source_logpoint_install=true`, reviewer, and reviewed `source_logpoint_install_input`; blocked inputs return `virtual://workspace/source-map-source-logpoint-install-result.json` without starting a browser.

When all gates pass, the route starts the configured BrowserProvider session, reuses the existing native `SourceLogpointManager`, and emits `virtual://workspace/source-map-source-logpoint-install-result.json` together with `virtual://workspace/source-logpoints.json` and `virtual://workspace/source-logpoint-timeline.json`. The workspace contract indexes `workspace/source-map-source-logpoint-install-result.json` with future alias `/workspace/debugger/source-map-source-logpoint-install-result.json` and category `audit`; coordinator extraction / artifact category mapping classify the optional evidence source as `audit`; hook review warns on successful installs so reviewers inspect the source-logpoint timeline and blocks failed install results.

Boundary: this closes only the first selected Source Map executor application surface for source-logpoints. It sends Runtime / Debugger commands only after explicit review approval and only for the reviewed source-logpoint install input. Step 279 closes the rebuild metadata selected executor application surface, Step 280 closes the debugger selected executor application surface, and Step 281 closes the hook selected executor application surface; automatic source-logpoint installation without review, automatic Source Map follow-through, MCP integration, and mobile full runtime chains remain separate follow-up work.

Tests cover reviewed source-logpoint application, blocked pre-browser approval gates, workspace route aliasing, coordinator payload/category mapping, hook-review warning / blocker wiring, compileall, and related native-web / workspace / coordinator / hook regressions.

### Step 279：Source Map selected rebuild metadata executor application MVP

Status: implemented as an explicit-review-only / metadata-only selected executor application after the Step 277 Source Map selected executor apply-preflight descriptor and the Step 278 source-logpoint application MVP. This is not reviewed rebuild bundle generation or raw-source-aware rebuild generation, not automatic debugger execution, not hook installation, not Source Map fetching, not browser launch, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`native-web` exposes this through `source-map-rebuild-application` / `source-map-rebuild-metadata-application` / `source-map-rebuild-result` / `source-map-selected-rebuild-application` / `source-map-selected-rebuild-executor-application` / `apply-source-map-rebuild-metadata` / `run-reviewed-source-map-rebuild-metadata-generation`. The route first validates a ready `source-map-selected-executor-apply-preflight` descriptor for `selected_consumer=rebuild`, `selected_review_gate=explicit_rebuild_source_metadata_review`, explicit `mode=apply`, `review_approved=true`, `approve_source_map_rebuild_metadata=true`, reviewer, and reviewed digest-only `rebuild_source_metadata_input`; blocked inputs return `virtual://workspace/source-map-rebuild-result.json` without starting a browser.

When all gates pass, the route emits `virtual://workspace/source-map-rebuild-result.json` with `reverse-deepagent.source-map-rebuild-result.v1`, `metadata_only=true`, `rebuild_metadata_applied=true`, `rebuild_bundle_generated=false`, `rebuild_executed=false`, `raw_source_content_exported=false`, `preview_exported=false`, `browser_started=false`, `runtime_evaluated=false`, `cdp_command_sent=false`, `calls_mcp=false`, and `mobile_runtime_used=false`. The workspace contract indexes `workspace/source-map-rebuild-result.json` with future alias `/workspace/rebuild/source-map-rebuild-result.json` and category `audit`; coordinator extraction / artifact category mapping classify the optional evidence source as `audit`; hook review warns on successful metadata application so reviewers inspect rebuild metadata before any future rebuild generation and blocks failed application results.

Boundary: this closes only the selected Source Map executor application surface for rebuild metadata audit. It applies reviewed digest-only metadata and does not generate `sign_rebuild.py`, call rebuild bundle writers, export raw source text or previews, start browsers, evaluate JavaScript, send CDP, call MCP, or touch mobile full runtime chains. Step 280 closes the debugger selected executor application surface and Step 281 closes the hook selected executor application surface; automatic raw-source-aware rebuild generation from Source Map metadata, automatic Source Map follow-through, and mobile full runtime chains remain separate follow-up work.

Tests cover reviewed rebuild metadata application, blocked pre-browser approval gates, workspace route aliasing, coordinator payload/category mapping, hook-review warning / blocker wiring, compileall, and related native-web / workspace / coordinator / hook regressions.

### Step 280：Source Map selected debugger executor application MVP

Status: implemented as an explicit-review-only selected debugger executor application after the Step 277 Source Map selected executor apply-preflight descriptor, Step 278 source-logpoint application MVP, and Step 279 rebuild metadata application MVP. This is not automatic debugger continuation, not automatic live callFrame recovery, not hook installation, not reviewed rebuild bundle generation or raw-source-aware rebuild generation, not Source Map fetching, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`native-web` exposes this through `source-map-debugger-application` / `source-map-debugger-location-application` / `source-map-debugger-execution-result` / `source-map-selected-debugger-application` / `source-map-selected-debugger-executor-application` / `apply-source-map-debugger-location` / `execute-reviewed-source-map-debugger-location-action`. The route first validates a ready `source-map-selected-executor-apply-preflight` descriptor for `selected_consumer=debugger`, `selected_review_gate=explicit_debugger_location_review`, explicit `mode=apply`, `review_approved=true`, `approve_source_map_debugger_action=true`, reviewer, and reviewed `source_map_debugger_location_input`; blocked inputs return `virtual://workspace/source-map-debugger-execution-result.json` without starting a browser.

When all gates pass, the route starts the configured BrowserProvider session, maps the reviewed Source Map debugger location to one provider-neutral `BreakpointSpec`, reuses `BreakpointManager().set_breakpoint(...)`, and emits `virtual://workspace/source-map-debugger-execution-result.json` together with `virtual://workspace/breakpoints.json`, `virtual://workspace/debugger-paused.json`, `virtual://workspace/callframes.json`, and optional debugger actions / session / timeline artifacts. The result artifact uses `reverse-deepagent.source-map-debugger-execution-result.v1`, records breakpoint / pause / callframe counts, and fixes `automatic_continuation=false`, `automatic_loop=false`, `calls_mcp=false`, and `mobile_runtime_used=false`. The workspace contract indexes `workspace/source-map-debugger-execution-result.json` with future alias `/workspace/debugger/source-map-debugger-execution-result.json` and category `audit`; coordinator extraction / artifact category mapping classify it as `audit`; debugger review warns on successful application so reviewers inspect debugger artifacts and blocks failed / blocked application results.

Boundary: this closes only the selected Source Map executor application surface for one reviewed debugger location. It sends Debugger / Runtime commands only after explicit review approval and only for the reviewed breakpoint input. It does not automatically continue execution, recover live callFrames, capture the next paused event beyond existing breakpoint manager behavior, execute loops, install hooks, generate rebuild bundles, export raw source or previews, fetch Source Maps, call MCP, or touch mobile full runtime chains. Step 281 closes the hook selected executor application surface; automatic raw-source-aware rebuild generation from Source Map metadata, automatic Source Map follow-through, and mobile full runtime chains remain separate follow-up work.

Tests cover reviewed debugger application, blocked pre-browser approval gates, workspace route aliasing, coordinator payload/category mapping, debugger-review warning / blocker wiring, compileall, and related native-web / workspace / coordinator / debugger regressions.

### Step 281：Source Map selected hook executor application MVP

Status: implemented as an explicit-review-only selected hook executor application after the Step 277 Source Map selected executor apply-preflight descriptor and after the source-logpoint / rebuild metadata / debugger selected executor applications. This is not automatic hook inference from Source Map symbol-scope metadata, not automatic Source Map follow-through, not reviewed rebuild bundle generation or raw-source-aware rebuild generation, not Source Map fetching, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`native-web` exposes this through `source-map-hook-application` / `source-map-hook-install` / `source-map-hook-install-result` / `source-map-selected-hook-application` / `source-map-selected-hook-executor-application` / `apply-source-map-hook` / `install-reviewed-source-map-hook-symbol-scope`. The route first validates a ready `source-map-selected-executor-apply-preflight` descriptor for `selected_consumer=hook`, `selected_review_gate=explicit_hook_symbol_scope_review`, explicit `mode=apply`, `review_approved=true`, `approve_source_map_hook_install=true`, reviewer, and reviewed concrete `source_map_hook_install_input`; blocked inputs return `virtual://workspace/source-map-hook-install-result.json` without starting a browser.

When all gates pass, the route starts the configured BrowserProvider session, resolves the reviewed input to either `FunctionHookSpec` or `ModuleHookSpec`, reuses the existing `FunctionHookManager` / `ModuleHookManager`, and emits `virtual://workspace/source-map-hook-install-result.json` together with the matching `function-hooks.json` / `function-hook-timeline.json` or `module-hooks.json` / `module-hook-timeline.json`. The result artifact uses `reverse-deepagent.source-map-hook-install-result.v1`, records hook kind / target / install / event counts, and fixes `automatic_hook_installation=false`, `cdp_command_sent=false`, `calls_mcp=false`, and `mobile_runtime_used=false`. The workspace contract indexes `workspace/source-map-hook-install-result.json` with future alias `/workspace/hooks/source-map-hook-install-result.json` and category `audit`; coordinator extraction / artifact category mapping classify it as `audit`; hook review warns on successful application so reviewers inspect hook timelines and blocks failed / blocked application results.

Boundary: this closes only the selected Source Map executor application surface for one reviewed concrete hook input. It does not infer hook paths automatically from Source Map symbol scope, does not install hooks without explicit review approval and reviewer-supplied function / module hook input, does not send CDP debugger commands, does not generate rebuild bundles, export raw source or previews, fetch Source Maps, call MCP, or touch mobile full runtime chains. Automatic raw-source-aware rebuild generation from Source Map metadata, automatic Source Map follow-through, automatic hook inference / installation, and mobile full runtime chains remain separate follow-up work.

Tests cover reviewed function hook application, blocked pre-browser approval gates, workspace route aliasing, coordinator payload/category mapping, hook-review warning / blocker wiring, py_compile, and native-web / workspace / coordinator / hook regressions.

### Step 282：Source Map reviewed rebuild bundle generation MVP

Status: implemented as an explicit-review-only rebuild bundle generation surface after the Step 279 digest-only Source Map rebuild metadata application and the Step 281 selected hook executor application. This is not automatic Source Map follow-through, not raw-source-aware Source Map compilation, not Source Map fetching, not browser launch, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`native-web` exposes this through `source-map-rebuild-generation` / `source-map-rebuild-bundle-generation` / `source-map-rebuild-generation-result` / `source-map-selected-rebuild-generation` / `source-map-selected-rebuild-generation-executor` / `generate-reviewed-source-map-rebuild-bundle` / `run-reviewed-source-map-rebuild-generation`. The route requires `mode=apply`, `review_approved=true`, `approve_source_map_rebuild_generation=true`, a non-empty reviewer, a successful metadata-only `source_map_rebuild_result`, a local `artifact_root`, and reviewed `task_card` plus `final_result` JSON object inputs. Blocked inputs return `virtual://workspace/source-map-rebuild-generation-result.json` without creating directories, starting a browser, evaluating JavaScript, sending CDP, fetching Source Maps, calling MCP, or touching mobile runtimes.

When all gates pass, the route reuses the existing `write_rebuild_bundle(...)` generator and writes `workspace/rebuild-plan.json` plus generated rebuild files such as `rebuild/sign_rebuild.py`, `rebuild/replay_demo.py`, `rebuild/scrapy_middleware.py`, and the Scrapy export project under the reviewed `artifact_root`. It emits `virtual://workspace/source-map-rebuild-generation-result.json` with `reverse-deepagent.source-map-rebuild-generation-result.v1`, generated file metadata, strategy id, ready status, and fixed side-effect metadata: `browser_started=false`, `runtime_evaluated=false`, `cdp_command_sent=false`, `source_map_fetched=false`, `raw_source_content_exported=false`, `preview_exported=false`, `calls_mcp=false`, and `mobile_runtime_used=false`. The workspace contract indexes `workspace/source-map-rebuild-generation-result.json` with future alias `/workspace/rebuild/source-map-rebuild-generation-result.json` and category `audit`; coordinator extraction / artifact category mapping classify it as `audit`; hook review warns on successful generation so reviewers inspect generated rebuild artifacts before delivery and blocks failed / blocked generation results.

Boundary: this closes only the reviewed rebuild bundle generation bridge from Source Map metadata into the existing rebuild generator. It does not infer rebuild inputs automatically from Source Map source text, does not export raw source or previews, does not fetch Source Maps, does not launch a browser, does not call MCP, and does not publish or execute delivery. Automatic Source Map follow-through, complete binding / lexical scope resolution, raw-source-aware Source Map rebuild generation, automatic hook inference / installation, automatic debugger continuation, and mobile full runtime chains remain separate follow-up work.

Tests cover reviewed bundle generation, blocked pre-write approval gates, workspace route aliasing, coordinator payload/category mapping, hook-review warning / blocker wiring, compileall, and related rebuild / native-web / workspace / coordinator / hook regressions.

### Step 283：Source Map hook candidate refinement descriptor baseline

Status: implemented as a read-only / review-only hook candidate refinement descriptor after the Step 281 selected hook executor application MVP and Step 282 reviewed rebuild bundle generation MVP. This is not automatic hook installation, not automatic Source Map follow-through, not Source Map fetching, not browser launch, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`SourceMapHookCandidateRefinementManager` consumes a ready `bundler-symbol-scope` descriptor plus optional typed hook payload / module discovery hints / reviewed runtime function paths, and emits `reverse-deepagent.source-map-hook-candidates.v1` with function-symbol and module-export hook candidates, confidence signals, review blockers, and suggested explicit hook install inputs. Candidates are review packages only: `install_automatically=false`, `hook_installed=false`, and `install_supported_now=false` remain fixed until a later selected hook install review supplies concrete approved input.

`native-web` exposes this through `source-map-hook-candidates` / `source-map-hook-candidate-refinement` / `source-map-hook-candidate-review` / `source-map-selected-hook-candidates` / `refine-source-map-hook-candidates` / `review-source-map-hook-candidates` before browser session acquisition and returns `virtual://workspace/source-map-hook-candidates.json`. The workspace contract indexes `workspace/source-map-hook-candidates.json` with future alias `/workspace/hooks/source-map-hook-candidates.json` and category `triage`; coordinator extraction / artifact category mapping classify the optional evidence source as `triage`; hook review warns when candidates are ready and blocks failed / blocked candidate descriptors.

Boundary: this closes only the reviewed hook candidate refinement layer between Source Map symbol-scope metadata and selected hook install review. It does not install hooks, infer and execute hooks automatically, fetch Source Maps, export raw source or previews, start browsers, evaluate JavaScript, send CDP, call MCP, or touch mobile full runtime chains. Automatic Source Map follow-through, automatic hook installation without explicit review, complete binding / lexical scope resolution, raw-source-aware rebuild generation, automatic debugger continuation, and mobile full runtime chains remain separate follow-up work.

Tests cover manager-level candidate generation / blocked inputs, native-web no-browser route metadata, workspace route aliasing, coordinator payload/category mapping, hook-review warning / blocker wiring, compileall, and related source-map / native-web / workspace / coordinator / hook regressions.

### Step 284：Source Map debugger candidate review descriptor baseline

Status: implemented as a read-only / review-only debugger location candidate descriptor after the Step 280 selected debugger executor application MVP and Step 283 hook candidate refinement descriptor. This is not automatic debugger application, not automatic Source Map follow-through, not live callFrame recovery, not Source Map fetching, not browser launch, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`SourceMapDebuggerCandidateReviewManager` consumes ready Source Map lookup / `bundler-symbol-scope` evidence plus optional reviewed debugger location hints, and emits `reverse-deepagent.source-map-debugger-candidates.v1` with generated-location candidates, confidence signals, review blockers, and suggested explicit debugger location inputs. Candidates are review packages only: `apply_automatically=false`, `debugger_execution_performed=false`, `breakpoint_installed=false`, and `automatic_debugger_continuation=false` remain fixed until a later selected debugger apply review supplies concrete approved `source_map_debugger_location_input`.

`native-web` exposes this through `source-map-debugger-candidates` / `source-map-debugger-candidate-review` / `source-map-debugger-candidate-refinement` / `source-map-selected-debugger-candidates` / `rank-source-map-debugger-candidates` / `review-source-map-debugger-candidates` before browser session acquisition and returns `virtual://workspace/source-map-debugger-candidates.json`. The workspace contract indexes `workspace/source-map-debugger-candidates.json` with future alias `/workspace/debugger/source-map-debugger-candidates.json` and category `triage`; coordinator extraction / artifact category mapping classify the optional evidence source as `triage`; debugger review warns when candidates are ready and blocks failed / blocked candidate descriptors.

Boundary: this closes only the reviewed debugger candidate ranking layer between Source Map lookup / symbol-scope metadata and selected debugger apply review. It does not install breakpoints, infer and execute debugger actions automatically, fetch Source Maps, export raw source or previews, start browsers, evaluate JavaScript, send CDP, recover live callFrames, continue debugger execution, call MCP, or touch mobile full runtime chains. Automatic Source Map follow-through, automatic debugger continuation / live callFrame recovery, automatic debugger application without explicit review, complete binding / lexical scope resolution, raw-source-aware rebuild generation, and mobile full runtime chains remain separate follow-up work.

Tests cover manager-level ready / blocked candidate review, native-web metadata and no-side-effect verification, workspace route aliasing, coordinator payload/category mapping, debugger-subagent warning / blocker wiring, compileall, and related Source Map / native-web / workspace / coordinator / debugger regressions.

### Step 285：Source Map debugger candidate selection handoff descriptor baseline

Status: implemented as a read-only / review-only / plan-only / handoff-only debugger candidate selection descriptor after the Step 284 Source Map debugger candidate review descriptor. This is not selected executor input review invocation, not debugger application, not breakpoint installation, not automatic debugger continuation, not live callFrame recovery, not Source Map fetching, not browser launch, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`SourceMapDebuggerCandidateSelectionManager` consumes a ready `source-map-debugger-candidates` descriptor plus an explicit selected candidate id or index, verifies that the selected candidate is ready for debugger location review, and emits `reverse-deepagent.source-map-debugger-candidate-selection.v1` with the selected candidate, selected review metadata, a stable `selected_executor_input`, and `source_map_selected_executor_input_review_context` that can be handed to the existing `source-map-selected-executor-input-review` surface in a later reviewed step. It intentionally does not call that downstream manager automatically.

`native-web` exposes this through `source-map-debugger-candidate-selection` / `source-map-debugger-candidate-handoff` / `source-map-debugger-candidate-executor-input` / `source-map-selected-debugger-candidate` / `select-source-map-debugger-candidate` / `handoff-source-map-debugger-candidate` before the broader debugger-candidate review route so selection-specific requests are not swallowed by the source candidate descriptor. It returns `virtual://workspace/source-map-debugger-candidate-selection.json`. The workspace contract indexes `workspace/source-map-debugger-candidate-selection.json` with future alias `/workspace/debugger/source-map-debugger-candidate-selection.json` and category `triage`; coordinator extraction / artifact category mapping classify the optional evidence source as `triage`; debugger review warns when a selected candidate requires selected-executor-input review and blocks failed / ambiguous selection descriptors.

Boundary: this closes only the reviewed debugger candidate selection handoff layer between Source Map debugger candidate ranking and selected executor input review. It does not invoke selected executor review, install breakpoints, start browsers, evaluate JavaScript, send CDP, recover live callFrames, continue debugger execution, fetch Source Maps, export raw source or previews, call MCP, or touch mobile full runtime chains. Automatic Source Map follow-through, automatic debugger continuation / live callFrame recovery, automatic debugger application without explicit review, complete binding / lexical scope resolution, raw-source-aware rebuild generation, hook candidate selection helper consumption, and mobile full runtime chains remain separate follow-up work.

Tests cover manager-level ready / ambiguous selection, generated selected-executor-input-review context compatibility, native-web no-browser route metadata, route specificity before the broader debugger-candidate review route, workspace route aliasing, coordinator payload/category mapping, debugger-subagent warning / blocker wiring, compileall, and related Source Map / native-web / workspace / coordinator / debugger regressions.

### Step 286：Source Map debugger candidate selected-input review helper baseline

Status: implemented as a read-only / review-only / preflight-only / handoff-only helper after the Step 285 Source Map debugger candidate selection descriptor. This is not debugger application, not approval recording, not apply-preflight generation, not breakpoint installation, not automatic debugger continuation, not live callFrame recovery, not Source Map fetching, not browser launch, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`SourceMapSelectedExecutorInputReviewSpec.from_context(...)` now accepts a ready `source-map-debugger-candidate-selection` descriptor and reads its `source_map_selected_executor_input_review_context`. `SourceMapSelectedExecutorInputReviewManager` validates the candidate-selection source schema / status / ready flag / no-side-effect policy, then emits the existing canonical `reverse-deepagent.source-map-selected-executor-input-review.v1` descriptor with `source_debugger_candidate_selection_*` provenance fields, a stable downstream executor review package, and the usual explicit debugger review gate. This helper intentionally reuses the existing selected executor-input review artifact instead of introducing a duplicate workspace path.

`native-web` exposes this through `source-map-debugger-candidate-selected-input-review` / `source-map-debugger-candidate-executor-input-review` / `source-map-debugger-candidate-selected-executor-input-review` / `review-source-map-debugger-candidate-selected-input` protection names. These aliases return `virtual://workspace/source-map-selected-executor-input-review.json` before browser session acquisition, preserve the existing workspace contract route `workspace/source-map-selected-executor-input-review.json` with future alias `/workspace/debugger/source-map-selected-executor-input-review.json` and category `triage`, and include candidate-selection provenance in native-web verification / artifact metadata. The broader candidate-selection route explicitly skips these selected-input helper protection names so the handoff is not swallowed by the Step 285 selector route.

Boundary: this closes only the reviewed handoff from one selected Source Map debugger candidate into the existing selected executor-input review package. It does not invoke approval planning or apply preflight, install breakpoints, start browsers, evaluate JavaScript, send CDP, recover live callFrames, continue debugger execution, fetch Source Maps, export raw source or previews, call MCP, or touch mobile full runtime chains. Automatic Source Map follow-through beyond this packaging step, automatic debugger continuation / live callFrame recovery, automatic debugger application without explicit review, complete binding / lexical scope resolution, raw-source-aware rebuild generation, hook candidate selection helper consumption, and mobile full runtime chains remain separate follow-up work.

Tests cover manager-level candidate-selection handoff consumption, native-web helper aliases without browser start, route specificity against the Step 285 candidate-selection route, existing workspace route reuse, coordinator payload/category reuse, hook review behavior for `source-map-selected-executor-input-review`, compileall, and related Source Map / native-web / workspace / coordinator / hook regressions.
### Step 287：Source Map hook candidate selection handoff descriptor baseline

Status: implemented as a read-only / review-only / plan-only / handoff-only hook candidate selection descriptor after the Step 283 Source Map hook candidate refinement descriptor and the Step 286 debugger candidate selected-input review helper. This is not selected executor input review invocation, not hook installation, not automatic Source Map follow-through, not browser launch, not JavaScript evaluation, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`SourceMapHookCandidateSelectionManager` consumes a ready `source-map-hook-candidates` descriptor plus an explicit selected candidate id or index, verifies that the selected candidate is ready for hook install review, and emits `reverse-deepagent.source-map-hook-candidate-selection.v1` with the selected candidate, selected review metadata, a stable hook executor input containing `hook_symbol_scope` and `hook_install_input`, and `source_map_selected_executor_input_review_context` that can be handed to the existing `source-map-selected-executor-input-review` surface in a later reviewed step. It intentionally does not call that downstream manager automatically.

`native-web` exposes this through `source-map-hook-candidate-selection` / `source-map-hook-candidate-handoff` / `source-map-hook-candidate-executor-input` / `source-map-selected-hook-candidate` / `select-source-map-hook-candidate` / `handoff-source-map-hook-candidate` before the broader hook-candidate refinement route so selection-specific requests are not swallowed by the source candidate descriptor. It returns `virtual://workspace/source-map-hook-candidate-selection.json`. The workspace contract indexes `workspace/source-map-hook-candidate-selection.json` with future alias `/workspace/hooks/source-map-hook-candidate-selection.json` and category `triage`; coordinator extraction / artifact category mapping classify the optional evidence source as `triage`; hook review warns when a selected candidate requires selected-executor-input review and blocks failed / unsafe selection descriptors.

Boundary: this closes only the reviewed hook candidate selection handoff layer between Source Map hook candidate refinement and selected executor input review. It does not invoke selected executor review, install hooks, start browsers, evaluate JavaScript, send CDP, fetch Source Maps, export raw source or previews, call MCP, or touch mobile full runtime chains. Automatic Source Map follow-through beyond this packaging step, automatic hook installation without explicit review, complete binding / lexical scope resolution, raw-source-aware rebuild generation, automatic debugger continuation, and mobile full runtime chains remain separate follow-up work.

Tests cover manager-level ready / ambiguous selection, generated selected-executor-input-review context compatibility, native-web no-browser route metadata, route specificity before the broader hook-candidate refinement route, workspace route aliasing, coordinator payload/category mapping, hook-subagent warning / blocker wiring, compileall, and related Source Map / native-web / workspace / coordinator / hook regressions.
### Step 288：Source Map hook candidate selected-input review helper baseline

Status: implemented as a read-only / review-only / preflight-only / handoff-only helper after the Step 287 Source Map hook candidate selection descriptor. This is not hook installation, not approval recording, not apply-preflight generation, not automatic Source Map follow-through, not browser launch, not JavaScript evaluation, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`SourceMapSelectedExecutorInputReviewSpec.from_context(...)` now accepts a ready `source-map-hook-candidate-selection` descriptor and reads its `source_map_selected_executor_input_review_context`. `SourceMapSelectedExecutorInputReviewManager` validates the hook candidate-selection source schema / status / ready flag / no-side-effect policy, then emits the existing canonical `reverse-deepagent.source-map-selected-executor-input-review.v1` descriptor with `source_hook_candidate_selection_*` provenance fields, a stable downstream hook executor review package, and the usual explicit hook review gate. This helper intentionally reuses the existing selected executor-input review artifact instead of introducing a duplicate workspace path.

`native-web` exposes this through `source-map-hook-candidate-selected-input-review` / `source-map-hook-candidate-executor-input-review` / `source-map-hook-candidate-selected-executor-input-review` / `review-source-map-hook-candidate-selected-input` protection names. These aliases return `virtual://workspace/source-map-selected-executor-input-review.json` before browser session acquisition, preserve the existing workspace contract route `workspace/source-map-selected-executor-input-review.json` with future alias `/workspace/debugger/source-map-selected-executor-input-review.json` and category `triage`, and include hook candidate-selection provenance in native-web verification / artifact metadata. The broader hook-candidate selection / refinement routes explicitly skip these selected-input helper protection names so the handoff is not swallowed by candidate routing.

Boundary: this closes only the reviewed handoff from one selected Source Map hook candidate into the existing selected executor-input review package. It does not invoke approval planning or apply preflight, install hooks, start browsers, evaluate JavaScript, send CDP, fetch Source Maps, export raw source or previews, call MCP, or touch mobile full runtime chains. Automatic Source Map follow-through beyond this packaging step, automatic hook installation without explicit review, complete binding / lexical scope resolution, raw-source-aware rebuild generation, automatic debugger continuation, and mobile full runtime chains remain separate follow-up work.

Tests cover manager-level hook candidate-selection handoff consumption, native-web helper aliases without browser start, route specificity against the Step 287 candidate-selection and Step 283 hook-candidate refinement routes, existing workspace route reuse, coordinator payload/category reuse, hook review behavior for `source-map-selected-executor-input-review`, compileall, and related Source Map / native-web / workspace / coordinator / hook regressions.

### Step 289：Source Map follow-through chain readiness descriptor baseline

Status: implemented as a read-only / review-only / plan-only / orchestration-only descriptor after the Step 288 hook candidate selected-input review helper. This is not automatic Source Map follow-through, not debugger continuation, not hook or source-logpoint installation, not approval recording, not apply-preflight invocation, not Source Map fetching, not browser launch, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`SourceMapFollowthroughChainReadinessManager` consumes any reviewed subset of Source Map readiness, consumer action plan, consumer materialization, typed payload preflight, followthrough review, surface selection, selected executor input review, approval plan / record, apply preflight, debugger / hook candidate descriptors, and selected executor result artifacts. It emits `reverse-deepagent.source-map-followthrough-chain-readiness.v1` with the furthest proven stage, next missing / required artifact, selected consumer, candidate / result summaries, explicit next review action, fixed `automatic_followthrough_supported=false`, and no-side-effect policy.

`native-web` exposes this through `source-map-followthrough-chain-readiness` / `source-map-followthrough-chain-review` / `source-map-followthrough-status` / `source-map-chain-readiness` / `review-source-map-followthrough-chain` before browser session acquisition and returns `virtual://workspace/source-map-followthrough-chain-readiness.json`. The workspace contract indexes `workspace/source-map-followthrough-chain-readiness.json` with future alias `/workspace/debugger/source-map-followthrough-chain-readiness.json` and category `triage`; coordinator extraction / artifact category mapping classify the optional evidence source as `triage`; hook and debugger review warn when ready and block failed descriptors.

Boundary: this closes only the chain status normalization layer that helps the agent decide the next explicit review step. It does not invoke selected executor input review, record approval, run apply preflight, execute debugger commands, recover live callFrames, install source-logpoints or hooks, run rebuild generation, fetch Source Maps, export raw source or previews, start browsers, evaluate JavaScript, send CDP, call MCP, or touch mobile full runtime chains. Automatic Source Map follow-through beyond this reviewed chain readiness descriptor, complete binding / lexical scope resolution, raw-source-aware rebuild generation, automatic debugger continuation, and mobile full runtime chains remain separate follow-up work.

Tests cover manager-level furthest-stage / blocker behavior, native-web protection without browser start, workspace contract, coordinator payload/category, hook/debugger review warning and blocker behavior, compileall, and related Source Map / native-web / workspace / coordinator / subagent regressions.

### Step 290：Source Map reviewed one-step follow-through orchestrator plan

Status: implemented as a read-only / review-only / plan-only / orchestration-only / handoff-only descriptor after the Step 289 Source Map follow-through chain readiness descriptor. This is not automatic Source Map follow-through, not selected executor invocation, not approval recording, not apply-preflight invocation, not debugger continuation, not hook or source-logpoint installation, not rebuild generation, not Source Map fetching, not browser launch, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`SourceMapFollowthroughOneStepPlanManager` consumes a ready `reverse-deepagent.source-map-followthrough-chain-readiness.v1` descriptor plus optional expected consumer / next-action guards. It emits `reverse-deepagent.source-map-followthrough-one-step-plan.v1` with `planned_step` metadata using `reverse-deepagent.source-map-followthrough-one-step.v1`, source chain provenance, selected consumer, next required artifact, explicit next review action, fixed `will_invoke_next_action=false`, fixed `will_record_approval=false`, fixed `will_run_apply_preflight=false`, fixed runtime execution flags false, `automatic_followthrough_supported=false`, `automatic_execution_supported=false`, and no-side-effect policy.

`native-web` exposes this through `source-map-followthrough-one-step-plan` / `source-map-followthrough-orchestrator-plan` / `source-map-followthrough-next-step-plan` / `review-source-map-followthrough-one-step-plan` / `plan-source-map-followthrough-next-step` before browser session acquisition and returns `virtual://workspace/source-map-followthrough-one-step-plan.json`. The workspace contract indexes `workspace/source-map-followthrough-one-step-plan.json` with future alias `/workspace/debugger/source-map-followthrough-one-step-plan.json` and category `triage`; coordinator extraction / artifact category mapping classify the optional evidence source as `triage`; hook and debugger review warn when ready and block failed descriptors.

Boundary: this closes only the one-step planning layer that packages the next explicit Source Map follow-through review action. It does not invoke selected executor input review, record approval, run apply preflight, execute debugger commands, recover live callFrames, install source-logpoints or hooks, run rebuild generation, fetch Source Maps, export raw source or previews, start browsers, evaluate JavaScript, send CDP, call MCP, or touch mobile full runtime chains. Automatic Source Map follow-through beyond this reviewed one-step plan, complete binding / lexical scope resolution, raw-source-aware rebuild generation, automatic debugger continuation, and mobile full runtime chains remain separate follow-up work.

Tests cover manager-level ready / blocked behavior, native-web protection without browser start, workspace contract, coordinator payload/category, hook/debugger review warning and blocker behavior, compileall, and related Source Map / native-web / workspace / coordinator / subagent regressions.

### Step 291：Source Map follow-through dispatch preflight descriptor baseline

Status: implemented as a read-only / review-only / preflight-only / plan-only / orchestration-only descriptor after the Step 290 one-step follow-through plan. This is not a dispatcher, not automatic Source Map follow-through, not selected executor invocation, not approval recording, not apply-preflight invocation, not debugger continuation, not hook or source-logpoint installation, not rebuild generation, not Source Map fetching, not browser launch, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`SourceMapFollowthroughDispatchPreflightManager` consumes a ready `reverse-deepagent.source-map-followthrough-one-step-plan.v1` descriptor plus optional expected consumer / next-action / result-artifact guards. It emits `reverse-deepagent.source-map-followthrough-dispatch-preflight.v1` with dispatch target allowlist metadata, `reverse-deepagent.source-map-followthrough-dispatch-input.v1` dispatcher input metadata, selected consumer, planned next action, planned required artifact, fixed `will_invoke_dispatch_target=false`, fixed `will_invoke_next_action=false`, fixed approval / apply-preflight / runtime execution flags false, `automatic_dispatch_supported=false`, `automatic_followthrough_supported=false`, `automatic_execution_supported=false`, and no-side-effect policy.

`native-web` exposes this through `source-map-followthrough-dispatch-preflight` / `source-map-followthrough-dispatch-review` / `source-map-followthrough-executor-dispatch-preflight` / `review-source-map-followthrough-dispatch-preflight` / `preflight-source-map-followthrough-dispatch` before browser session acquisition and returns `virtual://workspace/source-map-followthrough-dispatch-preflight.json`. The workspace contract indexes `workspace/source-map-followthrough-dispatch-preflight.json` with future alias `/workspace/debugger/source-map-followthrough-dispatch-preflight.json` and category `triage`; coordinator extraction / artifact category mapping classify the optional evidence source as `triage`; hook and debugger review warn when ready and block failed descriptors.

Boundary: this closes only the dispatch-preflight layer that checks whether the one-step Source Map plan targets a known explicit review / executor surface. It does not invoke selected executor input review, record approval, run apply preflight, execute debugger commands, recover live callFrames, install source-logpoints or hooks, run rebuild generation, fetch Source Maps, export raw source or previews, start browsers, evaluate JavaScript, send CDP, call MCP, or touch mobile full runtime chains. Automatic Source Map follow-through beyond this reviewed dispatch preflight, complete binding / lexical scope resolution, raw-source-aware rebuild generation, automatic debugger continuation, and mobile full runtime chains remain separate follow-up work.

Tests cover manager-level ready / blocked behavior, native-web protection without browser start, workspace contract, coordinator payload/category, hook/debugger review warning and blocker behavior, compileall, and related Source Map / native-web / workspace / coordinator / subagent regressions.

### Step 292：Source Map follow-through dispatch approval / transaction plan descriptor baseline

Status: implemented as a read-only / review-only / approval-plan-only / transaction-plan-only descriptor after the Step 291 dispatch preflight. This is not an approval record writer, not a transaction journal writer, not a dispatcher, not automatic Source Map follow-through, not selected executor invocation, not apply-preflight invocation, not debugger continuation, not hook or source-logpoint installation, not rebuild generation, not Source Map fetching, not browser launch, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`SourceMapFollowthroughDispatchApprovalPlanManager` consumes a ready `reverse-deepagent.source-map-followthrough-dispatch-preflight.v1` descriptor plus optional expected consumer / dispatch-surface / result-artifact guards. It emits `reverse-deepagent.source-map-followthrough-dispatch-approval-plan.v1` with `reverse-deepagent.source-map-followthrough-dispatch-approval.v1` approval-plan metadata, `reverse-deepagent.source-map-followthrough-dispatch-transaction-plan.v1` transaction-plan metadata, selected consumer, dispatch surface, required result artifact, fixed `ready_to_dispatch_now=false`, fixed `approval_recorded=false`, fixed `transaction_started=false`, fixed `journal_written=false`, fixed `will_write_approval_record=false`, fixed `will_start_transaction=false`, fixed dispatch / runtime execution flags false, `automatic_dispatch_supported=false`, `automatic_followthrough_supported=false`, `automatic_execution_supported=false`, and no-side-effect policy.

`native-web` exposes this through `source-map-followthrough-dispatch-approval-plan` / `source-map-followthrough-executor-approval-plan` / `source-map-followthrough-dispatch-transaction-plan` / `review-source-map-followthrough-dispatch-approval-plan` / `plan-source-map-followthrough-dispatch-approval` before browser session acquisition and returns `virtual://workspace/source-map-followthrough-dispatch-approval-plan.json`. The workspace contract indexes `workspace/source-map-followthrough-dispatch-approval-plan.json` with future alias `/workspace/debugger/source-map-followthrough-dispatch-approval-plan.json` and category `triage`; coordinator extraction / artifact category mapping classify the optional evidence source as `triage`; hook and debugger review warn when ready and block failed descriptors.

Boundary: this closes only the approval / transaction planning layer before a future explicit approval-record writer. It does not write approval artifacts, start a transaction, write a journal, invoke selected executor input review, run apply preflight, execute debugger commands, recover live callFrames, install source-logpoints or hooks, run rebuild generation, fetch Source Maps, export raw source or previews, start browsers, evaluate JavaScript, send CDP, call MCP, or touch mobile full runtime chains. Automatic Source Map follow-through beyond this reviewed approval / transaction plan, complete binding / lexical scope resolution, raw-source-aware rebuild generation, automatic debugger continuation, and mobile full runtime chains remain separate follow-up work.

Tests cover manager-level ready / blocked behavior, native-web protection without browser start, workspace contract, coordinator payload/category, hook/debugger review warning and blocker behavior, compileall, and related Source Map / native-web / workspace / coordinator / subagent regressions.

### Step 293：Source Map follow-through dispatch approval-record writer baseline

Status: implemented as an explicit-review-only approval-record writer after the Step 292 dispatch approval / transaction plan descriptor. This is not a transaction preflight, not a transaction journal writer, not a dispatcher, not automatic Source Map follow-through, not selected executor invocation, not apply-preflight invocation, not debugger continuation, not hook or source-logpoint installation, not rebuild generation, not Source Map fetching, not browser launch, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`record_source_map_followthrough_dispatch_approval` consumes a ready `reverse-deepagent.source-map-followthrough-dispatch-approval-plan.v1` descriptor or artifact ref and writes `workspace/source-map-followthrough-dispatch-approval-record.json` only under explicit apply gates (`mode=apply`, `write_result=true`, `approve_approval_record=true`, reviewer, and optional approval-plan id / consumer / dispatch-surface / required-artifact / digest guards). The written artifact uses `reverse-deepagent.source-map-followthrough-dispatch-approval-record.v1`, records selected consumer, dispatch surface, required result artifact, approval-plan digest, transaction-plan summary, fixed `ready_to_dispatch_now=false`, fixed `transaction_started=false`, fixed `journal_written=false`, fixed `dispatch_target_invoked=false`, fixed runtime execution flags false, and no-side-effect policy except the approval artifact write itself.

The hook subagent exposes `record_source_map_followthrough_dispatch_approval` alongside the existing selected-executor approval writer. The workspace contract indexes `workspace/source-map-followthrough-dispatch-approval-record.json` with future alias `/workspace/debugger/source-map-followthrough-dispatch-approval-record.json` and category `audit`; coordinator extraction / artifact category mapping classify the optional evidence source as `audit`; hook and debugger review warn when a written approved record is ready for transaction-preflight follow-up and block failed / blocked records.

Boundary: this closes only the approval-record layer before a future transaction preflight / journal writer. It does not start a transaction, write a journal, invoke selected executor input review, run apply preflight, call a dispatch target, execute debugger commands, recover live callFrames, install source-logpoints or hooks, run rebuild generation, fetch Source Maps, export raw source or previews, start browsers, evaluate JavaScript, send CDP, call MCP, or touch mobile full runtime chains. Automatic Source Map follow-through beyond this reviewed approval record, transaction journal writing, explicit dispatcher handoff, complete binding / lexical scope resolution, raw-source-aware rebuild generation, automatic debugger continuation, and mobile full runtime chains remain separate follow-up work.

Tests cover explicit approval-record writer dry-run / apply gates / apply-write-only behavior, hook subagent tool exposure and prompt guidance, hook/debugger review warning and blocker behavior, workspace contract, coordinator payload/category, compileall, and related subagent regressions.

### Step 294：Source Map follow-through dispatch transaction preflight descriptor baseline

Status: implemented as a read-only / review-only / transaction-preflight-only descriptor after the Step 293 dispatch approval-record writer. This is not a transaction journal writer, not a dispatcher, not automatic Source Map follow-through, not selected executor invocation, not debugger continuation, not hook or source-logpoint installation, not rebuild generation, not Source Map fetching, not browser launch, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`SourceMapFollowthroughDispatchTransactionPreflightManager` consumes a ready `reverse-deepagent.source-map-followthrough-dispatch-approval-plan.v1` descriptor plus a written approved `reverse-deepagent.source-map-followthrough-dispatch-approval-record.v1` record. It verifies the approval-plan id, approval-record id, selected consumer, dispatch surface, required result artifact, approval-plan digest, transaction-plan id, approval decision, no prior transaction start / journal write, and no dispatch / executor side-effect claims. It emits `reverse-deepagent.source-map-followthrough-dispatch-transaction-preflight.v1` with a transaction preflight gate and a journal-writer gate while keeping `ready_to_write_now=false`, `ready_to_dispatch_now=false`, `transaction_started=false`, `journal_written=false`, `will_write_transaction_journal=false`, and all runtime execution flags false.

`native-web` exposes this through `source-map-followthrough-dispatch-transaction-preflight` / `source-map-followthrough-dispatch-journal-preflight` / `source-map-followthrough-dispatch-transaction-gate` / `review-source-map-followthrough-dispatch-transaction-preflight` / `preflight-source-map-followthrough-dispatch-transaction` protection names before browser session acquisition and returns `virtual://workspace/source-map-followthrough-dispatch-transaction-preflight.json`. The workspace contract indexes `workspace/source-map-followthrough-dispatch-transaction-preflight.json` with future alias `/workspace/debugger/source-map-followthrough-dispatch-transaction-preflight.json` and category `audit`; coordinator extraction / artifact category mapping classify the optional evidence source as `audit`; hook and debugger review warn when a ready transaction preflight is ready for journal-writer follow-up and block failed / blocked preflight descriptors.

Boundary: this closes only the read-only transaction-preflight layer before a future explicit transaction journal writer. It does not write a journal, start a transaction, invoke selected executor input review, run apply preflight, call a dispatch target, execute debugger commands, recover live callFrames, install source-logpoints or hooks, run rebuild generation, fetch Source Maps, export raw source or previews, start browsers, evaluate JavaScript, send CDP, call MCP, or touch mobile full runtime chains. Automatic Source Map follow-through beyond this reviewed transaction preflight, transaction journal writing, bounded dispatch gating, explicit dispatcher handoff, complete binding / lexical scope resolution, raw-source-aware rebuild generation, automatic debugger continuation, and mobile full runtime chains remain separate follow-up work.

Tests cover manager-level ready / mismatch blockers, native-web no-browser route metadata, hook/debugger review warning and blocker behavior, workspace contract, coordinator payload/category, compileall, and related Source Map / native-web / workspace / coordinator / subagent regressions.

### Step 295：Source Map follow-through dispatch transaction journal writer baseline

Status: implemented as an explicit-review-only transaction journal writer after the Step 294 dispatch transaction preflight descriptor. This is not a bounded dispatch gate, not a dispatcher, not automatic Source Map follow-through, not selected executor invocation, not debugger continuation, not hook or source-logpoint installation, not rebuild generation, not Source Map fetching, not browser launch, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`record_source_map_followthrough_dispatch_transaction_journal` consumes a ready `reverse-deepagent.source-map-followthrough-dispatch-transaction-preflight.v1` descriptor or artifact ref and writes `workspace/source-map-followthrough-dispatch-transaction-journal.json` only under explicit apply gates (`mode=apply`, `write_result=true`, `approve_transaction_journal=true`, reviewer, and optional transaction-preflight / approval-record / transaction-plan / approval-plan / consumer / dispatch-surface / required-artifact / digest guards). The written artifact uses `reverse-deepagent.source-map-followthrough-dispatch-transaction-journal.v1`, records selected consumer, dispatch surface, required result artifact, transaction-preflight digest, journal entries, fixed `ready_to_dispatch_now=false`, fixed `dispatch_target_invoked=false`, fixed `executor_invoked=false`, fixed runtime execution flags false, and no-side-effect policy except the journal artifact write itself.

The hook subagent exposes `record_source_map_followthrough_dispatch_transaction_journal` alongside the existing Source Map selected-executor and follow-through dispatch approval writers. The workspace contract indexes `workspace/source-map-followthrough-dispatch-transaction-journal.json` with future alias `/workspace/debugger/source-map-followthrough-dispatch-transaction-journal.json` and category `audit`; coordinator extraction / artifact category mapping classify the optional evidence source as `audit`; hook and debugger review warn when a written journal is ready for bounded dispatch gate follow-up and block failed / blocked journal descriptors.

Boundary: this closes only the explicit transaction-journal writer layer before a future bounded dispatch gate. It writes a journal audit artifact but does not call a dispatch target, invoke selected executor input review, run apply preflight, execute debugger commands, recover live callFrames, install source-logpoints or hooks, run rebuild generation, fetch Source Maps, export raw source or previews, start browsers, evaluate JavaScript, send CDP, call MCP, or touch mobile full runtime chains. Automatic Source Map follow-through beyond this reviewed journal write, bounded dispatch gating, explicit dispatcher handoff, complete binding / lexical scope resolution, raw-source-aware rebuild generation, automatic debugger continuation, and mobile full runtime chains remain separate follow-up work.

Tests cover explicit transaction journal dry-run / apply gates / apply-write-only behavior, hook subagent tool exposure and prompt guidance, hook/debugger review warning and blocker behavior, workspace contract, coordinator payload/category, compileall, and related subagent regressions.

### Step 296：Source Map follow-through dispatch bounded executor gate descriptor baseline

Status: implemented as a read-only / review-only / bounded-gate-only descriptor after the Step 295 dispatch transaction journal writer. This is not a dispatcher, not automatic Source Map follow-through, not selected executor invocation, not apply-preflight invocation, not debugger continuation, not hook or source-logpoint installation, not rebuild generation, not Source Map fetching, not browser launch, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`SourceMapFollowthroughDispatchBoundedExecutorGateManager` consumes a written `reverse-deepagent.source-map-followthrough-dispatch-transaction-journal.v1` descriptor, verifies journal id / transaction-preflight id / approval-record id / transaction-plan id / approval-plan id / selected consumer / dispatch surface / required artifact / digest guards, and emits `reverse-deepagent.source-map-followthrough-dispatch-bounded-executor-gate.v1`. The descriptor fixes `ready_to_dispatch_now=false`, `ready_to_execute_now=false`, `dispatch_target_invoked=false`, `executor_invoked=false`, `will_run_apply_preflight=false`, all runtime execution flags false, and exposes `bounded_dispatch_input` plus a future `reverse-deepagent.source-map-followthrough-dispatcher-contract.v1` with `implemented=false`.

`native-web` exposes this through `source-map-followthrough-dispatch-bounded-executor-gate` / `source-map-followthrough-dispatch-bounded-gate` / `source-map-followthrough-dispatch-executor-gate` / `review-source-map-followthrough-dispatch-bounded-executor-gate` / `gate-source-map-followthrough-dispatch-executor` before browser session acquisition and returns `virtual://workspace/source-map-followthrough-dispatch-bounded-executor-gate.json`. The workspace contract indexes `workspace/source-map-followthrough-dispatch-bounded-executor-gate.json` with future alias `/workspace/debugger/source-map-followthrough-dispatch-bounded-executor-gate.json` and category `audit`; coordinator extraction / artifact category mapping classify the optional evidence source as `audit`; hook and debugger review warn when a ready bounded gate is ready for dispatcher-handoff follow-up and block failed / blocked gate descriptors.

Boundary: this closes only the read-only bounded dispatch gate between a reviewed transaction journal and a future dispatcher handoff. It does not call a dispatch target, invoke selected executor input review, run apply preflight, execute debugger commands, recover live callFrames, install source-logpoints or hooks, run rebuild generation, fetch Source Maps, export raw source or previews, start browsers, evaluate JavaScript, send CDP, call MCP, or touch mobile full runtime chains. Automatic Source Map follow-through beyond this reviewed bounded gate, explicit dispatcher handoff, complete binding / lexical scope resolution, raw-source-aware rebuild generation, automatic debugger continuation, and mobile full runtime chains remain separate follow-up work.

Tests cover manager-level ready / bad-journal blockers, native-web no-browser route metadata, hook/debugger review warning and blocker behavior, workspace contract, coordinator payload/category, compileall, and related Source Map / native-web / workspace / coordinator / subagent regressions.

### Step 297：Source Map follow-through dispatcher handoff descriptor baseline

Status: implemented as a read-only / review-only / dispatcher-handoff-only descriptor after the Step 296 dispatch bounded executor gate. This is not a dispatcher, not automatic Source Map follow-through, not selected executor invocation, not apply-preflight invocation, not debugger continuation, not hook or source-logpoint installation, not rebuild generation, not Source Map fetching, not browser launch, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`SourceMapFollowthroughDispatcherHandoffManager` consumes a ready `reverse-deepagent.source-map-followthrough-dispatch-bounded-executor-gate.v1` descriptor, verifies bounded-gate readiness, transaction / approval ids, selected consumer, dispatch surface, required artifact, future dispatcher contract, digest guards, no prior dispatch / executor side effects, and emits `reverse-deepagent.source-map-followthrough-dispatcher-handoff.v1`. The descriptor fixes `ready_to_dispatch_now=false`, `ready_to_execute_now=false`, `dispatcher_invoked=false`, `dispatch_target_invoked=false`, `executor_invoked=false`, `apply_preflight_invoked=false`, and exposes a `dispatcher_handoff` payload plus a selected-executor review contract for the next explicit apply-preflight review.

`native-web` exposes this through `source-map-followthrough-dispatcher-handoff` / `source-map-followthrough-dispatch-handoff` / `source-map-followthrough-next-action-handoff` / `review-source-map-followthrough-dispatcher-handoff` / `handoff-source-map-followthrough-dispatcher` before browser session acquisition and returns `virtual://workspace/source-map-followthrough-dispatcher-handoff.json`. The workspace contract indexes `workspace/source-map-followthrough-dispatcher-handoff.json` with future alias `/workspace/debugger/source-map-followthrough-dispatcher-handoff.json` and category `audit`; coordinator extraction / artifact category mapping classify the optional evidence source as `audit`; hook and debugger review warn when a ready dispatcher handoff is ready for dispatcher apply-preflight review and block failed / blocked handoff descriptors.

Boundary: this closes only the read-only handoff between a reviewed bounded dispatch gate and a future explicit dispatcher apply-preflight / executor review. It does not call a dispatcher, call a dispatch target, invoke selected executor input review, run apply preflight, execute debugger commands, recover live callFrames, install source-logpoints or hooks, run rebuild generation, fetch Source Maps, export raw source or previews, start browsers, evaluate JavaScript, send CDP, call MCP, or touch mobile full runtime chains. Automatic Source Map follow-through beyond this reviewed handoff, explicit dispatcher apply-preflight / dispatcher MVP, complete binding / lexical scope resolution, raw-source-aware rebuild generation, automatic debugger continuation, and mobile full runtime chains remain separate follow-up work.

Tests cover manager-level ready / bad-gate blockers, native-web no-browser route metadata, hook/debugger review warning and blocker behavior, workspace contract, coordinator payload/category, compileall, and related Source Map / native-web / workspace / coordinator / subagent regressions.

### Step 298：Source Map follow-through dispatcher apply-preflight descriptor baseline

Status: implemented as a read-only / review-only / dispatcher-apply-preflight-only descriptor after the Step 297 dispatcher handoff descriptor. This is not a dispatcher MVP, not automatic Source Map follow-through, not selected executor invocation, not selected-executor apply-preflight invocation, not debugger continuation, not hook or source-logpoint installation, not rebuild generation, not Source Map fetching, not browser launch, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`SourceMapFollowthroughDispatcherApplyPreflightManager` consumes a ready `reverse-deepagent.source-map-followthrough-dispatcher-handoff.v1` descriptor, verifies handoff readiness, transaction / approval ids, selected consumer, dispatch surface, required artifact, dispatcher handoff payload, selected-executor review contract, digest guards, no prior dispatcher / dispatch-target / executor / apply-preflight side effects, and emits `reverse-deepagent.source-map-followthrough-dispatcher-apply-preflight.v1`. The descriptor fixes `ready_to_dispatch_now=false`, `ready_to_execute_now=false`, `dispatcher_invoked=false`, `dispatch_target_invoked=false`, `executor_invoked=false`, `selected_executor_apply_preflight_invoked=false`, `runtime_apply_preflight_invoked=false`, and exposes a `dispatcher_apply_preflight` payload plus a future dispatcher MVP contract with `implemented=false`.

`native-web` exposes this through `source-map-followthrough-dispatcher-apply-preflight` / `source-map-followthrough-dispatch-apply-preflight` / `source-map-followthrough-dispatcher-preflight` / `review-source-map-followthrough-dispatcher-apply-preflight` / `preflight-source-map-followthrough-dispatcher` before browser session acquisition and returns `virtual://workspace/source-map-followthrough-dispatcher-apply-preflight.json`. The workspace contract indexes `workspace/source-map-followthrough-dispatcher-apply-preflight.json` with future alias `/workspace/debugger/source-map-followthrough-dispatcher-apply-preflight.json` and category `audit`; coordinator extraction / artifact category mapping classify the optional evidence source as `audit`; hook and debugger review warn when a ready dispatcher apply-preflight is ready for explicit dispatcher MVP review and block failed / blocked preflight descriptors.

Boundary: this closes only the read-only preflight between a reviewed dispatcher handoff and a future explicit dispatcher MVP. It does not call a dispatcher, call a dispatch target, invoke selected executor input review, run selected-executor apply preflight, execute debugger commands, recover live callFrames, install source-logpoints or hooks, run rebuild generation, fetch Source Maps, export raw source or previews, start browsers, evaluate JavaScript, send CDP, call MCP, or touch mobile full runtime chains. Automatic Source Map follow-through beyond this reviewed preflight, explicit dispatcher MVP execution, complete binding / lexical scope resolution, raw-source-aware rebuild generation, automatic debugger continuation, and mobile full runtime chains remain separate follow-up work.

Tests cover manager-level ready / bad-handoff blockers, native-web no-browser route metadata, hook/debugger review warning and blocker behavior, workspace contract, coordinator payload/category, compileall, and related Source Map / native-web / workspace / coordinator / subagent regressions.

### Step 299：Explicit-review-only Source Map follow-through dispatcher result MVP

Status: implemented as an explicit-review-only dispatcher MVP decision record after the Step 298 dispatcher apply-preflight descriptor. This is not automatic Source Map follow-through, not selected executor invocation, not selected-executor apply-preflight invocation, not debugger continuation, not hook or source-logpoint installation, not rebuild generation, not Source Map fetching, not browser launch, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`SourceMapFollowthroughDispatcherManager` consumes a ready `reverse-deepagent.source-map-followthrough-dispatcher-apply-preflight.v1` descriptor, verifies dispatcher apply-preflight readiness, transaction / approval ids, selected consumer, dispatch surface, required artifact, future dispatcher MVP contract, selected-executor apply-preflight requirement, digest guards, and absence of prior dispatcher / dispatch-target / executor / apply-preflight side effects. Only `mode=apply`, `write_result=true`, `review_approved=true`, `approve_dispatcher_mvp=true`, and a non-empty reviewer record `reverse-deepagent.source-map-followthrough-dispatcher-result.v1` with `dispatcher_decision_recorded=true`.

`native-web` exposes this through `source-map-followthrough-dispatcher-result` / `source-map-followthrough-dispatcher-mvp` / `source-map-followthrough-dispatch-next-action` / `execute-source-map-followthrough-dispatcher-mvp` / `review-source-map-followthrough-dispatcher-mvp` before browser session acquisition and returns `virtual://workspace/source-map-followthrough-dispatcher-result.json`. The workspace contract indexes `workspace/source-map-followthrough-dispatcher-result.json` with future alias `/workspace/debugger/source-map-followthrough-dispatcher-result.json` and category `audit`; coordinator extraction / artifact category mapping classify the optional evidence source as `audit`; hook and debugger review warn when a dispatched decision is ready for selected executor apply-preflight review and block failed / blocked result descriptors.

Boundary: this closes only the explicit reviewed dispatcher decision layer. It records the dispatcher MVP result audit artifact but deliberately keeps `dispatcher_invoked=false`, `dispatch_target_invoked=false`, `executor_invoked=false`, `selected_executor_invoked=false`, `selected_executor_apply_preflight_invoked=false`, `ready_to_execute_selected_executor_now=false`, `browser_started=false`, `cdp_command_sent=false`, `runtime_evaluated=false`, `calls_mcp=false`, and `mobile_runtime_used=false`. Follow-up selected executor apply-preflight and selected executor execution remain separate explicit review-gated work.

### Step 300：Source Map dispatcher-result to selected-executor apply-preflight handoff audit

Status: implemented as an optional read-only / review-only handoff audit inside the existing Step 277 `source-map-selected-executor-apply-preflight` descriptor after the Step 299 dispatcher result MVP. This is not selected executor execution, not automatic Source Map follow-through, not debugger continuation, not hook or source-logpoint installation, not rebuild generation, not Source Map fetching, not browser launch, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`SourceMapSelectedExecutorApplyPreflightManager` may now consume an optional `reverse-deepagent.source-map-followthrough-dispatcher-result.v1` descriptor alongside the selected executor approval plan and approval record. When provided, it verifies the dispatcher result schema / dispatched status / decision record, selected consumer, required result artifact, optional id / digest guards, `requires_selected_executor_apply_preflight=true`, and absence of dispatcher / dispatch-target / selected-executor / selected-executor-apply-preflight / runtime side effects. The resulting `reverse-deepagent.source-map-selected-executor-apply-preflight.v1` descriptor records `dispatcher_result_verified`, `dispatcher_decision_recorded`, dispatcher-result digest / id metadata, and a `reverse-deepagent.source-map-selected-executor-apply-preflight-dispatcher-result-handoff.v1` summary while keeping `ready_to_apply_now=false` and `future_executor_contract.implemented=false`.

`native-web` keeps the selected executor apply-preflight route before any browser session acquisition and now exposes dispatcher-result handoff verification metadata on `virtual://workspace/source-map-selected-executor-apply-preflight.json`. The dispatcher-result route no longer steals explicit selected-executor apply-preflight requests that carry dispatcher-result evidence as context.

Boundary: this closes only the optional audit handoff from a reviewed dispatcher decision into the selected executor apply-preflight descriptor. It does not invoke the selected executor, run the selected-executor apply target, call a dispatch target, execute debugger commands, recover live callFrames, install source-logpoints or hooks, run rebuild generation, fetch Source Maps, export raw source or previews, start browsers, evaluate JavaScript, send CDP, call MCP, or touch mobile full runtime chains. Selected executor execution and automatic Source Map follow-through remain separate explicit review-gated follow-ups.

Tests cover manager-level dispatcher-result handoff success / blocker behavior, native-web route metadata without browser start, compileall, and related Source Map / native-web regressions.

### Step 301：Source Map selected-executor application handoff descriptor baseline

Status: implemented as a read-only / review-only / plan-only selected-executor application handoff descriptor after the Step 300 dispatcher-result to selected-executor apply-preflight handoff audit. This is not selected executor execution, not automatic Source Map follow-through, not debugger continuation, not hook or source-logpoint installation, not rebuild generation, not Source Map fetching, not browser launch, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`SourceMapSelectedExecutorApplicationHandoffManager` consumes a ready `reverse-deepagent.source-map-selected-executor-apply-preflight.v1` descriptor and emits `reverse-deepagent.source-map-selected-executor-application-handoff.v1`. It verifies the apply-preflight schema / status, selected consumer, selected review gate, approval-record verification, executor input readiness, optional action / consumer / digest guards, and no prior execution claims. The descriptor maps the ready selected executor into an explicit application surface (`source-map-debugger-application`, `source-map-source-logpoint-application`, `source-map-hook-application`, or `source-map-rebuild-metadata-application`), required approval flags, application input key, future result artifact, and a `reverse-deepagent.source-map-selected-executor-application-review-input.v1` review package while keeping `ready_to_execute_now=false`, `surface_executor_invoked=false`, `application_invoked=false`, and all runtime side-effect flags false.

`native-web` exposes this through `source-map-selected-executor-application-handoff` / `source-map-selected-executor-application-review-input` / `source-map-followthrough-application-handoff` before browser session acquisition and returns `virtual://workspace/source-map-selected-executor-application-handoff.json`. The workspace contract indexes `workspace/source-map-selected-executor-application-handoff.json` with future alias `/workspace/debugger/source-map-selected-executor-application-handoff.json` and category `audit`; coordinator extraction / artifact category mapping classify the optional evidence source as `audit`; hook and debugger review warn when a ready handoff is ready for selected executor application review and block failed / blocked handoff descriptors.

Boundary: this closes only the review-input handoff between selected-executor apply-preflight and existing explicit application routes. It does not call those routes, invoke the selected executor, execute debugger commands, recover live callFrames, install source-logpoints or hooks, run rebuild generation, fetch Source Maps, export raw source or previews, start browsers, evaluate JavaScript, send CDP, call MCP, or touch mobile full runtime chains. Selected executor execution and automatic Source Map follow-through remain separate explicit review-gated follow-ups.

Tests cover manager-level ready / blocked handoff behavior, native-web no-browser route metadata, workspace contract, coordinator payload/category, hook/debugger review warning and blocker behavior, compileall, and related Source Map / native-web / workspace / coordinator / subagent regressions.

### Step 302：Source Map selected-executor application result checkpoint baseline

Status: implemented as a read-only / review-only / checkpoint-only descriptor after the Step 301 selected-executor application handoff. This is not selected executor execution, not automatic Source Map follow-through, not debugger continuation, not hook or source-logpoint installation, not rebuild generation, not Source Map fetching, not browser launch, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`SourceMapSelectedExecutorResultCheckpointManager` consumes one explicit selected executor application result (`workspace/source-map-debugger-execution-result.json`, `workspace/source-map-source-logpoint-install-result.json`, `workspace/source-map-hook-install-result.json`, or `workspace/source-map-rebuild-result.json`) plus optional `reverse-deepagent.source-map-selected-executor-application-handoff.v1` evidence. It emits `reverse-deepagent.source-map-selected-executor-result-checkpoint.v1`, verifies selected action / consumer / review gate / result schema / approval flag / `mode=apply` / success marker / handoff digest guards, records observed application side effects, and keeps checkpoint side-effect fields false. Optional `workspace/source-map-rebuild-generation-result.json` can be attached as review evidence for rebuild follow-up without making rebuild generation automatic.

`native-web` exposes this through `source-map-selected-executor-result-checkpoint` / `source-map-selected-executor-application-result-checkpoint` / `source-map-followthrough-result-checkpoint` before browser session acquisition and returns `virtual://workspace/source-map-selected-executor-result-checkpoint.json`. The workspace contract indexes `workspace/source-map-selected-executor-result-checkpoint.json` with future alias `/workspace/debugger/source-map-selected-executor-result-checkpoint.json` and category `audit`; coordinator extraction / artifact category mapping classify the optional evidence source as `audit`; hook and debugger review warn when a ready checkpoint needs follow-through review and block failed / blocked checkpoint descriptors.

Boundary: this closes only the read-only checkpoint after an explicit selected executor application result. It may observe that the previous explicit application started a browser, sent CDP, evaluated JavaScript, installed a source-logpoint / hook, or applied rebuild metadata, but the checkpoint itself does not invoke any selected executor application route, continue debugger execution, install additional hooks or logpoints, generate rebuild bundles, fetch Source Maps, export raw source or previews, start browsers, evaluate JavaScript, send CDP, call MCP, perform delivery, or touch mobile full runtime chains. Automatic Source Map follow-through remains unsupported.

Tests cover manager-level ready / blocked checkpoint behavior, native-web no-browser route metadata, workspace contract, coordinator payload/category, hook/debugger review warning and blocker behavior, compileall, and related Source Map / native-web / workspace / coordinator / subagent regressions.

### Step 303：Source Map follow-through completion / next-action checkpoint descriptor baseline

Status: implemented as a read-only / review-only / checkpoint-only descriptor after the Step 302 selected-executor application result checkpoint. This is not automatic Source Map follow-through, not selected executor execution, not debugger continuation, not hook or source-logpoint installation, not rebuild generation, not delivery, not Source Map fetching, not browser launch, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`SourceMapFollowthroughCompletionCheckpointManager` consumes a ready `reverse-deepagent.source-map-selected-executor-result-checkpoint.v1` descriptor plus optional `reverse-deepagent.source-map-followthrough-chain-readiness.v1` and optional `reverse-deepagent.source-map-rebuild-generation-result.v1` evidence. It emits `reverse-deepagent.source-map-followthrough-completion-checkpoint.v1`, verifies result-checkpoint readiness / digest guards / consumer and action guards / no automated-next-step claims, then normalizes the selected consumer into a terminal review candidate or explicit follow-up requirement. Debugger, hook, and source-logpoint consumers become terminal review candidates for artifact or timeline inspection; rebuild without a reviewed generation result remains `followup_required` and points to reviewed rebuild generation.

`native-web` exposes this through `source-map-followthrough-completion-checkpoint` / `source-map-followthrough-completion-review` / `source-map-followthrough-next-action-checkpoint` before browser session acquisition and returns `virtual://workspace/source-map-followthrough-completion-checkpoint.json`. The workspace contract indexes `workspace/source-map-followthrough-completion-checkpoint.json` with future alias `/workspace/debugger/source-map-followthrough-completion-checkpoint.json` and category `audit`; coordinator extraction / artifact category mapping classify the optional evidence source as `audit`; hook and debugger review warn when a ready completion checkpoint is ready for completion review and block failed / blocked checkpoint descriptors.

Boundary: this closes only the read-only completion / next-action checkpoint after a selected executor result checkpoint. It does not invoke selected executor application routes, continue debugger execution, install additional hooks or logpoints, generate rebuild bundles, run delivery, fetch Source Maps, export raw source or previews, start browsers, evaluate JavaScript, send CDP, call MCP, or touch mobile full runtime chains. Automatic Source Map follow-through remains unsupported; any terminal artifact inspection, rebuild generation, delivery, or subsequent execution remains a separate explicit reviewed action.

Tests cover manager-level debugger terminal-review candidate, rebuild follow-up, bad-checkpoint blockers, native-web no-browser route metadata, workspace contract, coordinator payload/category, hook/debugger review warning and blocker behavior, compileall, and related Source Map / native-web / workspace / coordinator / subagent regressions.

### Step 304：Source Map terminal review package / audit handoff descriptor baseline

Status: implemented as a read-only / review-only / audit-handoff-only terminal review package after the Step 303 follow-through completion checkpoint. This is not automatic Source Map follow-through, not selected executor execution, not debugger continuation, not hook or source-logpoint installation, not rebuild generation, not delivery, not Source Map fetching, not browser launch, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`SourceMapTerminalReviewPackageManager` consumes a ready `reverse-deepagent.source-map-followthrough-completion-checkpoint.v1` descriptor plus optional `reverse-deepagent.source-map-selected-executor-result-checkpoint.v1` evidence. It emits `reverse-deepagent.source-map-terminal-review-package.v1`, verifies completion-checkpoint readiness / digest guards / consumer and action guards / no automated-next-step claims, then packages terminal review or follow-up review evidence into `reverse-deepagent.source-map-terminal-review-package.payload.v1` with recommended review action, required artifacts and manual review steps.

`native-web` exposes this through `source-map-terminal-review-package` / `source-map-terminal-review-handoff` / `source-map-followthrough-terminal-review-package` / `source-map-followthrough-audit-handoff` before browser session acquisition and returns `virtual://workspace/source-map-terminal-review-package.json`. The workspace contract indexes `workspace/source-map-terminal-review-package.json` with future alias `/workspace/review/source-map-terminal-review-package.json` and category `audit`; coordinator extraction / artifact category mapping classify the optional evidence source as `audit`; hook and debugger review warn when a ready package is ready for terminal review and block failed / blocked package descriptors.

Boundary: this closes only the read-only terminal review package / audit handoff after a Source Map completion checkpoint. It does not execute the recommended review action, invoke selected executor application routes, continue debugger execution, install additional hooks or logpoints, generate rebuild bundles, run delivery, fetch Source Maps, export raw source or previews, start browsers, evaluate JavaScript, send CDP, call MCP, or touch mobile full runtime chains. Automatic Source Map follow-through remains unsupported; package consumers must still run any follow-up as a separate explicit reviewed action.

Tests cover manager-level ready / blocked package behavior, native-web no-browser route metadata, workspace contract, coordinator payload/category, hook/debugger review warning and blocker behavior, compileall, and related Source Map / native-web / workspace / coordinator / subagent regressions.

### Step 305：Source Map terminal review observed-result / closure audit checkpoint baseline

Status: implemented as a read-only / review-only / audit-checkpoint-only observed-result closure checkpoint after the Step 304 terminal review package. This is not automatic Source Map follow-through, not terminal review action execution, not selected executor execution, not debugger continuation, not hook or source-logpoint installation, not rebuild generation, not delivery, not Source Map fetching, not browser launch, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`SourceMapTerminalReviewClosureCheckpointManager` consumes a ready `reverse-deepagent.source-map-terminal-review-package.v1` descriptor plus reviewer-provided observed terminal review result evidence such as `reverse-deepagent.source-map-terminal-review-observed-result.v1`. It emits `reverse-deepagent.source-map-terminal-review-closure-checkpoint.v1`, verifies terminal package readiness / digest guards / consumer and action guards / observed manual review completion / no checkpoint-side-effect claims, and packages the closure evidence into `reverse-deepagent.source-map-terminal-review-closure-audit.v1` with observed review action, required artifacts, closure status, package digest, observed-result digest, and review notes digest metadata.

`native-web` exposes this through `source-map-terminal-review-closure-checkpoint` / `source-map-terminal-review-observed-result-checkpoint` / `source-map-followthrough-closure-audit` / `source-map-terminal-review-closure-audit` before browser session acquisition and returns `virtual://workspace/source-map-terminal-review-closure-checkpoint.json`. The workspace contract indexes `workspace/source-map-terminal-review-closure-checkpoint.json` with future alias `/workspace/review/source-map-terminal-review-closure-checkpoint.json` and category `audit`; coordinator extraction / artifact category mapping classify the optional evidence source as `audit`; hook and debugger review warn when a ready checkpoint is ready for closure review and block failed / blocked checkpoint descriptors.

Boundary: this closes only the read-only observed-result / closure audit checkpoint after a Source Map terminal review package. It does not execute the recommended review action, invoke selected executor application routes, continue debugger execution, install hooks or source-logpoints, generate rebuild bundles, run delivery, fetch Source Maps, export raw source or previews, start browsers, evaluate JavaScript, send CDP, call MCP, or touch mobile full runtime chains. Automatic Source Map follow-through remains unsupported; any follow-up remains a separate explicit reviewed action.

Tests cover manager-level ready / blocked closure checkpoint behavior, native-web no-browser route metadata, workspace contract, coordinator payload/category, hook/debugger review warning and blocker behavior, compileall, and related Source Map / native-web / workspace / coordinator / subagent regressions.

### Step 306：Source Map terminal review closure summary / final audit rollup descriptor baseline

Status: implemented as a read-only / review-only / audit-rollup-only final audit descriptor after the Step 305 terminal review closure checkpoint. This is not automatic Source Map follow-through, not terminal review action execution, not selected executor execution, not debugger continuation, not hook or source-logpoint installation, not rebuild generation, not delivery, not Source Map fetching, not browser launch, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`SourceMapTerminalReviewFinalAuditManager` consumes a ready `reverse-deepagent.source-map-terminal-review-closure-checkpoint.v1` descriptor plus optional `reverse-deepagent.source-map-terminal-review-package.v1` evidence. It emits `reverse-deepagent.source-map-terminal-review-final-audit.v1`, verifies closure-checkpoint readiness / digest guards / consumer and action guards / no rollup-side-effect claims, then packages final Source Map follow-through review closure evidence into `reverse-deepagent.source-map-terminal-review-final-audit-rollup.v1` with final audit status, observed review action, recommended review action, required artifacts, closure digest, terminal package digest, observed-result digest, and manual review evidence metadata.

`native-web` exposes this through `source-map-terminal-review-final-audit` / `source-map-terminal-review-final-audit-rollup` / `source-map-followthrough-final-audit` / `source-map-terminal-review-closure-summary` before browser session acquisition and returns `virtual://workspace/source-map-terminal-review-final-audit.json`. The workspace contract indexes `workspace/source-map-terminal-review-final-audit.json` with future alias `/workspace/review/source-map-terminal-review-final-audit.json` and category `audit`; coordinator extraction / artifact category mapping classify the optional evidence source as `audit`; hook and debugger review warn when a ready rollup is ready for final audit review and block failed / blocked rollup descriptors.

Boundary: this closes only the read-only closure summary / final audit rollup after a Source Map terminal review closure checkpoint. It does not execute the recommended review action, invoke selected executor application routes, continue debugger execution, install hooks or source-logpoints, generate rebuild bundles, run delivery, fetch Source Maps, export raw source or previews, start browsers, evaluate JavaScript, send CDP, call MCP, or touch mobile full runtime chains. Automatic Source Map follow-through remains unsupported; any next action remains a separate explicit reviewed action.

Tests cover manager-level ready / blocked final audit behavior, native-web no-browser route metadata, workspace contract, coordinator payload/category, hook/debugger review warning and blocker behavior, compileall, and related Source Map / native-web / workspace / coordinator / subagent regressions.

### Step 307：BrowserProvider doctor launch-command review payload baseline

Status: implemented as a doctor-level side-effect-free BrowserProvider launch-smoke command review payload after the BrowserProvider smoke CLI `--print-launch-command` baseline. This is not browser launch, not provider availability probing, not provider registration resolution, not provider factory invocation, not workspace artifact writing, not CDP endpoint probing, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`reverse-agent-doctor --print-browser-launch-command` now emits `browser_provider_launch_command` with `reverse-deepagent.browser-provider-launch-command.v1`, reusing the redaction-safe command planner from `reverse_deepagent.browser_provider_smoke`. The payload records requested BrowserProvider config, a reviewed `review_command_hint.command` containing `--launch-browser-smoke`, and a strict side-effect policy with `provider_registry_resolved=false`, `provider_factories_invoked=false`, `starts_browser=false`, `writes_artifact=false`, `calls_mcp=false`, and `touches_mobile_full_runtime_chains=false`. Malformed browser args return a structured `ok=false` command payload without probing ports or starting any provider.

Boundary: this closes only the doctor entrypoint gap for reviewed BrowserProvider launch-smoke command planning. It does not replace `reverse-agent-browser-provider-smoke --print-launch-command`, does not validate provider availability, does not accept launch smoke evidence, does not attach smoke evidence to a pipeline, and does not make metadata-only evidence count as runtime launch smoke. Real provider smoke still requires a separate explicit `--launch-browser-smoke` run.

Tests cover successful CloakBrowser command planning with proxy / args redaction, malformed args blockers, skipped port probes in command-only mode, and side-effect policy assertions through `tests.test_doctor`; full suite was rerun after the preceding Source Map final-audit batch.

### Step 308：BrowserProvider smoke attachment acceptance report baseline

Status: implemented as a nested metadata-only / review-only acceptance report inside the existing `attachment_acceptance` gate for reviewed `workspace/browser-provider-smoke.json` evidence. This is not smoke generation, not provider availability probing, not provider factory invocation, not browser launch, not CDP endpoint probing, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`_browser_provider_smoke_acceptance(...)` now emits `attachment_acceptance.acceptance_report` with `reverse-deepagent.browser-provider-smoke-acceptance-report.v1`. The report records evidence level, provider summary, evidence summary, side-effect summary, blockers / warnings, required follow-up, and next action. Metadata-only and availability-check evidence remain attachable provider metadata but keep `runtime_launch_smoke_accepted=false`; only a matching accepted `launch-smoke` evidence payload can be marked as runtime launch smoke.

Boundary: this closes only the reviewer-facing acceptance-report layer for already supplied smoke evidence. It does not read smoke files by itself, generate smoke evidence, call BrowserProvider factories, check availability, probe CDP endpoints, start browsers, call MCP, or touch mobile full runtime chains.

Tests cover metadata-only evidence report propagation into workspace smoke attachment / artifact index and provider mismatch blockers with side-effect summary through `tests.test_coordinator`.

### Step 309：BrowserProvider smoke standalone review CLI baseline

Status: implemented as a standalone metadata-only / review-only smoke evidence reviewer after the Step 308 attachment acceptance report. This is not smoke generation, not provider availability probing, not provider registration resolution, not provider factory invocation, not workspace artifact writing, not browser launch, not CDP endpoint probing, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`reverse-agent-browser-provider-smoke --review-smoke-json <path> --expected-provider <provider>` now reads an existing UTF-8 `workspace/browser-provider-smoke.json` object, reuses the same `browser_provider_smoke_acceptance(...)` gate as pipeline attachment, and prints `reverse-deepagent.browser-provider-smoke-review.v1` with `attachment_acceptance` plus `acceptance_report`. The acceptance implementation now lives in `reverse_deepagent.browser_provider_smoke_acceptance` so coordinator attachment and standalone CLI review share the same blocker / warning / side-effect semantics.

Boundary: this closes only the standalone reviewer / CI gate for already supplied smoke evidence. It does not run the Web pipeline, generate smoke evidence, call BrowserProvider factories, check availability, resolve provider registrations, probe CDP endpoints, start browsers, write artifacts, call MCP, or touch mobile full runtime chains.

Tests cover helper-level metadata-only review, module CLI provider-mismatch review with malformed irrelevant browser args ignored, coordinator attachment compatibility after helper extraction, compileall, and BrowserProvider smoke / coordinator regressions.

### Step 310：BrowserProvider smoke policy decision gate baseline

Status: implemented as a metadata-only / review-only policy decision layer on top of the Step 309 standalone smoke JSON reviewer. This is not smoke generation, not provider availability probing, not provider registration resolution, not provider factory invocation, not workspace artifact writing, not browser launch, not CDP endpoint probing, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`reverse-agent-browser-provider-smoke --review-smoke-json <path> --expected-provider <provider> --minimum-evidence-level <metadata-only|availability-check|launch-smoke>` now evaluates the existing `attachment_acceptance` result through `reverse-deepagent.browser-provider-smoke-policy-decision.v1`. The policy decision records `observed_evidence_level`, `minimum_evidence_level`, `decision` (`pass` / `warn` / `block`), `policy_passed`, blockers, warnings, and a strict side-effect policy. `--block-on-warnings` upgrades acceptance warnings into a blocking policy decision for CI / PR gates that want a stricter review posture.

Boundary: this closes only the decision-gate layer for already supplied smoke evidence. It consumes the existing acceptance report and never generates smoke evidence, calls BrowserProvider factories, checks availability, resolves provider registrations, probes CDP endpoints, starts browsers, writes artifacts, calls MCP, or touches mobile full runtime chains. Metadata-only evidence can pass the default metadata review with `decision=warn`, but it still has `runtime_launch_smoke_accepted=false`; requiring `--minimum-evidence-level launch-smoke` blocks metadata-only or availability-check evidence from being treated as runtime launch smoke.

Tests cover default metadata-only warning pass, insufficient evidence blocking when launch smoke is required, matching launch-smoke pass, provider mismatch blocking through the module CLI, compileall, and BrowserProvider smoke CLI regressions.

### Step 311：BrowserProvider smoke policy CI / PR gate wrapper baseline

Status: implemented as a side-effect-free CI / PR wrapper around the Step 310 BrowserProvider smoke policy decision gate. This is not smoke generation, not provider availability probing, not provider registration resolution, not provider factory invocation, not browser launch, not CDP endpoint probing, not workspace artifact writing, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`reverse-agent-browser-provider-smoke-policy --smoke-json <path> --expected-provider <provider>` now reads an existing UTF-8 `workspace/browser-provider-smoke.json` object, delegates to the same `review_browser_provider_smoke_json(...)` / `browser_provider_smoke_policy_decision(...)` path, and emits `reverse-deepagent.browser-provider-smoke-policy-gate.v1`. The gate defaults to `--minimum-evidence-level launch-smoke`, supports `--block-on-warnings`, exits `0` when policy passes, and exits `2` when policy blocks. It includes `ci_gate`, `policy_decision`, `attachment_acceptance`, `acceptance_report`, the nested standalone review payload, and a strict side-effect policy.

Public CI now runs a pure JSON regression step in `.github/workflows/ci.yml`: synthetic metadata-only evidence must be blocked when launch-smoke is required, and synthetic launch-smoke evidence with a passed smoke result must pass. The package smoke also checks that the wheel exposes `reverse-agent-browser-provider-smoke-policy --help`. The CI path does not launch Playwright / CloakBrowser / Remote CDP / hosted browsers, does not call MCP, and does not touch mobile runtime chains.

Boundary: this closes only the reusable CI / PR policy wrapper and public hosted CI regression for already supplied smoke evidence. Real provider launch smoke remains an explicit local or self-hosted action; metadata-only or availability-check evidence remains useful review metadata but cannot satisfy the default launch-smoke gate.

Tests cover helper-level metadata-only default blocking, launch-smoke pass, module CLI exit code `2` on policy block, BrowserProvider smoke CLI regressions, compileall, and CI YAML side-effect contract review.

### Step 312：BrowserProvider smoke policy fixture evidence baseline

Status: implemented as fixed synthetic evidence samples for the Step 311 CI / PR policy wrapper. This is not real browser launch evidence, not smoke generation, not provider availability probing, not provider registration resolution, not provider factory invocation, not CDP endpoint probing, not workspace artifact writing, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`tests/fixtures/browser-provider-smoke/cloakbrowser-metadata-only.json` and `tests/fixtures/browser-provider-smoke/cloakbrowser-launch-smoke.json` now provide redacted, UTF-8, side-effect-explicit BrowserProvider smoke evidence samples. The metadata-only fixture must block under the default launch-smoke CI gate with `insufficient_browser_provider_smoke_evidence_level`; the launch-smoke fixture must pass and expose `runtime_launch_smoke_accepted=true` without causing the gate itself to start a browser.

`.github/workflows/ci.yml` now runs the public BrowserProvider smoke policy regression against these repository fixtures instead of generating ad hoc JSON in the workflow step. This keeps the CI gate, local tests, and documentation aligned on the same evidence contract.

Boundary: this closes only the fixture contract for BrowserProvider smoke policy review. The launch-smoke fixture is synthetic contract evidence for the gate; real BrowserProvider launch smoke remains an explicit local or self-hosted action before runtime smoke can be claimed for a real environment.

Tests cover fixture schema / redaction / side-effect contract, metadata-only blocking, launch-smoke pass, module CLI exit code behavior, compileall, and BrowserProvider smoke policy / smoke CLI regressions.

### Step 313：Scoped runtime-collected object graph diff baseline

Status: implemented as an explicit scoped runtime-collected object graph diff baseline after the review-only Step 203 object graph diff descriptor. This is not full JS heap diff, not CDP HeapProfiler integration, not complete heap traversal, not default recon, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`RuntimeObjectGraphDiffManager` accepts `runtime-object-graph-diff` / `runtime_collected_object_graph_diff` style requests with a strict dotted object root such as `window.__appState`. It collects bounded descriptor-safe before / after snapshots around an optional explicit trigger, avoids accessor getter invocation and prototype traversal, reuses `ObjectGraphDiffManager` review semantics for diff / risk / hook readiness, and emits `reverse-deepagent.runtime-object-graph-diff.v1` with `runtime_evaluated=true`, `full_heap_snapshot=false`, and `complete_heap_traversal=false`.

`native-web` exposes this through explicit `runtime-object-graph-diff` / `runtime-collected-object-graph-diff` / `js-runtime-object-graph-diff` protection names and returns `virtual://workspace/runtime-object-graph-diff.json`. The workspace contract indexes `workspace/runtime-object-graph-diff.json` with future alias `/workspace/browser/runtime-object-graph-diff.json` and category `triage`; coordinator extraction / artifact category mapping classify `runtime_object_graph_diff` evidence as `triage`; hook review warns with `runtime_object_graph_diff_requires_review` when ready and blocks failed / blocked descriptors.

Boundary: this closes only scoped runtime collection for one explicit object root. It does not weaken `object-graph-diff` caller-provided / review-only semantics, does not collect browser heap snapshots, does not prove complete reachability, does not start browsers by metadata-only review paths, does not send CDP commands, does not call MCP, and does not touch Android / iOS / mini-program full runtime chains.

Tests cover manager-level runtime collection / unsafe-root blocking, native-web explicit route metadata, workspace contract, coordinator payload/category, hook review warning / blocker behavior, compileall, and related page mutation / native-web / workspace / coordinator / hook subagent regressions.


### Step 314：CDP HeapProfiler heap snapshot readiness preflight baseline

Status: implemented as a read-only / review-only / preflight-only descriptor after the Step 313 scoped runtime-collected object graph diff baseline. This is not heap snapshot collection, not CDP HeapProfiler enable / takeHeapSnapshot execution, not heap diff, not complete heap traversal, not default recon, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`HeapSnapshotReadinessManager` accepts `heap-snapshot-readiness` / `cdp-heap-snapshot-readiness` / `heap-profiler-readiness` style requests and only normalizes caller-provided BrowserProvider id, CDP availability, HeapProfiler capability evidence, max snapshot byte budget, raw-heap-export intent, and redaction gate metadata. It emits `reverse-deepagent.heap-snapshot-readiness.v1` with `heap_snapshot_collected=false`, `heap_diff_computed=false`, `complete_heap_traversal_claimed=false`, `future_collection_contract.implemented=true`, and a strict side-effect policy that keeps `browser_started=false`, `provider_factory_invoked=false`, `cdp_command_sent=false`, `heap_profiler_enabled=false`, `runtime_evaluated=false`, `raw_heap_exported=false`, `calls_mcp=false`, and `mobile_runtime_used=false`.

`native-web` exposes this through explicit `heap-snapshot-readiness` / `cdp-heap-snapshot-readiness` / `heap-profiler-readiness` protection names and returns `virtual://workspace/heap-snapshot-readiness.json` without starting the configured BrowserProvider. The workspace contract indexes `workspace/heap-snapshot-readiness.json` with future alias `/workspace/browser/heap-snapshot-readiness.json` and category `triage`; coordinator extraction / artifact category mapping classify `heap_snapshot_readiness` evidence as `triage`; hook review warns with `heap_snapshot_readiness_requires_review` when ready and blocks failed / blocked descriptors.

Boundary: this closes only the review preflight before the separate HeapProfiler collection surface. It does not collect heap snapshots, enable HeapProfiler, call `takeHeapSnapshot`, compute heap diffs, export raw heap data, prove complete object reachability, start browsers by metadata-only review paths, invoke provider factories, send CDP commands, call MCP, or touch Android / iOS / mini-program full runtime chains. Full JS heap diff remains separate capability-gated follow-up work; explicit review-gated heap snapshot collection is tracked by Step 315 as a digest-only metadata MVP.

Tests cover manager-level ready / blocked readiness, native-web no-browser protection routing, workspace contract aliasing, coordinator payload/category mapping, hook review warning / blocker behavior, and related page mutation / native-web / workspace / coordinator / hook subagent regressions.

### Step 315：Explicit review-gated HeapProfiler heap snapshot collect metadata MVP

Status: implemented as an explicit-review-only one-shot CDP HeapProfiler collection metadata MVP after the Step 314 readiness preflight. This is not full JS heap diff, not raw heap export, not complete heap traversal, not default recon, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`HeapSnapshotCollectManager` accepts `heap-snapshot-collect` / `cdp-heap-snapshot-collect` / `collect-heap-snapshot` style requests only when `review_approved=true`, an explicit collection flag such as `collect_heap_snapshot=true`, a ready `heap-snapshot-readiness` descriptor, CDP availability, HeapProfiler capability evidence, size budget, and redaction gates are present. Successful collection sends only `HeapProfiler.enable`, `HeapProfiler.takeHeapSnapshot`, and best-effort `HeapProfiler.disable`, then emits `reverse-deepagent.heap-snapshot-collect.v1` digest / byte-count / chunk-count metadata with `raw_heap_exported=false`, `heap_diff_computed=false`, and `complete_heap_traversal_claimed=false`.

`native-web` exposes this through explicit `heap-snapshot-collect` / `cdp-heap-snapshot-collect` / `collect-heap-snapshot` protection names and returns `virtual://workspace/heap-snapshot-collect.json`. The workspace contract indexes `workspace/heap-snapshot-collect.json` with future alias `/workspace/browser/heap-snapshot-collect.json` and category `audit`; coordinator extraction / artifact category mapping classify `heap_snapshot_collect` evidence as `audit`; hook review warns with `heap_snapshot_collect_requires_review` when collected and blocks failed / blocked / unsupported descriptors.

Boundary: this closes only reviewed one-shot HeapProfiler metadata collection. It does not weaken `heap-snapshot-readiness` no-browser preflight semantics, does not export raw heap chunks, does not compute heap diff, does not prove complete object reachability, does not run in default recon, does not call MCP, and does not touch Android / iOS / mini-program full runtime chains. Full JS heap diff remains separate capability-gated follow-up work.

Tests cover manager-level approval gating / digest-only collection, native-web explicit route CDP command boundaries, workspace contract aliasing, coordinator payload/category mapping, hook review warning / blocker behavior, compileall, and related page mutation / native-web / workspace / coordinator / hook subagent regressions.

### Step 316：Heap snapshot diff readiness pair-review descriptor baseline

Status: implemented as a read-only / review-only / preflight-only pair-review descriptor after the Step 315 explicit HeapProfiler collect metadata MVP. This is not full JS heap diff, not raw heap loading, not raw heap export, not complete heap traversal, not default recon, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`HeapSnapshotDiffReadinessManager` accepts `heap-snapshot-diff-readiness` / `heap-snapshot-diff-review` / `review-heap-snapshot-diff` style requests and only consumes two already-collected `reverse-deepagent.heap-snapshot-collect.v1` metadata descriptors. It verifies schema, `status=collected`, digest presence, byte count, raw-heap absence, no prior heap diff, no complete-traversal claim, and optional same-provider consistency. It emits `reverse-deepagent.heap-snapshot-diff-readiness.v1` with digest / byte-count delta pair summary, `heap_diff_computed=false`, `raw_heap_loaded=false`, `raw_heap_exported=false`, `complete_heap_traversal_claimed=false`, and `future_diff_executor_implemented=false`.

`native-web` exposes this through explicit `heap-snapshot-diff-readiness` / `heap-snapshot-diff-review` / `review-heap-snapshot-diff` protection names and returns `virtual://workspace/heap-snapshot-diff-readiness.json` without starting the configured BrowserProvider. The workspace contract indexes `workspace/heap-snapshot-diff-readiness.json` with future alias `/workspace/browser/heap-snapshot-diff-readiness.json` and category `triage`; coordinator extraction / artifact category mapping classify `heap_snapshot_diff_readiness` evidence as `triage`; hook review warns with `heap_snapshot_diff_readiness_requires_review` when ready and blocks failed / blocked / unsupported descriptors.

Boundary: this closes only the review gate between two digest-only collect metadata artifacts and any future heap diff executor. It does not load raw heap chunks, does not compute heap diffs, does not prove complete object reachability, does not start browsers, does not send CDP commands, does not call MCP, and does not touch Android / iOS / mini-program full runtime chains. Full JS heap diff remains separate capability-gated follow-up work.

Tests cover manager-level pair review / raw-heap blocker behavior, native-web no-browser route metadata, workspace contract aliasing, coordinator payload/category mapping, hook review warning / blocker behavior, compileall, and related page mutation / native-web / workspace / coordinator / hook subagent regressions.

### Step 317：Heap snapshot diff executor preflight / raw-ingestion safety gate baseline

Status: implemented as a read-only / review-only / preflight-only executor gate after the Step 316 heap snapshot diff readiness pair-review descriptor. This is not full JS heap diff, not raw heap loading, not raw heap parsing, not raw heap export, not complete heap traversal, not default recon, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`HeapSnapshotDiffExecutorPreflightManager` accepts `heap-snapshot-diff-executor-preflight` / `heap-snapshot-diff-preflight` / `raw-heap-diff-preflight` style requests and consumes a ready `reverse-deepagent.heap-snapshot-diff-readiness.v1` descriptor plus explicit review approval, raw heap ingestion policy, parser sandbox, redaction plan, and size budget metadata. It emits `reverse-deepagent.heap-snapshot-diff-executor-preflight.v1` with `raw_heap_loaded=false`, `raw_heap_parsed=false`, `raw_heap_exported=false`, `heap_diff_computed=false`, `complete_heap_traversal_claimed=false`, and `diff_executor_implemented=false`. Unsupported raw ingestion policies, disabled parser sandbox / redaction plan, raw export requests, actual diff execution requests, or complete traversal claims are blocked before any runtime side effect.

`native-web` exposes this through explicit `heap-snapshot-diff-executor-preflight` / `heap-snapshot-diff-preflight` / `raw-heap-diff-preflight` protection names and returns `virtual://workspace/heap-snapshot-diff-executor-preflight.json` without starting the configured BrowserProvider. The workspace contract indexes `workspace/heap-snapshot-diff-executor-preflight.json` with future alias `/workspace/browser/heap-snapshot-diff-executor-preflight.json` and category `triage`; coordinator extraction / artifact category mapping classify `heap_snapshot_diff_executor_preflight` evidence as `triage`; hook review warns with `heap_snapshot_diff_executor_preflight_requires_review` when ready and blocks failed / blocked / unsupported descriptors.

Boundary: this closes only the raw-ingestion and parser-safety gate between digest-only diff readiness and any future raw-heap-aware diff executor. It does not read heap snapshot chunks, parse raw heap graph structures, compute heap diffs, prove complete object reachability, start browsers, send CDP commands, call MCP, or touch Android / iOS / mini-program full runtime chains. Full JS heap diff remains separate capability-gated follow-up work.

Tests cover manager-level ready / blocked preflight behavior, native-web no-browser route metadata, workspace contract aliasing, coordinator payload/category mapping, hook review warning / blocker behavior, compileall, and related page mutation / native-web / workspace / coordinator / hook subagent regressions.

### Step 318：Heap snapshot diff executor approval / transaction plan baseline

Status: implemented as a read-only / review-only / approval-plan-only / transaction-plan-only descriptor after the Step 317 heap snapshot diff executor preflight. This is not approval recording, not transaction journal writing, not bounded executor gating, not heap diff execution, not raw heap loading, not raw heap parsing, not raw heap export, not complete heap traversal, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`HeapSnapshotDiffExecutorApprovalPlanManager` accepts `heap-snapshot-diff-executor-approval-plan` / `heap-snapshot-diff-approval-plan` / `raw-heap-diff-approval-plan` style requests and consumes a ready `reverse-deepagent.heap-snapshot-diff-executor-preflight.v1` descriptor. It emits `reverse-deepagent.heap-snapshot-diff-executor-approval-plan.v1` with approval-record, transaction-journal, bounded-gate, idempotency-key, and result-artifact planning metadata while keeping `approval_recorded=false`, `transaction_started=false`, `journal_written_now=false`, `bounded_executor_gate_written=false`, `executor_invoked=false`, `raw_heap_loaded=false`, `raw_heap_parsed=false`, `raw_heap_exported=false`, `heap_diff_computed=false`, and `diff_executor_implemented=false`.

`native-web` exposes this through explicit `heap-snapshot-diff-executor-approval-plan` / `heap-snapshot-diff-approval-plan` / `raw-heap-diff-approval-plan` protection names and returns `virtual://workspace/heap-snapshot-diff-executor-approval-plan.json` without starting the configured BrowserProvider. The workspace contract indexes `workspace/heap-snapshot-diff-executor-approval-plan.json` with future alias `/workspace/browser/heap-snapshot-diff-executor-approval-plan.json` and category `triage`; coordinator extraction / artifact category mapping classify `heap_snapshot_diff_executor_approval_plan` evidence as `triage`; hook review warns with `heap_snapshot_diff_executor_approval_plan_requires_review` when ready and blocks failed / blocked / unsupported descriptors.

Boundary: this closes only the review planning gate between preflight and a separate explicit approval-record writer / future transaction-journal writer / bounded executor gate. It does not record approval, write transaction journals, write bounded gate artifacts, invoke an executor, read heap snapshot chunks, parse raw heap graph structures, compute heap diffs, prove complete object reachability, start browsers, send CDP commands, call MCP, or touch Android / iOS / mini-program full runtime chains. Full JS heap diff remains separate capability-gated follow-up work.

### Step 319：Heap snapshot diff executor approval-record writer baseline

Status: implemented as an explicit-review-only approval-record writer after the Step 318 heap snapshot diff executor approval / transaction plan descriptor. This is not a transaction preflight, not a transaction journal writer, not a bounded executor gate, not raw heap parsing, not heap diff execution, not browser launch, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

The hook subagent now exposes `record_heap_snapshot_diff_executor_approval`. The writer consumes a ready `reverse-deepagent.heap-snapshot-diff-executor-approval-plan.v1` descriptor, requires `mode=apply`, `write_result=true`, `approve_heap_snapshot_diff_executor=true`, a reviewer, matching optional approval-scope / transaction-id / idempotency-key / digest guards, and writes only `workspace/heap-snapshot-diff-executor-approval-record.json`. Dry-run and blocked paths remain read-only.

The workspace contract indexes `workspace/heap-snapshot-diff-executor-approval-record.json` with future alias `/workspace/browser/heap-snapshot-diff-executor-approval-record.json` and category `audit`; coordinator extraction / artifact category mapping classify `heap_snapshot_diff_executor_approval_record` evidence as `audit`; hook review warns with `heap_snapshot_diff_executor_approval_record_ready_for_transaction_preflight` when a written approved record is ready for the next transaction-preflight follow-up and blocks failed / blocked records.

Boundary: this closes only the explicit reviewer approval-record layer between the approval / transaction plan and future transaction-preflight / transaction-journal / bounded-gate / executor work. It does not start transactions, write journals, write bounded gate artifacts, invoke an executor, read heap snapshot chunks, parse raw heap graph structures, compute heap diffs, prove complete object reachability, start browsers, send CDP commands, call MCP, or touch Android / iOS / mini-program full runtime chains. Full JS heap diff remains separate capability-gated follow-up work.

Tests cover explicit approval-record writer dry-run / apply gates / apply-write-only behavior, hook subagent tool exposure and prompt guidance, hook review warning and blocker behavior, workspace contract, coordinator payload/category, compileall, and related subagent regressions.


Tests cover manager-level ready / blocked approval-plan behavior, native-web no-browser route metadata, workspace contract aliasing, coordinator payload/category mapping, hook review warning / blocker behavior, compileall, and related page mutation / native-web / workspace / coordinator / hook subagent regressions.

### Step 320：Heap snapshot diff executor transaction-preflight descriptor baseline

Status: implemented as a read-only / review-only / transaction-preflight-only descriptor after the Step 319 heap snapshot diff executor approval-record writer. This is not a transaction journal writer, not a bounded executor gate, not raw heap parsing, not heap diff execution, not browser launch, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`HeapSnapshotDiffExecutorTransactionPreflightManager` consumes a ready `reverse-deepagent.heap-snapshot-diff-executor-approval-plan.v1` descriptor plus a written approved `reverse-deepagent.heap-snapshot-diff-executor-approval-record.v1` record. It verifies approval-plan / approval-record schemas and statuses, approval scope, transaction id, idempotency key, optional digest guards, no prior transaction start, no journal write, no bounded gate write, no executor invocation, no raw heap load / parse / export, no heap diff computation, and no complete traversal claim. It emits `reverse-deepagent.heap-snapshot-diff-executor-transaction-preflight.v1` with `ready_to_write_journal` only as a future journal-writer review gate while keeping `transaction_started=false`, `journal_written=false`, `bounded_executor_gate_written=false`, `executor_invoked=false`, `raw_heap_loaded=false`, `raw_heap_parsed=false`, `raw_heap_exported=false`, and `heap_diff_computed=false`.

`native-web` exposes this through `heap-snapshot-diff-executor-transaction-preflight` / `heap-snapshot-diff-transaction-preflight` / `heap-diff-executor-transaction-preflight` / `raw-heap-diff-transaction-preflight` / `review-heap-snapshot-diff-executor-transaction-preflight` protection names before browser session acquisition and returns `virtual://workspace/heap-snapshot-diff-executor-transaction-preflight.json`. The workspace contract indexes `workspace/heap-snapshot-diff-executor-transaction-preflight.json` with future alias `/workspace/browser/heap-snapshot-diff-executor-transaction-preflight.json` and category `triage`; coordinator extraction / artifact category mapping classify `heap_snapshot_diff_executor_transaction_preflight` evidence as `triage`; hook review warns with `heap_snapshot_diff_executor_transaction_preflight_ready_for_journal_writer` when ready and blocks failed / blocked descriptors.

Boundary: this closes only the read-only transaction-preflight layer between the approval record and future transaction-journal / bounded-gate / executor work. It does not write journals, write bounded gate artifacts, invoke an executor, read heap snapshot chunks, parse raw heap graph structures, compute heap diffs, prove complete object reachability, start browsers, send CDP commands, call MCP, or touch Android / iOS / mini-program full runtime chains. Full JS heap diff remains separate capability-gated follow-up work.

Tests cover manager-level ready / blocked transaction-preflight behavior, native-web no-browser route metadata, workspace contract aliasing, coordinator payload/category mapping, hook review warning / blocker behavior, compileall, and related page mutation / native-web / workspace / coordinator / hook subagent regressions.

### Step 321：Heap snapshot diff executor transaction journal writer baseline

Status: implemented as an explicit-review-only transaction journal writer after the Step 320 heap snapshot diff executor transaction-preflight descriptor. This is not a bounded executor gate, not raw heap parsing, not heap diff execution, not browser launch, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

The hook subagent now exposes `record_heap_snapshot_diff_executor_transaction_journal`. The writer consumes a ready `reverse-deepagent.heap-snapshot-diff-executor-transaction-preflight.v1` descriptor, requires `mode=apply`, `write_result=true`, `approve_transaction_journal=true`, a reviewer, and matching optional approval-scope / transaction-id / idempotency-key / transaction-preflight digest guards, and writes only `workspace/heap-snapshot-diff-executor-journal.json`. Dry-run and blocked paths remain read-only.

The workspace contract indexes `workspace/heap-snapshot-diff-executor-journal.json` with future alias `/workspace/browser/heap-snapshot-diff-executor-journal.json` and category `audit`; coordinator extraction / artifact category mapping classify `heap_snapshot_diff_executor_transaction_journal` evidence as `audit`; hook review warns with `heap_snapshot_diff_executor_transaction_journal_ready_for_bounded_gate` when a written journal is ready for bounded-gate follow-up and blocks failed / blocked journals.

Boundary: this closes only the explicit transaction-journal audit layer between transaction-preflight and future bounded-gate / executor work. It records `transaction_started=true` and `journal_written=true` only as reviewed audit metadata, but it does not write bounded gate artifacts, invoke an executor, read heap snapshot chunks, parse raw heap graph structures, compute heap diffs, prove complete object reachability, start browsers, send CDP commands, call MCP, or touch Android / iOS / mini-program full runtime chains. Full JS heap diff remains separate capability-gated follow-up work.

Tests cover explicit journal writer dry-run / apply gates / apply-write-only behavior, hook subagent tool exposure and prompt guidance, hook review warning behavior, workspace contract, coordinator payload/category, compileall, and related hook / workspace / coordinator regressions.

### Step 322：Heap snapshot diff executor bounded executor gate descriptor baseline

Status: implemented as a read-only / review-only / bounded-gate-only descriptor after the Step 321 heap snapshot diff executor transaction journal writer. This is not raw heap parsing, not heap diff execution, not browser launch, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`HeapSnapshotDiffExecutorBoundedGateManager` consumes a written `reverse-deepagent.heap-snapshot-diff-executor-transaction-journal.v1` record and emits `reverse-deepagent.heap-snapshot-diff-executor-bounded-gate.v1`. The descriptor verifies journal schema / status, transaction id / idempotency key metadata, `transaction_started=true`, `journal_written=true`, `requires_bounded_executor_gate_followup=true`, explicit executor review readiness, no prior bounded gate write, no executor invocation, no raw heap load / parse / export, no heap diff computation, and no complete traversal claim. It keeps `ready_to_execute_now=false`, `bounded_executor_gate_written=false`, `executor_invoked=false`, `raw_heap_loaded=false`, `raw_heap_parsed=false`, `raw_heap_exported=false`, `heap_diff_computed=false`, and `diff_executor_implemented=false`.

`native-web` exposes this through `heap-snapshot-diff-executor-bounded-gate` / `heap-snapshot-diff-executor-bounded-executor-gate` / `heap-snapshot-diff-bounded-gate` / `heap-diff-executor-bounded-gate` / `raw-heap-diff-bounded-gate` / `review-heap-snapshot-diff-executor-bounded-gate` protection names before browser session acquisition and returns `virtual://workspace/heap-snapshot-diff-executor-bounded-gate.json`. The workspace contract indexes `workspace/heap-snapshot-diff-executor-bounded-gate.json` with future alias `/workspace/browser/heap-snapshot-diff-executor-bounded-gate.json` and category `triage`; coordinator extraction / artifact category mapping classify `heap_snapshot_diff_executor_bounded_gate` evidence as `triage`; hook review warns with `heap_snapshot_diff_executor_bounded_gate_ready_for_executor_review` when ready and blocks failed / blocked descriptors.

Boundary: this closes only the bounded executor input review layer between the transaction journal and future raw-heap-aware parser / executor work. It does not write bounded gate artifacts beyond the descriptor output, invoke an executor, read heap snapshot chunks, parse raw heap graph structures, compute heap diffs, prove complete object reachability, start browsers, send CDP commands, call MCP, or touch Android / iOS / mini-program full runtime chains. Full JS heap diff remains separate capability-gated follow-up work; the next review action is `review_heap_snapshot_diff_executor_raw_heap_parser_or_executor_mvp`.

Tests cover manager-level ready / blocked bounded-gate behavior, native-web no-browser route metadata, workspace contract aliasing, coordinator payload/category mapping, hook review warning / blocker behavior, compileall, and related page mutation / native-web / workspace / coordinator / hook subagent regressions.

### Step 323：Heap snapshot diff executor raw heap parser / summary diff MVP

Status: implemented as an explicit-review-only summary diff executor MVP after the Step 322 bounded executor gate descriptor. This is not full JS heap graph diff, not raw heap export, not retained-size proof, not path-to-root analysis, not browser launch, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`HeapSnapshotDiffExecutorManager` consumes a ready `reverse-deepagent.heap-snapshot-diff-executor-bounded-gate.v1` descriptor plus two caller-provided V8 heap snapshot JSON objects. It requires `mode=apply`, `review_approved=true`, `approve_heap_snapshot_diff_executor_execution=true`, a reviewer, size-budget checks, no raw heap export request, and no complete traversal claim request before parsing. It emits `reverse-deepagent.heap-snapshot-diff-executor-result.v1` with digest / size metadata, node / edge totals, bounded node-type / edge-type counts, redacted constructor top-N counts, and node / edge / self-size / constructor delta summaries. Successful MVP execution may set `raw_heap_loaded=true`, `raw_heap_parsed=true`, `heap_diff_computed=true`, and `executor_invoked=true`, but it must keep `raw_heap_exported=false`, `raw_strings_exported=false`, `complete_heap_traversal_claimed=false`, `browser_started=false`, `cdp_command_sent=false`, `calls_mcp=false`, and `mobile_runtime_used=false`.

`native-web` exposes this through `execute-heap-snapshot-diff-executor` / `heap-snapshot-diff-executor-result` / `heap-snapshot-diff-executor-mvp` / `raw-heap-diff-executor` / `review-heap-snapshot-diff-executor-raw-heap-parser-or-executor-mvp` protection names before browser session acquisition and returns `virtual://workspace/heap-snapshot-diff-executor-result.json`. The workspace contract indexes `workspace/heap-snapshot-diff-executor-result.json` with future alias `/workspace/browser/heap-snapshot-diff-executor-result.json` and category `audit`; coordinator extraction / artifact category mapping classify `heap_snapshot_diff_executor_result` evidence as `audit`; hook review warns with `heap_snapshot_diff_executor_result_requires_review` for executed results and blocks failed / blocked descriptors.

Boundary: this closes only a bounded summary-diff MVP for caller-provided V8 heap snapshots. It does not read raw heap files by path, write or export raw heap chunks, export raw string tables, compute retained-size proof, compute path-to-root, prove complete object reachability, start browsers, send CDP commands, call MCP, or touch Android / iOS / mini-program full runtime chains. Full JS heap diff remains capability-gated follow-up work beyond this summary MVP.

Tests cover manager-level executed / blocked MVP behavior, native-web no-browser route metadata, workspace contract aliasing, coordinator payload/category mapping, hook review warning / blocker behavior, compileall, and related page mutation / native-web / workspace / coordinator / hook subagent regressions.

### Step 324：Heap snapshot diff result follow-up checkpoint baseline

Status: implemented as a read-only / review-only checkpoint after the Step 323 explicit summary-diff executor MVP. This is not retained-size proof, not path-to-root analysis, not raw heap file ingestion, not raw heap parsing, not a new heap diff computation, not browser launch, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`HeapSnapshotDiffFollowupCheckpointManager` consumes an existing `reverse-deepagent.heap-snapshot-diff-executor-result.v1` descriptor, verifies executed status, summary-only safety, no raw heap / raw string export, no complete traversal claim, and no forbidden browser / CDP / MCP / mobile side effects. It emits `reverse-deepagent.heap-snapshot-diff-followup-checkpoint.v1` with executor result summary, positive node / edge / constructor growth signals, and review-only recommendations such as constructor drilldown, retained-size planning, path-to-root planning, or larger-budget review. The checkpoint keeps `raw_heap_loaded=false`, `raw_heap_parsed=false`, `heap_diff_computed=false`, `retained_size_proven=false`, `path_to_root_computed=false`, and `automatic_followup_analysis=false`.

`native-web` exposes this through `heap-snapshot-diff-followup-checkpoint` / `heap-snapshot-diff-analysis-plan` / `review-heap-snapshot-diff-followup-checkpoint` / `review-heap-snapshot-diff-executor-result-followup` protection names before browser session acquisition and returns `virtual://workspace/heap-snapshot-diff-followup-checkpoint.json`. The workspace contract indexes `workspace/heap-snapshot-diff-followup-checkpoint.json` with future alias `/workspace/browser/heap-snapshot-diff-followup-checkpoint.json` and category `audit`; coordinator extraction / artifact category mapping classify `heap_snapshot_diff_followup_checkpoint` evidence as `audit`; hook review warns with `heap_snapshot_diff_followup_checkpoint_requires_review` for ready checkpoints and blocks failed / blocked descriptors.

Boundary: this closes only the review checkpoint between a summary-diff MVP result and future retained-size / path-to-root / larger-budget work. It does not read raw heap files, export raw heap chunks or raw strings, parse raw heap again, compute a new heap diff, prove retained size, compute path-to-root, prove complete object reachability, start browsers, send CDP commands, call MCP, or touch Android / iOS / mini-program full runtime chains. Full JS heap diff remains capability-gated follow-up work beyond this checkpoint.

Tests cover manager-level ready / blocked checkpoint behavior, native-web no-browser route metadata, workspace contract aliasing, coordinator payload/category mapping, hook review warning / blocker behavior, compileall, and related page mutation / native-web / workspace / coordinator / hook subagent regressions.

### Step 325：Heap snapshot diff selected-analysis input preflight baseline

Status: implemented as a read-only / review-only / preflight-only / selection-only descriptor after the Step 324 heap snapshot diff follow-up checkpoint. This is not retained-size proof, not path-to-root analysis, not constructor drilldown execution, not larger-budget second-pass execution, not raw heap file ingestion, not raw heap parsing, not a new heap diff computation, not browser launch, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`HeapSnapshotDiffSelectedAnalysisInputPreflightManager` consumes a ready `reverse-deepagent.heap-snapshot-diff-followup-checkpoint.v1` descriptor, validates checkpoint status / schema / review-only side-effect policy, selects one recommended follow-up action such as constructor growth review, node-type growth review, retained-size planning, path-to-root planning, or larger-budget review, and emits `reverse-deepagent.heap-snapshot-diff-selected-analysis-input-preflight.v1`. The descriptor carries selected candidate metadata from the summary-only checkpoint and a future executor contract while keeping `future_executor_contract.implemented=false`, `raw_heap_loaded=false`, `raw_heap_parsed=false`, `heap_diff_computed=false`, `retained_size_proven=false`, `path_to_root_computed=false`, and `automatic_followup_analysis=false`.

`native-web` exposes this through `heap-snapshot-diff-selected-analysis-input-preflight` / `heap-snapshot-diff-followup-selected-analysis-preflight` / `heap-snapshot-diff-selected-followup-preflight` / `review-heap-snapshot-diff-selected-analysis-input` protection names before browser session acquisition and returns `virtual://workspace/heap-snapshot-diff-selected-analysis-input-preflight.json`. The workspace contract indexes `workspace/heap-snapshot-diff-selected-analysis-input-preflight.json` with future alias `/workspace/browser/heap-snapshot-diff-selected-analysis-input-preflight.json` and category `triage`; coordinator extraction / artifact category mapping classify `heap_snapshot_diff_selected_analysis_input_preflight` evidence as `triage`; hook review warns with `heap_snapshot_diff_selected_analysis_input_preflight_requires_review` for ready preflights and blocks failed / blocked descriptors.

Boundary: this closes only the selected-analysis input review gate between a follow-up checkpoint and future constructor drilldown / retained-size / path-to-root / larger-budget work. It does not read raw heap files, export raw heap chunks or raw strings, parse raw heap again, compute a new heap diff, prove retained size, compute path-to-root, execute constructor drilldown, execute a larger-budget second pass, prove complete object reachability, start browsers, send CDP commands, call MCP, or touch Android / iOS / mini-program full runtime chains. Full JS heap diff remains capability-gated follow-up work beyond this preflight.

Tests cover manager-level ready / blocked preflight behavior, native-web no-browser route metadata, workspace contract aliasing, coordinator payload/category mapping, hook review warning / blocker behavior, compileall, and related page mutation / native-web / workspace / coordinator / hook subagent regressions.

### Step 326：Heap snapshot constructor-growth drilldown review descriptor baseline

Status: implemented as a read-only / review-only / drilldown-only / summary-only descriptor after the Step 325 selected-analysis input preflight. This is not retained-size proof, not path-to-root analysis, not raw heap file ingestion, not raw heap parsing, not a new heap diff computation, not browser launch, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`HeapSnapshotConstructorGrowthDrilldownManager` consumes a ready `reverse-deepagent.heap-snapshot-diff-selected-analysis-input-preflight.v1` descriptor for `review_constructor_growth`, validates schema / status / selected-action / side-effect policy / disabled future executor contract, and emits `reverse-deepagent.heap-snapshot-constructor-growth-drilldown.v1` with constructor growth candidates, summary deltas, source preflight metadata, and future retained-size / path-to-root review recommendations. It keeps `constructor_drilldown_computed=false`, `retained_size_proven=false`, `path_to_root_computed=false`, `raw_heap_loaded=false`, `raw_heap_parsed=false`, `raw_heap_exported=false`, `heap_diff_computed=false`, `new_heap_diff_computed=false`, and `automatic_followup_analysis=false`.

`native-web` exposes this through `heap-snapshot-constructor-growth-drilldown` / `heap-snapshot-diff-constructor-growth-drilldown` / `review-heap-snapshot-constructor-growth-drilldown` / `review-heap-snapshot-diff-constructor-growth` protection names before browser session acquisition and returns `virtual://workspace/heap-snapshot-constructor-growth-drilldown.json`. The workspace contract indexes `workspace/heap-snapshot-constructor-growth-drilldown.json` with future alias `/workspace/browser/heap-snapshot-constructor-growth-drilldown.json` and category `triage`; coordinator extraction / artifact category mapping classify `heap_snapshot_constructor_growth_drilldown` evidence as `triage`; hook review warns with `heap_snapshot_constructor_growth_drilldown_requires_review` for ready descriptors and blocks failed / blocked descriptors.

Boundary: this closes only the constructor-growth review descriptor after selected-analysis input preflight. It does not read raw heap files, export raw heap chunks or raw strings, parse raw heap again, compute a new heap diff, execute real constructor drilldown, prove retained size, compute path-to-root, execute a larger-budget second pass, prove complete object reachability, start browsers, send CDP commands, call MCP, or touch Android / iOS / mini-program full runtime chains.

Tests cover manager-level ready / blocked behavior, native-web no-browser route metadata, workspace contract aliasing, coordinator payload/category mapping, hook review warning / blocker behavior, compileall, and related page mutation / native-web / workspace / coordinator / hook subagent regressions.

### Step 327：Heap snapshot retained-size / path-to-root preflight handoff descriptor baseline

Status: implemented as a read-only / review-only / preflight-only / handoff-only descriptor after the Step 326 constructor-growth drilldown review descriptor. This is not retained-size proof, not path-to-root analysis, not raw heap file ingestion, not raw heap parsing, not a new heap diff computation, not browser launch, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`HeapSnapshotRetainedPathPreflightManager` consumes a ready `reverse-deepagent.heap-snapshot-constructor-growth-drilldown.v1` descriptor, validates schema / status / review-only summary policy / disabled future executor contracts, and emits `reverse-deepagent.heap-snapshot-retained-path-preflight.v1` with candidate inputs, raw heap requirements, and future retained-size / path-to-root executor contracts. It keeps `raw_heap_loaded=false`, `raw_heap_parsed=false`, `raw_heap_exported=false`, `raw_strings_exported=false`, `heap_diff_computed=false`, `retained_size_proven=false`, `path_to_root_computed=false`, `larger_budget_second_pass_computed=false`, and `automatic_followup_analysis=false`.

`native-web` exposes this through `heap-snapshot-retained-path-preflight` / `heap-snapshot-constructor-growth-retained-path-preflight` / `heap-snapshot-retained-size-path-to-root-preflight` / `review-heap-snapshot-retained-path-preflight` protection names before browser session acquisition and returns `virtual://workspace/heap-snapshot-retained-path-preflight.json`. The workspace contract indexes `workspace/heap-snapshot-retained-path-preflight.json` with future alias `/workspace/browser/heap-snapshot-retained-path-preflight.json` and category `triage`; coordinator extraction / artifact category mapping classify `heap_snapshot_retained_path_preflight` evidence as `triage`; hook review warns with `heap_snapshot_retained_path_preflight_requires_review` for ready descriptors and blocks failed / blocked descriptors.

Boundary: this closes only the retained-size / path-to-root input preflight after constructor-growth review. It does not read raw heap files, export raw heap chunks or raw strings, parse raw heap, compute retained size, compute path-to-root, execute retained-size or path-to-root executors, compute a new heap diff, execute a larger-budget second pass, prove complete object reachability, start browsers, send CDP commands, call MCP, or touch Android / iOS / mini-program full runtime chains.

Tests cover manager-level ready / blocked behavior, native-web no-browser route metadata, workspace contract aliasing, coordinator payload/category mapping, hook review warning / blocker behavior, compileall, and related page mutation / native-web / workspace / coordinator / hook subagent regressions.

### Step 328：Heap snapshot retained-size executor input review / approval gate descriptor baseline

Status: implemented as a read-only / review-only / input-review-only / approval-gate-only descriptor after the Step 327 retained-size / path-to-root preflight handoff descriptor. This is not retained-size proof, not path-to-root analysis, not raw heap file ingestion, not raw heap parsing, not heap diff computation, not approval recording, not transaction journal writing, not browser launch, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`HeapSnapshotRetainedSizeInputReviewManager` consumes a ready `reverse-deepagent.heap-snapshot-retained-path-preflight.v1` descriptor, validates schema / status / retained-size request / raw heap requirements / disabled future retained-size executor contract / candidate inputs, and emits `reverse-deepagent.heap-snapshot-retained-size-input-review.v1` with candidate inputs, raw heap requirements, retained-size executor input contract, and approval gate metadata. It keeps `executor_input_contract.implemented=false`, `approval_gate.ready_to_execute_now=false`, `raw_heap_loaded=false`, `raw_heap_parsed=false`, `raw_heap_exported=false`, `raw_strings_exported=false`, `heap_diff_computed=false`, `retained_size_proven=false`, `path_to_root_computed=false`, and `automatic_followup_analysis=false`.

`native-web` exposes this through `heap-snapshot-retained-size-input-review` / `heap-snapshot-retained-size-executor-input-review` / `heap-snapshot-retained-size-approval-gate` / `review-heap-snapshot-retained-size-input` protection names before browser session acquisition and returns `virtual://workspace/heap-snapshot-retained-size-input-review.json`. The workspace contract indexes `workspace/heap-snapshot-retained-size-input-review.json` with future alias `/workspace/browser/heap-snapshot-retained-size-input-review.json` and category `triage`; coordinator extraction / artifact category mapping classify `heap_snapshot_retained_size_input_review` evidence as `triage`; hook review warns with `heap_snapshot_retained_size_input_review_requires_review` for ready descriptors and blocks failed / blocked descriptors.

Boundary: this closes only the retained-size executor input review and approval-gate descriptor after the retained-path preflight. It does not read raw heap files, export raw heap chunks or raw strings, parse raw heap, compute retained size, compute path-to-root, record approval, start a transaction, write journals, execute retained-size or path-to-root executors, compute a new heap diff, execute a larger-budget second pass, prove complete object reachability, start browsers, send CDP commands, call MCP, or touch Android / iOS / mini-program full runtime chains.

Tests cover manager-level ready / blocked behavior, native-web no-browser route metadata, workspace contract aliasing, coordinator payload/category mapping, hook review warning / blocker behavior, compileall, and related page mutation / native-web / workspace / coordinator / hook subagent regressions.

### Step 329：Heap snapshot retained-size approval / transaction plan descriptor baseline

Status: implemented as a read-only / review-only / approval-plan-only / transaction-plan-only descriptor after the Step 328 retained-size executor input review descriptor. This is not retained-size proof, not path-to-root analysis, not raw heap file ingestion, not raw heap parsing, not heap diff computation, not approval recording, not transaction journal writing, not bounded gate writing, not browser launch, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`HeapSnapshotRetainedSizeApprovalPlanManager` consumes a ready `reverse-deepagent.heap-snapshot-retained-size-input-review.v1` descriptor, validates schema / status / approval-gate policy / disabled executor input contract / candidate inputs, and emits `reverse-deepagent.heap-snapshot-retained-size-approval-plan.v1` with approval-record plan, transaction-journal plan, bounded-gate artifact plan, result-artifact plan, candidate digest, and disabled executor input contract. It keeps `approval_recorded=false`, `transaction_started=false`, `journal_written_now=false`, `bounded_executor_gate_written=false`, `executor_invoked=false`, `raw_heap_loaded=false`, `raw_heap_parsed=false`, `raw_heap_exported=false`, `raw_strings_exported=false`, `heap_diff_computed=false`, `retained_size_proven=false`, `path_to_root_computed=false`, and `automatic_followup_analysis=false`.

`native-web` exposes this through `heap-snapshot-retained-size-approval-plan` / `heap-snapshot-retained-size-executor-approval-plan` / `heap-snapshot-retained-size-transaction-plan` / `review-heap-snapshot-retained-size-approval-plan` protection names before browser session acquisition and returns `virtual://workspace/heap-snapshot-retained-size-approval-plan.json`. The workspace contract indexes `workspace/heap-snapshot-retained-size-approval-plan.json` with future alias `/workspace/browser/heap-snapshot-retained-size-approval-plan.json` and category `triage`; coordinator extraction / artifact category mapping classify `heap_snapshot_retained_size_approval_plan` evidence as `triage`; hook review warns with `heap_snapshot_retained_size_approval_plan_requires_review` for ready descriptors and blocks failed / blocked descriptors.

Boundary: this closes only the approval / transaction planning layer after retained-size input review. It does not record approval, start a transaction, write journals, write bounded gates, invoke retained-size or path-to-root executors, read raw heap files, export raw heap chunks or raw strings, parse raw heap, compute retained size, compute path-to-root, compute a new heap diff, execute a larger-budget second pass, prove complete object reachability, start browsers, send CDP commands, call MCP, or touch Android / iOS / mini-program full runtime chains.

Tests cover manager-level ready / blocked behavior, native-web no-browser route metadata, workspace contract aliasing, coordinator payload/category mapping, hook review warning / blocker behavior, compileall, and related page mutation / native-web / workspace / coordinator / hook subagent regressions.

### Step 330：Heap snapshot retained-size approval-record writer baseline

Status: implemented as an explicit-review-only approval-record writer after the Step 329 retained-size approval / transaction plan descriptor. This is not retained-size proof, not path-to-root analysis, not raw heap file ingestion, not raw heap parsing, not heap diff computation, not transaction preflight, not transaction journal writing, not bounded gate writing, not browser launch, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`record_heap_snapshot_retained_size_approval` consumes a ready `reverse-deepagent.heap-snapshot-retained-size-approval-plan.v1` descriptor from JSON or artifact-ref, verifies reviewer / decision / mode / write-result / explicit `approve_heap_snapshot_retained_size` gate plus optional approval-plan id / transaction-plan id / candidate digest / descriptor digest guards, and writes `reverse-deepagent.heap-snapshot-retained-size-approval-record.v1` to `workspace/heap-snapshot-retained-size-approval-record.json` only when `mode=apply`, `write_result=true`, `approve_heap_snapshot_retained_size=true`, reviewer is non-empty, and all plan gates pass. Dry-run remains read-only and blocked attempts write nothing.

The hook subagent exposes `record_heap_snapshot_retained_size_approval` alongside heap snapshot diff approval / transaction journal writers. The workspace contract indexes `workspace/heap-snapshot-retained-size-approval-record.json` with future alias `/workspace/browser/heap-snapshot-retained-size-approval-record.json` and category `audit`; coordinator extraction / artifact category mapping classify `heap_snapshot_retained_size_approval_record` evidence as `audit`; hook review warns when a written approved record requires retained-size transaction preflight follow-up and blocks blocked records.

Boundary: this closes only the explicit approval-record layer after retained-size approval / transaction planning. It records approval metadata only, writes no transaction journal, starts no transaction, writes no bounded gate, invokes no retained-size or path-to-root executor, loads / parses / exports no raw heap or raw strings, computes no heap diff, proves no retained size, computes no path-to-root, starts no browser, sends no CDP, calls no MCP, and touches no Android / iOS / mini-program full runtime chain. Retained-size transaction preflight is implemented separately in Step 331; retained-size transaction journal writer, retained-size executor MVP and path-to-root executor MVP remain separate follow-up work.

Tests cover dry-run read-only behavior, apply gate blockers, apply-write-only behavior, hook subagent tool exposure / prompt guidance, hook review warning / blocker behavior, workspace contract aliasing, coordinator payload/category mapping, compileall, and related hook / workspace / coordinator regressions.

### Step 331：Heap snapshot retained-size transaction-preflight descriptor baseline

Status: implemented as a read-only / review-only / transaction-preflight-only descriptor after the Step 330 retained-size approval-record writer. This is not transaction journal writing, not bounded gate writing, not retained-size proof, not path-to-root analysis, not raw heap file ingestion, not raw heap parsing, not heap diff computation, not browser launch, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`HeapSnapshotRetainedSizeTransactionPreflightManager` consumes a ready `reverse-deepagent.heap-snapshot-retained-size-approval-plan.v1` descriptor plus a written approved `reverse-deepagent.heap-snapshot-retained-size-approval-record.v1` descriptor, verifies approval-plan id / transaction-plan id / candidate digest / optional plan digest guards, approval-record status, approved execution flag, and absence of prior transaction / journal / bounded-gate / executor / raw-heap / retained-size / path-to-root side effects. It emits `reverse-deepagent.heap-snapshot-retained-size-transaction-preflight.v1` with approval summary, transaction summary, candidate summary, guard summary, `journal_writer_contract.ready_for_journal_review=true`, `future_executor_contract.implemented=false`, `safety_gates.ready_to_write_journal=true`, and fixed `ready_to_execute_now=false`.

`native-web` exposes this through `heap-snapshot-retained-size-transaction-preflight` / `heap-snapshot-retained-size-executor-transaction-preflight` / `review-heap-snapshot-retained-size-transaction-preflight` protection names before browser session acquisition and returns `virtual://workspace/heap-snapshot-retained-size-transaction-preflight.json`. The workspace contract indexes `workspace/heap-snapshot-retained-size-transaction-preflight.json` with future alias `/workspace/browser/heap-snapshot-retained-size-transaction-preflight.json` and category `triage`; coordinator extraction / artifact category mapping classify `heap_snapshot_retained_size_transaction_preflight` evidence as `triage`; hook review warns with `heap_snapshot_retained_size_transaction_preflight_ready_for_journal_writer` for ready descriptors and blocks failed / blocked descriptors.

Boundary: this closes only the retained-size transaction preflight before a future transaction journal writer. It does not start a transaction, write a journal, write a bounded gate, invoke retained-size or path-to-root executors, read raw heap files, export raw heap chunks or raw strings, parse raw heap, compute retained size, compute path-to-root, compute a new heap diff, execute a larger-budget second pass, prove complete object reachability, start browsers, send CDP commands, call MCP, or touch Android / iOS / mini-program full runtime chains. Retained-size transaction journal writer is implemented separately in Step 332; retained-size bounded executor gate is implemented separately in Step 333; retained-size executor MVP and path-to-root executor MVP remain separate follow-up work.

Tests cover manager-level ready / blocked behavior, native-web no-browser route metadata, workspace contract aliasing, coordinator payload/category mapping, hook review warning / blocker behavior, compileall, and related page mutation / native-web / workspace / coordinator / hook subagent regressions.

### Step 332：Heap snapshot retained-size transaction journal writer baseline

Status: implemented as an explicit-review-only transaction journal writer after the Step 331 retained-size transaction-preflight descriptor. This is not bounded gate writing, not retained-size proof, not path-to-root analysis, not raw heap file ingestion, not raw heap parsing, not heap diff computation, not browser launch, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`record_heap_snapshot_retained_size_transaction_journal` consumes a ready `reverse-deepagent.heap-snapshot-retained-size-transaction-preflight.v1` descriptor from JSON or artifact-ref, verifies reviewer / mode / write-result / explicit `approve_transaction_journal` gate plus optional approval-plan id / transaction-plan id / candidate digest / transaction-preflight digest guards, blocks if the target journal already exists, and writes `reverse-deepagent.heap-snapshot-retained-size-transaction-journal.v1` to `workspace/heap-snapshot-retained-size-executor-journal.json` only when `mode=apply`, `write_result=true`, `approve_transaction_journal=true`, reviewer is non-empty, and all preflight gates pass. Dry-run remains read-only and blocked attempts write nothing.

The hook subagent exposes `record_heap_snapshot_retained_size_transaction_journal` alongside retained-size approval-record writer and heap snapshot diff journal writers. The workspace contract indexes `workspace/heap-snapshot-retained-size-executor-journal.json` with future alias `/workspace/browser/heap-snapshot-retained-size-executor-journal.json` and category `audit`; coordinator extraction / artifact category mapping classify `heap_snapshot_retained_size_transaction_journal` evidence as `audit`; hook review warns with `heap_snapshot_retained_size_transaction_journal_ready_for_bounded_gate` for written journals and blocks failed / blocked journal descriptors.

Boundary: this closes only the retained-size transaction journal layer after transaction preflight. It may mark `transaction_started=true` and `journal_written=true` as audit metadata, but it writes no bounded gate, invokes no retained-size or path-to-root executor, loads / parses / exports no raw heap or raw strings, computes no heap diff, proves no retained size, computes no path-to-root, starts no browser, sends no CDP, calls no MCP, and touches no Android / iOS / mini-program full runtime chain. Retained-size executor MVP and path-to-root executor MVP remain separate follow-up work.

Tests cover dry-run read-only behavior, apply gate blockers, apply-write-only behavior, hook subagent tool exposure / prompt guidance, hook review warning behavior, workspace contract aliasing, coordinator payload/category mapping, compileall, and related hook / workspace / coordinator regressions.

### Step 333：Heap snapshot retained-size bounded executor gate descriptor baseline

Status: implemented as a read-only / review-only / bounded-gate-only descriptor after the Step 332 retained-size transaction journal writer. This is not retained-size proof, not path-to-root analysis, not raw heap file ingestion, not raw heap parsing, not heap diff computation, not browser launch, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`HeapSnapshotRetainedSizeBoundedGateManager` consumes a written `reverse-deepagent.heap-snapshot-retained-size-transaction-journal.v1` descriptor, verifies journal schema / written status / transaction-start audit metadata / approval-plan id / transaction-plan id / candidate digest / optional guard digests, confirms the bounded-gate follow-up and explicit executor review gates, and rejects any journal that already claims bounded-gate, executor, raw-heap, retained-size, path-to-root, complete-traversal, browser, CDP, MCP, or mobile side effects. It emits `reverse-deepagent.heap-snapshot-retained-size-bounded-gate.v1` with `bounded_executor_gate_ready_for_review=true`, disabled `ready_to_execute_now=false`, and a future executor contract for `execute_heap_snapshot_retained_size_analysis` with `implemented=false` until Step 334 supplies the separate explicit-review-only estimate executor MVP.

`native-web` exposes this through `heap-snapshot-retained-size-bounded-gate` / `heap-snapshot-retained-size-bounded-executor-gate` / `heap-snapshot-retained-size-executor-bounded-gate` / `review-heap-snapshot-retained-size-bounded-gate` protection names before browser session acquisition and returns `virtual://workspace/heap-snapshot-retained-size-bounded-gate.json`. The workspace contract indexes `workspace/heap-snapshot-retained-size-bounded-gate.json` with future alias `/workspace/browser/heap-snapshot-retained-size-bounded-gate.json` and category `triage`; coordinator extraction / artifact category mapping classify `heap_snapshot_retained_size_bounded_gate` evidence as `triage`; hook review warns with `heap_snapshot_retained_size_bounded_gate_ready_for_executor_review` for ready descriptors and blocks failed / blocked descriptors.

Boundary: this closes only the retained-size bounded executor gate layer after transaction journal writing. It writes no artifact from the manager, invokes no retained-size or path-to-root executor, loads / parses / exports no raw heap or raw strings, computes no heap diff, proves no retained size, computes no path-to-root, starts no browser, sends no CDP, calls no MCP, and touches no Android / iOS / mini-program full runtime chain. Retained-size executor MVP and path-to-root executor MVP remain separate follow-up work.

Tests cover manager-level ready / blocked behavior, native-web no-browser route metadata, hook review warning / blocker behavior, workspace contract aliasing, coordinator payload/category mapping, compileall, and related page mutation / native-web / hook / workspace / coordinator regressions.

### Step 334：Heap snapshot retained-size executor MVP

Status: implemented as an explicit-review-only bounded retained-size candidate estimate executor after the Step 333 retained-size bounded executor gate descriptor. This is not retained-size proof, not path-to-root analysis, not complete heap graph traversal, not raw heap file ingestion / export, not browser launch, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`HeapSnapshotRetainedSizeExecutorManager` consumes a ready `reverse-deepagent.heap-snapshot-retained-size-bounded-gate.v1` descriptor plus a caller-provided V8 heap snapshot JSON object. It requires `mode=apply`, `review_approved=true`, `approve_heap_snapshot_retained_size_execution=true`, reviewer, candidate names, and bounded raw-heap size / node budgets. It emits `reverse-deepagent.heap-snapshot-retained-size-analysis.v1` and `workspace/heap-snapshot-retained-size-analysis.json` / `virtual://workspace/heap-snapshot-retained-size-analysis.json` with redacted candidate self-size and directly-owned local retained-size estimates, while keeping `raw_heap_exported=false`, `raw_strings_exported=false`, `retained_size_proven=false`, `path_to_root_computed=false`, and `complete_heap_traversal_claimed=false`.

`native-web` exposes this through `execute-heap-snapshot-retained-size-analysis` / `heap-snapshot-retained-size-analysis` / `heap-snapshot-retained-size-executor-result` / `heap-snapshot-retained-size-executor-mvp` and returns `virtual://workspace/heap-snapshot-retained-size-analysis.json` without starting the browser provider. The workspace contract indexes `workspace_heap_snapshot_retained_size_analysis` under `/workspace/browser/` as an `audit` artifact, coordinator payload mapping writes `heap-snapshot-retained-size-analysis.json`, and hook review reports `heap_snapshot_retained_size_analysis_ready_for_path_to_root_or_second_pass_review` for executed estimate results.

Boundary: this closes only the retained-size estimate MVP for caller-provided V8 heap snapshots. It does not read heap snapshots from paths, write or export raw heap chunks, export raw string tables, compute retained-size proof, compute path-to-root, prove complete object reachability, run constructor drilldown execution, run larger-budget second pass, start browsers, send CDP commands, call MCP, or touch Android / iOS / mini-program full runtime chains. Path-to-root proof, proof-oriented retained-size analysis, raw-heap constructor drilldown proof, larger-budget second pass, and automatic follow-up analysis remain capability-gated follow-up work.

### Step 335：Heap snapshot path-to-root executor MVP

Status: implemented as an explicit-review-only bounded path-to-root candidate estimate executor after the Step 334 retained-size estimate MVP. This is not path-to-root proof, not retained-size proof, not complete heap graph traversal, not raw heap file ingestion / export, not browser launch, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`HeapSnapshotPathToRootExecutorManager` consumes an executed `reverse-deepagent.heap-snapshot-retained-size-analysis.v1` result or a ready retained-size / path-to-root preflight descriptor plus a caller-provided V8 heap snapshot JSON object. It requires `mode=apply`, `review_approved=true`, `approve_heap_snapshot_path_to_root_execution=true`, reviewer, candidate names, and bounded raw-heap / node / depth budgets. It emits `reverse-deepagent.heap-snapshot-path-to-root-analysis.v1` and `workspace/heap-snapshot-path-to-root-analysis.json` / `virtual://workspace/heap-snapshot-path-to-root-analysis.json` with redacted bounded incoming-edge path estimates, while keeping `raw_heap_exported=false`, `raw_strings_exported=false`, `path_to_root_proven=false`, `retained_size_proven=false`, and `complete_heap_traversal_claimed=false`.

`native-web` exposes this through `execute-heap-snapshot-path-to-root-analysis` / `heap-snapshot-path-to-root-analysis` / `heap-snapshot-path-to-root-executor-result` / `heap-snapshot-path-to-root-executor-mvp` and returns `virtual://workspace/heap-snapshot-path-to-root-analysis.json` without starting the browser provider. The workspace contract indexes `workspace_heap_snapshot_path_to_root_analysis` under `/workspace/browser/` as an `audit` artifact, coordinator payload mapping writes `heap-snapshot-path-to-root-analysis.json`, and hook review reports `heap_snapshot_path_to_root_analysis_ready_for_second_pass_or_constructor_drilldown_review` for executed estimate results.

Boundary: this closes only the path-to-root estimate MVP for caller-provided V8 heap snapshots. It does not read heap snapshots from paths, write or export raw heap chunks, export raw string tables, prove root reachability, compute retained-size proof, prove complete object reachability, run constructor drilldown execution, run larger-budget second pass, start browsers, send CDP commands, call MCP, or touch Android / iOS / mini-program full runtime chains. Proof-oriented retained-size analysis, proof-oriented path-to-root analysis, raw-heap constructor drilldown proof, larger-budget second pass, and automatic follow-up analysis remain capability-gated follow-up work.

### Step 336：Heap snapshot constructor-growth drilldown executor MVP

Status: implemented as an explicit-review-only descriptor-backed constructor-growth drilldown analysis executor after the Step 335 path-to-root estimate MVP. This is not raw heap drilldown proof, not retained-size proof, not path-to-root proof, not complete heap graph traversal, not raw heap file ingestion / export, not browser launch, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`HeapSnapshotConstructorGrowthDrilldownExecutorManager` consumes a ready `reverse-deepagent.heap-snapshot-constructor-growth-drilldown.v1` review descriptor. It requires `mode=apply`, `review_approved=true`, `approve_heap_snapshot_constructor_growth_drilldown_execution=true`, reviewer, and bounded candidate limits. It emits `reverse-deepagent.heap-snapshot-constructor-growth-drilldown-analysis.v1` and `workspace/heap-snapshot-constructor-growth-drilldown-analysis.json` / `virtual://workspace/heap-snapshot-constructor-growth-drilldown-analysis.json` with descriptor-backed constructor growth prioritization rows, growth scores, severity labels, and retained-size / path-to-root follow-up review actions, while keeping `raw_heap_loaded=false`, `raw_heap_exported=false`, `heap_diff_computed=false`, `constructor_drilldown_proven=false`, `retained_size_proven=false`, `path_to_root_computed=false`, and `complete_heap_traversal_claimed=false`.

`native-web` exposes this through `execute-heap-snapshot-constructor-growth-drilldown` / `execute-heap-snapshot-constructor-growth-drilldown-analysis` / `heap-snapshot-constructor-growth-drilldown-analysis` / `heap-snapshot-constructor-growth-drilldown-executor-result` and returns `virtual://workspace/heap-snapshot-constructor-growth-drilldown-analysis.json` without starting the browser provider. The workspace contract indexes `workspace_heap_snapshot_constructor_growth_drilldown_analysis` under `/workspace/browser/` as an `audit` artifact, coordinator payload mapping writes `heap-snapshot-constructor-growth-drilldown-analysis.json`, and hook review reports `heap_snapshot_constructor_growth_drilldown_analysis_ready_for_retained_size_path_or_second_pass_review` for executed analysis results.

Boundary: this closes only the descriptor-backed constructor-growth drilldown analysis MVP. It does not read heap snapshots from paths, parse raw heap, write or export raw heap chunks, export raw string tables, prove constructor reachability, compute retained-size proof, compute path-to-root proof, run larger-budget second pass, start browsers, send CDP commands, call MCP, or touch Android / iOS / mini-program full runtime chains. Raw-heap constructor drilldown proof, proof-oriented retained-size / path-to-root analysis, larger-budget second pass, and automatic follow-up analysis remain capability-gated follow-up work.

### Step 337：Heap snapshot automatic follow-up planner baseline

Status: implemented as a read-only / review-only / plan-only heap follow-up planner after the Step 336 descriptor-backed constructor-growth drilldown analysis MVP. This is not retained-size proof, not path-to-root proof, not raw heap file ingestion, not raw heap parsing, not heap diff recomputation, not larger-budget second-pass execution, not browser launch, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`HeapSnapshotAutomaticFollowupPlanManager` consumes existing executed `reverse-deepagent.heap-snapshot-retained-size-analysis.v1`, `reverse-deepagent.heap-snapshot-path-to-root-analysis.v1`, and `reverse-deepagent.heap-snapshot-constructor-growth-drilldown-analysis.v1` descriptors, validates their schema / executed status / no raw export / no proof claims / no browser, CDP, MCP, or mobile side effects, and emits `reverse-deepagent.heap-snapshot-automatic-followup-plan.v1` with prioritized manual next actions. The descriptor keeps `review_only=true`, `plan_only=true`, `raw_heap_loaded=false`, `raw_heap_parsed=false`, `raw_heap_exported=false`, `raw_strings_exported=false`, `heap_diff_computed=false`, `new_heap_diff_computed=false`, `retained_size_estimated_now=false`, `retained_size_proven=false`, `path_to_root_estimated_now=false`, `path_to_root_proven=false`, `constructor_drilldown_computed_now=false`, `constructor_drilldown_proven=false`, `complete_heap_traversal_claimed=false`, `automatic_followup_analysis=false`, and `automatic_execution_allowed=false`.

`native-web` exposes this through `heap-snapshot-automatic-followup-plan` / `heap-snapshot-automatic-followup-planner` / `heap-snapshot-followup-plan` / `plan-heap-snapshot-automatic-followup` / `review-heap-snapshot-automatic-followup-plan` protection names before browser session acquisition and returns `virtual://workspace/heap-snapshot-automatic-followup-plan.json` without starting the browser provider. The workspace contract indexes `workspace/heap-snapshot-automatic-followup-plan.json` with future alias `/workspace/browser/heap-snapshot-automatic-followup-plan.json` and category `triage`; coordinator extraction / artifact category mapping classify `heap_snapshot_automatic_followup_plan` evidence as `triage`; hook review warns with `heap_snapshot_automatic_followup_plan_ready_for_proof_or_second_pass_review` for ready descriptors and blocks failed / blocked descriptors.

Boundary: this closes only the automatic follow-up planner baseline. It recommends manual review actions such as combined candidate evidence review, proof-grade retained-size / path-to-root planning, raw-heap constructor drilldown proof planning, and larger-budget second-pass planning, but it does not execute any recommendation, invoke retained-size / path-to-root / constructor executors, read heap snapshots from paths, parse or export raw heap chunks, export raw string tables, recompute heap diffs, prove retained size, prove path-to-root, prove constructor reachability, claim complete heap traversal, start browsers, send CDP commands, call MCP, or touch Android / iOS / mini-program full runtime chains. Proof-oriented retained-size / path-to-root analysis, raw-heap constructor drilldown proof, raw heap file ingestion / export, larger-budget second-pass execution, and automatic follow-up execution remain capability-gated follow-up work.

Tests cover manager-level ready / blocked behavior, native-web no-browser route metadata, workspace contract aliasing, coordinator payload/category mapping, hook review warning / blocker behavior, compileall, and related page mutation / native-web / workspace / coordinator / hook subagent regressions.

### Step 338：Heap snapshot retained-size proof plan descriptor baseline

Status: implemented as a read-only / review-only / plan-only retained-size proof planning descriptor after the Step 337 automatic follow-up planner. This is not retained-size proof, not raw heap ingestion, not dominator-tree computation, not browser launch, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`HeapSnapshotRetainedSizeProofPlanManager` consumes an executed `reverse-deepagent.heap-snapshot-retained-size-analysis.v1` bounded estimate descriptor plus an optional ready `reverse-deepagent.heap-snapshot-automatic-followup-plan.v1` descriptor, validates schema / status / candidate estimates / no raw export / no proof claims / no forbidden browser, CDP, MCP, or mobile side effects, and emits `reverse-deepagent.heap-snapshot-retained-size-proof-plan.v1` with candidate proof inputs, raw heap ingestion preflight requirements, parser sandbox / redaction / size-budget requirements, dominator-tree / ownership-graph requirements, and a disabled future executor contract for `execute_heap_snapshot_retained_size_proof`. The descriptor keeps `review_only=true`, `plan_only=true`, `proof_plan_only=true`, `future_executor_contract.implemented=false`, `future_executor_contract.ready_to_execute_now=false`, `raw_heap_loaded=false`, `raw_heap_parsed=false`, `raw_heap_exported=false`, `raw_strings_exported=false`, `heap_diff_computed=false`, `retained_size_proven=false`, `complete_heap_traversal_claimed=false`, `proof_executor_invoked=false`, and `automatic_execution_allowed=false`.

`native-web` exposes this through `heap-snapshot-retained-size-proof-plan` / `heap-snapshot-retained-size-proof-planner` / `plan-heap-snapshot-retained-size-proof` / `review-heap-snapshot-retained-size-proof-plan` protection names before browser session acquisition and returns `virtual://workspace/heap-snapshot-retained-size-proof-plan.json` without starting the browser provider. The workspace contract indexes `workspace/heap-snapshot-retained-size-proof-plan.json` with future alias `/workspace/browser/heap-snapshot-retained-size-proof-plan.json` and category `triage`; coordinator extraction / artifact category mapping classify `heap_snapshot_retained_size_proof_plan` evidence as `triage`; hook review warns with `heap_snapshot_retained_size_proof_plan_ready_for_raw_heap_ingestion_review` for ready descriptors and blocks failed / blocked descriptors.

Boundary: this closes only the retained-size proof planning layer. It does not ingest raw heap, read heap snapshot paths, parse or export raw heap chunks, export raw string tables, compute a dominator tree, compute retained-size proof, prove complete object reachability, invoke retained-size / path-to-root / constructor executors, run larger-budget second pass, start browsers, send CDP commands, call MCP, or touch Android / iOS / mini-program full runtime chains. The actual retained-size proof executor, path-to-root proof planning / execution, raw-heap constructor drilldown proof, raw heap file ingestion / export policy, larger-budget second-pass execution, and automatic follow-up execution remain capability-gated follow-up work.

Tests cover manager-level ready / blocked behavior, native-web no-browser route metadata, workspace contract aliasing, coordinator payload/category mapping, hook review warning / blocker behavior, compileall, and related page mutation / native-web / workspace / coordinator / hook subagent regressions.

### Step 339：Heap snapshot path-to-root proof plan descriptor baseline

Status: implemented as a read-only / review-only / plan-only path-to-root proof planning descriptor after the Step 338 retained-size proof plan descriptor. This is not path-to-root proof, not raw heap ingestion, not full incoming-edge graph traversal, not browser launch, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`HeapSnapshotPathToRootProofPlanManager` consumes an executed `reverse-deepagent.heap-snapshot-path-to-root-analysis.v1` bounded path estimate descriptor plus optional executed retained-size estimate, ready automatic follow-up plan, and ready retained-size proof plan descriptors. It validates schema / status / candidate path rows / no raw export / no proof claims / no forbidden browser, CDP, MCP, or mobile side effects, and emits `reverse-deepagent.heap-snapshot-path-to-root-proof-plan.v1` with candidate path proof inputs, raw heap ingestion preflight requirements, parser sandbox / redaction / size-budget requirements, root-set policy, full incoming-edge walk / cycle-detection requirements, and a disabled future executor contract for `execute_heap_snapshot_path_to_root_proof`. The descriptor keeps `review_only=true`, `plan_only=true`, `proof_plan_only=true`, `future_executor_contract.implemented=false`, `future_executor_contract.ready_to_execute_now=false`, `raw_heap_loaded=false`, `raw_heap_parsed=false`, `raw_heap_exported=false`, `raw_strings_exported=false`, `heap_diff_computed=false`, `path_to_root_proven=false`, `complete_heap_traversal_claimed=false`, `proof_executor_invoked=false`, and `automatic_execution_allowed=false`.

`native-web` exposes this through `heap-snapshot-path-to-root-proof-plan` / `heap-snapshot-path-to-root-proof-planner` / `plan-heap-snapshot-path-to-root-proof` / `review-heap-snapshot-path-to-root-proof-plan` protection names before browser session acquisition and returns `virtual://workspace/heap-snapshot-path-to-root-proof-plan.json` without starting the browser provider. The workspace contract indexes `workspace/heap-snapshot-path-to-root-proof-plan.json` with future alias `/workspace/browser/heap-snapshot-path-to-root-proof-plan.json` and category `triage`; coordinator extraction / artifact category mapping classify `heap_snapshot_path_to_root_proof_plan` evidence as `triage`; hook review warns with `heap_snapshot_path_to_root_proof_plan_ready_for_raw_heap_ingestion_review` for ready descriptors and blocks failed / blocked descriptors.

Boundary: this closes only the path-to-root proof planning layer. It does not ingest raw heap, read heap snapshot paths, parse or export raw heap chunks, export raw string tables, walk the full object graph, prove root reachability, prove complete object reachability, invoke retained-size / path-to-root / constructor executors, run larger-budget second pass, start browsers, send CDP commands, call MCP, or touch Android / iOS / mini-program full runtime chains. The actual path-to-root proof executor, raw-heap constructor drilldown proof, raw heap file ingestion / export policy, larger-budget second-pass execution, and automatic follow-up execution remain capability-gated follow-up work.

Tests cover manager-level ready / blocked behavior, native-web no-browser route metadata, workspace contract aliasing, coordinator payload/category mapping, hook review warning / blocker behavior, compileall, and related page mutation / native-web / workspace / coordinator / hook subagent regressions.


### Step 340：Heap snapshot raw-heap constructor drilldown proof plan descriptor baseline

Status: implemented as a read-only / review-only / plan-only raw-heap constructor drilldown proof planning descriptor after the Step 339 path-to-root proof plan descriptor. This is not raw-heap constructor proof, not raw heap ingestion, not full retainer / dominator traversal, not browser launch, not CDP execution, not MCP integration, and not an Android / iOS / mini-program full runtime chain.

`HeapSnapshotRawHeapConstructorDrilldownProofPlanManager` consumes an executed `reverse-deepagent.heap-snapshot-constructor-growth-drilldown-analysis.v1` descriptor plus optional ready automatic follow-up, retained-size proof plan, and path-to-root proof plan descriptors. It validates schema / status / candidate constructor rows / no raw heap load or export / no proof claims / no forbidden browser, CDP, MCP, or mobile side effects, and emits `reverse-deepagent.heap-snapshot-raw-heap-constructor-drilldown-proof-plan.v1` with candidate constructor proof inputs, raw heap ingestion preflight requirements, parser sandbox / redaction / size-budget requirements, constructor reachability graph / retainer edge index requirements, and a disabled future executor contract for `execute_heap_snapshot_raw_heap_constructor_drilldown_proof`. The descriptor keeps `review_only=true`, `plan_only=true`, `proof_plan_only=true`, `future_executor_contract.implemented=false`, `future_executor_contract.ready_to_execute_now=false`, `raw_heap_loaded=false`, `raw_heap_parsed=false`, `raw_heap_exported=false`, `raw_strings_exported=false`, `heap_diff_computed=false`, `constructor_drilldown_proven=false`, `retained_size_proven=false`, `path_to_root_proven=false`, `complete_heap_traversal_claimed=false`, `proof_executor_invoked=false`, and `automatic_execution_allowed=false`.

`native-web` exposes this through `heap-snapshot-raw-heap-constructor-drilldown-proof-plan` / `heap-snapshot-raw-heap-constructor-drilldown-proof-planner` / `plan-heap-snapshot-raw-heap-constructor-drilldown-proof` / `review-heap-snapshot-raw-heap-constructor-drilldown-proof-plan` protection names before browser session acquisition and returns `virtual://workspace/heap-snapshot-raw-heap-constructor-drilldown-proof-plan.json` without starting the browser provider. The workspace contract indexes `workspace/heap-snapshot-raw-heap-constructor-drilldown-proof-plan.json` with future alias `/workspace/browser/heap-snapshot-raw-heap-constructor-drilldown-proof-plan.json` and category `triage`; coordinator extraction / artifact category mapping classify `heap_snapshot_raw_heap_constructor_drilldown_proof_plan` evidence as `triage`; hook review warns with `heap_snapshot_raw_heap_constructor_drilldown_proof_plan_ready_for_raw_heap_ingestion_review` for ready descriptors and blocks failed / blocked descriptors.

Boundary: this closes only the raw-heap constructor drilldown proof planning layer. It does not ingest raw heap, read heap snapshot paths, parse or export raw heap chunks, export raw string tables, build a full constructor reachability graph, prove constructor reachability, compute retained-size proof, compute path-to-root proof, run larger-budget second pass, start browsers, send CDP commands, call MCP, or touch Android / iOS / mini-program full runtime chains. The actual raw-heap constructor drilldown proof executor, retained-size proof executor, path-to-root proof executor, raw heap file ingestion / export policy, larger-budget second-pass execution, and automatic follow-up execution remain capability-gated follow-up work.

Tests cover manager-level ready / blocked behavior, native-web no-browser route metadata, workspace contract aliasing, coordinator payload/category mapping, hook review warning / blocker behavior, and focused page mutation / native-web / workspace / coordinator / hook subagent regressions.
