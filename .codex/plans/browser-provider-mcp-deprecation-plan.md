...
- Boundary：这是 local SQLite transactional baseline；不联系 Redis / etcd / Postgres / MySQL，不提供跨机器 consensus，不自动 lease renewal，不强制 downstream fencing enforcement，也不替换现有 `delivery-transaction-lock.json` LocalDeliveryExecutor gate。
