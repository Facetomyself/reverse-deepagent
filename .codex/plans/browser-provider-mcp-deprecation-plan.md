...
- Boundary：这是显式审批 baseline，不自动覆盖同名 asset，不绕过 duplicate guard，不处理复杂 GitHub rate-limit / retry budget；provider-specific retry hardening、完整 recovery executor、cross-run rollback state machine 仍是后续工作。Android / iOS / 小程序完整运行链路继续搁置。
...
- Boundary：真正的 GitHub asset DELETE + replacement upload、partial failure recovery、provider-specific rate-limit / retry hardening 仍是后续步骤；Android / iOS / 小程序完整运行链路继续搁置。
...
- Boundary：ledger 是审计 baseline，不执行 publish / retry / recovery / rollback，不替代 duplicate guard，不绕过 review；dry-run 不写 ledger 文件；完整 recovery executor、cross-run rollback state machine、provider-specific retry / rate-limit policy 与 GitHub asset overwrite / delete 仍是后续工作。
...
- 后续仍需完整 recovery executor / cross-run rollback state machine / transaction idempotency hardening beyond ledger baseline / GitHub Release asset 覆盖或删除 / provider-specific retry hardening；Android / iOS / 小程序完整运行链路继续搁置。
...
- 后续仍需 DeepAgents 虚拟文件夹物理迁移 / broader consumer adoption、完整 recovery executor / rollback state machine / transaction idempotency hardening beyond ledger baseline、GitHub Release asset 覆盖或删除 / provider-specific retry hardening，以及更高级 Browser/CDP 调试能力；Android / iOS / 小程序完整运行链路继续搁置。
...
- 后续仍需 GitHub Release asset 覆盖或删除 / provider-specific retry hardening，以及完整 recovery executor / cross-run rollback state machine / transaction idempotency hardening beyond ledger baseline；本 baseline 不管理云 SDK、bucket、credentials，也不绕过 duplicate guard。
...
- 后续仍需 GitHub Release asset 覆盖或删除 / provider-specific retry hardening 与更多第三方 provider，以及完整 recovery executor / rollback state machine / transaction idempotency hardening beyond ledger baseline；webhook 与 presigned object-storage provider 已有内置 baseline。
